...
41. **Heights, Mordell--Weil, and the Faltings--Tate Reduction** — Construct Weil and
   ...
   cancellation after Northcott; its moduli route still requires the arithmetic Siegel height
   package, which is isolated rather than cited as an unsourced Faltings theorem, so Tate
   semisimplicity remains conditional.
...
138. **Integral Local Types and Type Lattices** — Construct integral primary and companion lattices for principal, special, depth-zero dihedral, and tame positive-depth local types, together with their represented enhanced deformation conditions. Realize the Steinberg flag rows on the generic parahoric tower and identify their correct bad-fiber extension as a strict procyclic constructible complex, conditional only at the precise principal node-and-stabilizer theorem $({\rm PNS}_v)$. Banality, ordering, sign, and saturation hypotheses are retained, and primitive wild or ramified dyadic positive-depth types are excluded.
...
157. **Local Geometry of Hilbert--Blumenthal Moduli** — Construct real, good, ordinary,
   ...
   package.  These point-centered opens do not
   themselves construct the integral slice or coset-complete good packets needed for
   a singleton mixed moving presentation.  Section 13.2A proves finite-field Bertini with imposed
   first jets and the stable relative complete-intersection slice once a compatible smooth
   projective joining model is supplied.  It proves the conditional handoff to Book 154 and
   isolates the exact remaining datum as the finite-type framed joining theorem: compatibility
   of one projective model and its ample coefficient spaces with the normalized paired-frame
   cusp completion and the good interior.
...
168. **Compatible Coefficient Systems and Purity** — Import Book 127's curve-carried weak family,
   ...
   Faltings moduli-height package; Book 41 proves the required finite polarized descent and
   Zarhin cancellation. Raw dyadic-carrier existence and the coefficient-prime boundary remain
   separate.
...
178. **Auxiliary Dihedral Data and Residual Potential Modularity** — Construct the two-prime Hilbert--Blumenthal specialization and dihedral residual packet from a supplied compatible corrected relative ray datum and a complete admissibility ledger. Prove relative-ray compatibility over the rational base and use an anti-cyclotomic Grunwald--Wang twist to impose finitely many exact protected residual branch characters, including unramified Frobenius values, without changing the determinant. Its pre-specialization ledger retains the degree-indexed allowed-good Abel condition needed for the unique special factor. The compact carrier is obtained in the selected range, and the cross-prime stable-lattice comparison supplies a nonzero upper target localization on the carrier ramified at the distinguished special place; it supplies no integral carrier switch there. Relative-ray compatibility remains conditional over a general totally real base; post-specialization auxiliary cleanability, actual target-lowering edges, and their integral comparison hypotheses remain conditional.

179. **Compatible Systems of Galois Representations** — Define and analyze compatible systems, coefficient fields, residual members, purity, and recognition from Frobenius data in the precise rank-two range needed later. The volume packages the already constructed curve systems and image criteria; its abelian-variety examples use Book 41's conditional Tate theorem. The exact input is finite isogeny classes, while the displayed uniform source remains the Faltings--Shafarevich moduli-height package; Book 41 proves finite polarized descent and integral factor cancellation. It does not create a descended base-field family or an integral lattice at a new coefficient prime.
