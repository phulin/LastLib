...
176. **Nonminimal Modularity Lifting** — Import the conditional one-prime Ihara, type-complex,
   ...
   modules, and formal recovery. Proposition 1.1E proves that even faithful balanced patching
   and maximal Cohen--Macaulay support do not imply the two missing arithmetic assertions: the
   ...
   and occurrence on every avoidance component $({\rm ACT}^{\rm av}_S)$. Proposition 1.1
   proves component-support transfer once those two global facts hold. Their conjunction is
   the single reusable componentwise Ihara--avoidance theorem $({\rm CIAV}_P)$, used once at
   the auxiliary set and once at the target set; no current alternative is smaller and
   source-closed. Lemma 1.1A
