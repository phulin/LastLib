...
47. **Betti, de Rham, and Étale Comparison for Curves** — Construct singular (co)homology,
   ...
   curve-cohomology subquotients and their one-dimensional constituents.
...
168. **Compatible Coefficient Systems and Purity** — Import Book 127's curve-carried weak family,
   ...
   Book 47 proves inheritance by geometric subquotients, so the sole missing input in this route
   is smooth-proper Hodge--Tate comparison; no Faltings-height or ambient Tate-semisimplicity
...
176. **Nonminimal Modularity Lifting** — Import the conditional one-prime Ihara, type-complex,
   ...
   Ihara-avoidance route $({\rm IAV}_S)$: paired desired/avoidance local rings with a common
   special fiber, synchronized Taylor--Wiles systems, and patched component-support transfer.
   This is the scope-minimal route for the FLT consumer, is not supplied by minimal $R=T$,
   and does not require a several-place old--new cube. For the geometric alternative, reuse Book 84's one-global-module
...
177. **Potential Modularity of Two-Dimensional Representations** — Construct the paired-frame potential-modularity bridge and prove its residual automorphy conclusion only for a lifting-admissible specialization. The automorphic target packet and stable-lattice comparison give a nonzero upper target localization on the selected compact carrier, but no target-prime level lowering or integral switch to a carrier split at its finite ramification place follows from that fact alone. Complete splitting makes every place above the auxiliary base place active, so a one-prime theorem cannot simply be iterated. The scope-minimal missing theorem is Book 176's direct finite-set Ihara-avoidance package $({\rm IAV}_{P_{\rm act}})$, including paired local rings, synchronized patching, and special-fiber support transfer. On the geometric alternatives, localized Ihara remains the first gap; the all-point route then needs mixed exactness, iterated-switch product residue, full joint support, coherent simultaneous augmentation, and finite-set reciprocity, while the pointed route needs the final joint cube, simultaneous branch verification and reciprocity on its torsion-free top carrier, and occurrence of the one global product component through the Tate point. Thus the advertised unconditional existence theorem is not ready.
...
183. **Brauer Induction for Automorphy Families** — Construct the elementary fixed-field packet array, overlap identities, integral Brauer classes, common polynomials, and virtual Frey-special local record from a supplied controlled top datum. Retain the common unramified algebraic Galois pair for every elementary packet away from two and show that finite twisting and Weil induction form the auxiliary local Mackey objects. Prove an independent uniform raw-to-global certificate for the whole finite array: under $(\mathrm{HT}_1)$, cuspidality makes every attached semisimple packet realization absolutely irreducible, hence the raw representation equals it and the dyadic monodromy operator, line, and sign survive at every odd coefficient place. The rank-one local-algebraicity and geometric-subquotient reductions are proved; the exact remaining arithmetic input is smooth-proper Hodge--Tate comparison. Ambient Tate semisimplicity is only a stronger optional alternative. Effectivity is deliberately deferred, and neither the missing raw dyadic carrier nor the controlled seed is supplied.
