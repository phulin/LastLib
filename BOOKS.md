...
176. **Nonminimal Modularity Lifting** — Import the conditional one-prime Ihara, type-complex,
   ...
   desired components specialize bijectively, while the raw desired ring itself is
   ...
   modules, and formal recovery. The first missing theorem is now
   $({\rm PGC}_S)$, preservation of that component bijection after quotienting by the actual
   global patched relation ideal; avoidance full support remains the subsequent support
   assertion. Proposition 1.1 proves component-support transfer once those two global facts
   hold. Lemma 1.1A
...
181. **Finite Image and the Balanced Minimal-Lift Argument** — Turn the balanced hardly-ramified presentation into a normalized primitive special lift. The finite-image and complete-local-algebra argument consumes only the restricted-special-fibre certificate $({\rm SF}_{\rm SP})$: finiteness of the restricted scalar special fiber plus the exact local quotient. From it, finite restricted image, finite-index passage, Carayol trace descent, and topological Nakayama give finite scalar image, whole-ring finite flatness, and a horizontal point. On the preferred split route, after Book 176's auxiliary $({\rm IAV}_{P_{\rm aux}})$ handoff and Book 178's upper packet, form the target broad-unipotent ring. The exact upper special ring does not surject onto the minimal ring because its line/sign/Frobenius incidence persists on the zero-monodromy boundary, but the broad ring does surject by setting inertia to one. A target certificate $({\rm TIAV}_{P_{\rm tar}})$—whose local pair and residual synchronization are now proved, but whose $({\rm PGC}_{P_{\rm tar}})$ and avoidance-full-support clauses remain—gives that broad ring a finite automorphic action with nilpotent kernel, hence makes the whole minimal special fibre finite; after the point is constructed, the same support theorem makes it automorphic. Thus auxiliary and target IAV are two coefficient-specific instances of one reusable theorem schema. The stronger $({\rm DMS}_{P_{\rm tar}})$ route through $({\rm FLO}_v)$ remains optional for exact minimal $R=\mathbb T$. The chosen point is never used backward to prove its finite-fibre certificate.

182. **Potential Automorphy and Galois Refinement of a Chosen Lift** — Start after Book 181 has chosen the primitive signed-special lift. Delimit the earlier split boundary precisely: Book 158 supplies the simultaneous point-centered Galois top, Book 157 supplies the good ordinary coefficient-prime replacement, and Book 176 must supply the remaining global $({\rm PGC}_{P_{\rm aux}})$ and avoidance-full-support clauses for the full auxiliary active set; its local pair, residual module congruence, common prime sets, and formal recovery are proved. Book 178 then gives an upper target packet. For the live set $P_{\rm tar}$ of retained multiplicative places above $3$ and post-specialization accidental semistable places, the preferred second input is Book 176's target broad-unipotent $({\rm TIAV}_{P_{\rm tar}})$ certificate, with the same two global clauses remaining. It supplies the finite minimal special fibre used by Book 181 and, after the point exists, the same support theorem proves that point automorphic; no target residual level lowering is required. The auxiliary and target certificates are distinct coefficient-characteristic instances of one reusable IAV schema. The exact-minimal route $({\rm DMS}_{P_{\rm tar}})$ through $({\rm FLO}_v)$ and the ordered Book 140 curve path remain stronger optional alternatives. None uses singleton moving geometry or toroidal joining. Transport the chosen point's automorphy to the controlled top, construct parity-complete compact carriers and the common auxiliary unramified Galois pairs for the elementary fixed-field array, and use the proved unitary bypasses for raw dyadic SP. The remaining raw-to-global input is the smooth-proper source of $({\rm HT}_1)$.
