...
139. **Ihara Theory and Saturated Degeneracy Maps on Shimura Curves** — Compute the two-map tree kernel and the integral degeneracy filtration for the clean compact Shimura curves. The noncongruence-character quotient is identified with the routed sum of transgression kernels on the $K_c^v$-invariant continuous characters of the full profinite congruence kernels. For fixed $(F,B,v)$ the underlying kernel is proved invariant under every open spectator level, but an explicit semidirect-product audit shows that this does not transport centrality upward. The compact finite-coefficient multiplier kernel is identified with its exact transgression quotient, including the extra stem condition needed, once centrality is known, to make the map from the whole character group injective. Book 6 proves the terminal scalar cokernel is the dual of the global roots of unity; the missing multiplier theorem must still construct the injection into that cokernel. Saturation and residual injectivity remain conditional on the routed localized odd-primary vanishing $({\rm AC}^{\rm loc}_\ell)_{\mathscr R}$. The stronger sufficient theorem $({\rm R2CM})_{F,B,v}$ is also unproved: its first missing assertion is reference-level rank-two centrality, and the perfect adelic congruence--metaplectic pairing, split/division local multiplier classification, and restricted-product/no-cross-factor reduction are absent. Conditional on that full package, the common kernel over a totally real field has order at most two and is therefore central with trivial routing action at every spectator level.
...
176. **Nonminimal Modularity Lifting** — Import the conditional one-prime Ihara, type-complex,
   ...
   modularity packages from Books 139, 174, and 175. Reuse Book 84's one-global-module
   several-prime degeneracy map, Book 152's regular-refinement source splitting, Book 14's
...
177. **Potential Modularity of Two-Dimensional Representations** — Construct the paired-frame potential-modularity bridge and prove its residual automorphy conclusion only for a lifting-admissible specialization. The automorphic target packet and stable-lattice comparison give a nonzero upper target localization on the selected compact carrier, but no target-prime level lowering or integral switch to a carrier split at its finite ramification place follows from that fact alone. Localized Ihara remains the first gap. For the all-point multi-place route, mixed exactness, iterated-switch product residue, full joint support, coherent simultaneous augmentation, and finite-set reciprocity remain unresolved. For the actual named Tate point, the smaller unresolved gate is construction of the final joint cube, simultaneous branch verification and reciprocity on its torsion-free top carrier, and occurrence of the one global product component through that point. Thus the advertised unconditional existence theorem is not ready.
