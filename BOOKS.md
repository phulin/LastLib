...
178. **Auxiliary Dihedral Data and Residual Potential Modularity** — Construct the two-prime Hilbert--Blumenthal specialization and dihedral residual packet under a complete admissibility and cleanability ledger. The compact carrier is obtained in the selected range, but post-specialization one-prime cleanability and its integral comparison hypotheses remain conditional.
...
181. **Finite Image and the Balanced Minimal-Lift Argument** — Turn the balanced hardly-ramified presentation into a normalized primitive special lift under the stated automorphic seed and restricted reduced-finiteness hypothesis. Neither the seed nor that finiteness hypothesis is constructed here, so the lift theorem is conditional.

182. **Potential Automorphy and Galois Refinement of a Chosen Lift** — Refine the chosen lift into a controlled potential-automorphy datum and isolate exactly what Brauer descent may consume. The required controlled residual automorphic seed and bridge-readiness ledger remain unproved; the volume does not make its downstream compatible family unconditional.
...
188. **Hardly Ramified $3$-adic Representations** — Analyze the selected three-adic Frey member using a named finite-flat lattice, its dyadic residual thickness, and the small-discriminant classification. The final reducibility contradiction is conditional on the coefficient-prime and residual-image certificates supplied by the preceding handoff; rational nonzero monodromy alone is insufficient.
