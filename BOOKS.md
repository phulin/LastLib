...
176. **Nonminimal Modularity Lifting** — Import the conditional one-prime Ihara, type-complex,
   ...
   special fiber, component-bijective patched specialization, and synchronized automorphic
   Taylor--Wiles systems with exact recovery. Proposition 1.1 proves component-support transfer
   formally once these arithmetic data, including avoidance full support, exist.
   ...
   and Smith normal form gives integral saturation. This removes curve abelian-Ihara from the
   preferred definite seed route but does not create lower-level support. The exact remaining
   arithmetic statement is $({\rm DLO}_{P_{\rm tar}})$, upper-to-minimal residual occurrence
...
181. **Finite Image and the Balanced Minimal-Lift Argument** — Turn the balanced hardly-ramified presentation into a normalized primitive special lift. The finite-image and complete-local-algebra argument consumes only the restricted-special-fibre certificate $({\rm SF}_{\rm SP})$: finiteness of the restricted scalar special fiber plus the exact local quotient. From it, finite restricted image, finite-index passage, Carayol trace descent, and topological Nakayama give finite scalar image, whole-ring finite flatness, and a horizontal point. The full exact automorphic seed remains the natural stated hypothesis because Book 182 additionally evaluates the restricted $R=\mathbb T$ theorem to make that chosen point automorphic over the same field. On the preferred split route, after Book 176's separate $({\rm IAV}_{P_{\rm aux}})$ handoff and Book 178's upper packet, Book 176 proves definite Ihara and saturation, so the first missing source is $({\rm DLO}_{P_{\rm tar}})$: upper-to-minimal residual occurrence for the actual finite set. Book 176 reduces DLO to the one-place fixed-away-level outside-$\ell$ theorem $({\rm FLO}_v)$; its regular part is Mazur principle, while a repeated-root accidental place requires scalar support stronger than the general spherical-or-special level-optimization conclusion. Together with the clean Book 173 structural ledger DLO is $({\rm DMS}_{P_{\rm tar}})$. The chosen point is never used backward to prove either certificate.

182. **Potential Automorphy and Galois Refinement of a Chosen Lift** — Start after Book 181 has chosen the primitive signed-special lift. Delimit the earlier split seed precisely: Book 158 supplies the simultaneous point-centered Galois top, Book 157 supplies the good ordinary coefficient-prime replacement, and Book 176 must supply $({\rm IAV}_{P_{\rm aux}})$ for the full auxiliary active set. Book 178 then gives an upper target packet. Because the split top has even degree, ordinary Jacquet--Langlands and a fresh primitive class-set lattice give a nonzero upper localization directly on the totally definite algebra. Book 176 proves scalar definite Ihara and saturation, but not the finite-set support change $({\rm DLO}_{P_{\rm tar}})$. The live $P_{\rm tar}$ contains retained multiplicative places above $3$ and post-specialization accidental semistable places, not the retired $W_0(M)$ set. Book 176 reduces DLO to $({\rm FLO}_v)$ at those actual places; no simultaneous lowering theorem is then needed, but the integral Mazur-principle proof and the stronger repeated-root scalar-support case are still absent. The preferred remaining seed theorem is $({\rm DMS}_{P_{\rm tar}})=({\rm DLO}_{P_{\rm tar}})+({\rm CLN}_{\min})$; the ordered Book 140 curve path is a stronger secondary alternative. Neither route uses singleton moving geometry or toroidal joining. Once the full minimal datum exists, evaluate Book 173 at the chosen point and transport automorphy to the controlled top. Construct parity-complete compact carriers and the common auxiliary unramified Galois pairs for the elementary fixed-field array. The raw signed-special and compatible-family inputs remain separate.

183. **Brauer Induction for Automorphy Families** — Construct the elementary fixed-field packet array, overlap identities, integral Brauer classes, common polynomials, and virtual Frey-special local record from a supplied controlled top datum. Ordinary elementary packet attachment and the common unramified algebraic Galois pair away from two are proved without principal/dihedral type parameters; finite twisting and Weil induction then form the auxiliary local Mackey objects. The sole remaining raw-carrier input is $(\mathrm{RSP}_2)$ at the dyadic signed-special place: Book 122, Section 10.5 plus either even-degree away-from-two eligibility or a mixed-dyadic geometric construction. Under $(\mathrm{HT}_1)$, cuspidality then makes every attached semisimple packet realization absolutely irreducible, so the raw representation equals it and the dyadic monodromy operator, line, and sign survive at every odd coefficient place. Ambient Tate semisimplicity is only a stronger optional alternative. Effectivity is deliberately deferred, and neither $(\mathrm{RSP}_2)$ nor the controlled seed is supplied.
