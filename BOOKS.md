...
182. **Potential Automorphy and Galois Refinement of a Chosen Lift** — Start after Book 181 has chosen the primitive signed-special lift. Delimit the earlier split seed precisely: Book 158 supplies the simultaneous point-centered Galois top; Book 157 supplies the good ordinary coefficient-prime replacement when the multiplicative residual model cannot itself be a finite-flat characteristic-zero point; and Book 176 must supply a finite-set certificate for the full auxiliary active set. This route uses no singleton moving presentation or toroidal joining theorem. Once the exact minimal datum is available, evaluate Book 173 at the chosen point and transport automorphy to the controlled top. Keep the auxiliary active set separate from the later target-extra set; a primitive upper occurrence plus a path of actual Book 140 edge certificates propagates to the minimal signed-special localization, while the bottom structural ledger remains an input. Construct parity-complete compact carriers and all-coefficient attachment for every elementary fixed-field candidate, and obtain the common unramified algebraic Galois pairs used downstream. The stronger raw signed-special carrier and compatible-family inputs remain separately stated; the volume does not make them unconditional.
