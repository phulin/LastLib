...
139. **Ihara Theory and Saturated Degeneracy Maps on Shimura Curves** — Compute the two-map tree kernel and the integral degeneracy filtration for the clean compact Shimura curves. The noncongruence-character quotient is identified with the routed sum of transgression kernels on the $K_c^v$-invariant continuous characters of the full profinite congruence kernels. For fixed $(F,B,v)$ the full congruence kernel is proved invariant under every open spectator level, and the compact finite-coefficient multiplier kernel is identified with its exact transgression quotient, including the extra stem condition needed, once centrality is known, to make the map from the whole character group injective. Saturation and residual injectivity remain conditional on the named localized odd-primary vanishing: the isolated rank-two centrality, perfect adelic congruence--metaplectic pairing, split/division local multiplier classification, and adelic reduction to the scalar Hilbert-symbol problem are not supplied by strong approximation or the existing reciprocity books.
...
157. **Local Geometry of Hilbert--Blumenthal Moduli** — Construct real, good, ordinary,
   ...
   necessary finite-residue rational-point condition.
...
176. **Nonminimal Modularity Lifting** — Import the conditional one-prime Ihara, type-complex,
   ...
   modularity packages from Books 139, 174, and 175. Construct the strict joint scalar ray
   quotient by cross-local prescriptions, so its product group ring and fixed-prime
   augmentation do not come from potentially diagonal one-place images. Isolate the mixed
   obstruction at two places and prove the finite-set pointwise conclusion
...
178. **Auxiliary Dihedral Data and Residual Potential Modularity** — Construct the two-prime Hilbert--Blumenthal specialization and dihedral residual packet from a supplied compatible corrected relative ray datum and a complete admissibility ledger. Prove relative-ray compatibility over the rational base and use an anti-cyclotomic Grunwald--Wang twist to impose finitely many exact protected residual branch characters, including unramified Frobenius values, without changing the determinant. Its pre-specialization ledger retains the degree-indexed allowed-good Abel condition needed for the unique special factor. Book 157 proves the Kummer normalization, equivariant fan, complete-trait inverse, Mumford-side deformation calculation, and slicing reductions, but its $(\mathrm{IIT})$, $(\mathrm{MPE})$, $(\mathrm{ACE})$, and fan-dependent toroidal joining remain Required; hence the integral-open repair is conditional. The compact carrier is obtained in the selected range, and the cross-prime stable-lattice comparison supplies a nonzero upper target localization on the carrier ramified at the distinguished special place; it supplies no integral carrier switch there. Relative-ray compatibility remains conditional over a general totally real base; without the toroidal and separately stated repeatability, flexibility, and realized-normal-sheet hypotheses the singleton moving input remains part of admissibility, and post-specialization auxiliary cleanability, actual target-lowering edges, and their integral comparison hypotheses remain conditional.
