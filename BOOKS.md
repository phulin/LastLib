...
138. **Integral Local Types and Type Lattices** — Construct integral primary and companion lattices for principal, special, depth-zero dihedral, and tame positive-depth local types, together with their represented enhanced deformation conditions. Realize the Steinberg flag rows on the generic parahoric tower and identify their correct bad-fiber extension as a strict procyclic constructible complex, conditional at Book 122's remaining actual spectator/ray Kummer datum and completed Kummer--tube theorem $({\rm BTK})$ inside $({\rm HDB}_v)$; Book 122 proves the formerly separate $({\rm RGC}_v)$. Banality, ordering, sign, and saturation hypotheses are retained, and primitive wild or ramified dyadic positive-depth types are excluded.

139. **Ihara Theory and Saturated Degeneracy Maps on Shimura Curves** — Compute the two-map tree kernel and the integral degeneracy filtration for the clean compact Shimura curves. The noncongruence-character quotient is identified with the routed sum of transgression kernels on the $K_c^v$-invariant continuous characters of the full profinite congruence kernels. Saturation and residual injectivity remain conditional on its named localized odd-primary vanishing at every actual routed spectator level; strong approximation, density, finiteness, centrality, and the geometry developed here do not by themselves prove that vanishing.

140. **Integral Level Change and Jacquet--Langlands Comparison** — Compare one-prime integral level change with selected Jacquet--Langlands using the corrected Ihara filtration, graph lattice, and faithful Hecke orders. The theorem remains conditional on abelian Ihara and datum-specific node uniformization. Relative to the separate abelian Ihara input, generic support in the clean constant-coefficient block is proved from Book 125's component-routed semisimple decomposition and the corrected filtration. The type ledger records that an integral flag projector handles the $q_v+1$-unit quotient/augmentation range and that type transport is formal from a flagged node-groupoid equivalence; it proves neither the nonbanal quotient-new/filtered-cofiber assertions nor a several-prime comparison.
...
157. **Local Geometry of Hilbert--Blumenthal Moduli** — Construct real, good, ordinary,
   ...
   Section 13.2B normalizes the cusp charts in the two frame-division lattices and constructs a
   simultaneous unit- and twist-invariant regular projective fan.  Required Theorem 13.2B.3a
   isolates the first missing input: the relative inverse-to-Mumford PEL equivalence, full
   faithfulness, and completed deformation comparison.  The separate $(\mathrm{ACE})$ ledger
   retains finite-type cusp effectivity, exact overlaps and trait coverage, and compatible theta
   positivity.  Conditional on those unframed inputs, Proposition 13.2B.3b proves that finite
   normalization gives the framed completed charts and smooth proper model and that a
   symmetrized ample pullback gives effective twist descent.  Their combined output is the still
   conditional fan-dependent toroidal theorem, Required Theorem 13.2B.3.
   Consequently the framed joining theorem, stable integral connecting slice, and singleton
   mixed moving repair remain conditional; the literal marked-good-section form additionally
   requires its finite-residue rational-point condition.
...
168. **Compatible Coefficient Systems and Purity** — Import Book 127's curve-carried weak family,
   ...
   classes are the exact arithmetic input. Book 41 proves the finite polarized descent and
   Zarhin cancellation steps, but its standard uniform route still retains the potential-semistability,
   ramified Hodge-base-change, toroidal-compactification, and metrized theta--Hodge interfaces and the
   logarithmic-Hilbert and theta upper-slope certificates. Raw
...
174. **One-Prime Type Complexes and Component Support** — Build the enhanced one-prime type complexes. The actual common normalized principal/ray diagram, its isomorphic lifted top legs, the geometrically constant ray twist, and the finite-wild/procyclic strict-node complex, with Frobenius, expansion-one pull and trace, orientation, and normalization filtration, are proved. Book 122 proves $({\rm DLB}_1)$, all active finite-depth normalization rings, the raw higher branch/inertia/residue/node-annular and boundary-stabilizer ledger, the invariant-field/Cartier/group calculation, completed endpoint factorization from the actual Kummer datum, the exact Milnor-tube formalism, and $({\rm RGC}_v)$. The remaining $({\rm HDB}_v)$ input is the actual spectator/ray Kummer datum and completed Kummer--tube theorem $({\rm BTK})$ required for $({\rm PNS}_v)$ and hence for $({\rm TPE}_v)$. Conditional on that input, unit-order coarse descent, and the named abelian-Ihara family, the integral projector for the actual quotient/augmentation flag pair when $q_v+1$ is a unit proves type-Ihara and primitive residue from the Shapiro constant rows; even when $q_v\equiv-1\pmod\ell$, the augmentation-companion pull is automatic from the abelian-Ihara rows. The two remaining nonbanal algebraic inputs are the quotient-new injection (5.0d) and primitive filtered-cofiber comparison (6.0d). Type-compatible node transport is formal from a superspecial classification stated as an equivalence of flagged PEL node groupoids, but that geometric classification and the nonminimal boundary chart-and-dimension theorem remain hypotheses. The typed filtration and Book 125 prove the generic-residue kernel equality and hence equality of the corresponding integral faithful image orders, so generic support is not another hypothesis. Clean auxiliary augmentation is exact on represented rings, primary and companion complexes, modules, and pairings; its acting-order map remains a surjection until base faithfulness is known.

175. **One-Prime Nonminimal Patching and R=T** — In the scalar-residual line-special range, derive the finite-level one-prime reduced comparison and pointwise modularity from the exact type-complex package. The balanced enhanced obstruction presentation and finiteness force the conductor-one ring to be finite flat over the coefficient DVR. Full $R=T$ is equivalent to characteristic-zero exact-condition adjoint Selmer vanishing at every point. The line is retained in the enhanced tangent calculation; at a monodromy-zero characteristic-zero point its motion is uniquely determined by the two distinct Frobenius characters. Under a coherent clean unpadded auxiliary system, prove that vanishing and full patched and finite $R=T$ by equal-variable rigidity: the enhanced absolute deformation source and diamond source have the same $q$ variables, and Book 174's nonzero primary module free over the diamond source makes the regular source act faithfully. The strict ray twist transports the equality to the scalar family and fixed-character fibers, and auxiliary acting-order augmentation is then automatically exact. Without the coherent auxiliary system, the isolated branchwise torsion-cotangent theorem is the exact alternative input. No generic- or vexing-row theorem and no simultaneous finite-set result is asserted.
...
176. **Nonminimal Modularity Lifting** — Import the conditional one-prime Ihara, type-complex,
   ...
   modularity packages from Books 139, 174, and 175. Isolate the mixed obstruction at two
   places and prove the finite-set pointwise conclusion
...
177. **Potential Modularity of Two-Dimensional Representations** — Construct the paired-frame potential-modularity bridge and prove its residual automorphy conclusion only for a lifting-admissible specialization. The automorphic target packet and stable-lattice comparison give a nonzero upper target localization on the selected compact carrier, but no target-prime level lowering or integral switch to a carrier split at its finite ramification place follows from that fact alone. Localized Ihara, the actual multi-place mixed exactness, product-residue comparison, joint support, coherent simultaneous ring--complex--module--action--pairing augmentation, and finite-set reciprocity are unresolved, so the advertised unconditional existence theorem is not ready.

178. **Auxiliary Dihedral Data and Residual Potential Modularity** — Construct the two-prime Hilbert--Blumenthal specialization and dihedral residual packet from a supplied compatible corrected relative ray datum and a complete admissibility ledger. Prove relative-ray compatibility over the rational base and use an anti-cyclotomic Grunwald--Wang twist to impose finitely many exact protected residual branch characters, including unramified Frobenius values, without changing the determinant. Its pre-specialization ledger retains the degree-indexed allowed-good Abel condition needed for the unique special factor; Book 157 reduces the integral-open route to its still-unproved fan-dependent toroidal joining theorem. The compact carrier is obtained in the selected range, and the cross-prime stable-lattice comparison supplies a nonzero upper target localization on the carrier ramified at the distinguished special place; it supplies no integral carrier switch there. Relative-ray compatibility remains conditional over a general totally real base; the singleton moving input, post-specialization auxiliary cleanability, actual target-lowering edges, and their integral comparison hypotheses remain conditional.

179. **Compatible Systems of Galois Representations** — Define and analyze compatible systems, coefficient fields, residual members, purity, and recognition from Frobenius data in the precise rank-two range needed later. The volume packages the already constructed curve systems and image criteria; its abelian-variety examples use Book 41's conditional Tate theorem. The exact input is finite isogeny classes; Book 41 proves finite polarized descent and integral factor cancellation, while its displayed uniform source retains the potential-semistability, ramified Hodge-base-change, toroidal-compactification, and metrized theta--Hodge interfaces and the logarithmic-Hilbert and theta upper-slope certificates. It does not create a descended base-field family or an integral lattice at a new coefficient prime.
...
181. **Finite Image and the Balanced Minimal-Lift Argument** — Turn the balanced hardly-ramified presentation into a normalized primitive special lift under the stated exact automorphic seed. The seed first lets Book 173 make the restricted minimal ring finite; finite restricted image, finite-index passage, Carayol trace descent, and topological Nakayama then give finite scalar image and whole-ring finite flatness before the horizontal point is chosen. The point is not used backward to prove its $R=\mathbb T$ source. The seed is the sole new arithmetic hypothesis and is not constructed here, so the lift theorem is conditional.

182. **Potential Automorphy and Galois Refinement of a Chosen Lift** — Apply the rational-base relative-ray compatibility and protected anti-cyclotomic Grunwald correction of Book 178 to construct exact auxiliary Frobenius branches and paired-frame seeds over $\mathbf Q_2$, $\mathbf Q_3$, and $\mathbf Q_\ell$. In the multiplicative-$\ell$ branch, the resulting Tate point realizes only the finite-flat residual $\ell$-torsion, not the full characteristic-zero finite-flat condition, and the selected model forces any good replacement to be ordinary rather than local--local. Book 157 now constructs the required one-prime seed $(\mathrm{Ord}^{\mathrm{base}}_\ell)$ over $\mathbf Q_\ell$. Since the frozen multiplicative auxiliary row may be incompatible with good reduction, Proposition 6.2C then reselects $q$, retargets its branch after coefficient enlargement, and constructs corrected $(\mathrm{FF}^{\mathrm{base}}_\ell)$ with both frames and the full local ledger. Required Moving Theorem 6.3 remains conditional: Book 157 proves its Kummer-normalization, equivariant-fan, finite-field Bertini, and DVR-slicing reductions but not the required toroidal cusp gluing/algebraization. Once a moving presentation is supplied, the normal-sheet and split-certificate reductions force Galois-closure constant-field avoidance. The Frey-adapted auxiliary-$q$ residual branch is unramified at every dyadic place while its Tate seed has nonzero monodromy, so Book 178's literal one-active-place conclusion is unavailable: an actual ordered route-appropriate chain or genuine Book 176 finite-set datum is required, separately from the later target-lowering set. Prove the first post-specialization nonvanishing theorem: Book 104 places the split target packet on a path-compatible compact carrier ramified at a retained dyadic SP place and therefore split at every target-extra place; a primitive upper packet lattice plus an ordered path of actual Book 140 edge certificates forces every lower localization, including the bottom minimal SP localization, to be nonzero. Book 104 then transfers the final path-carrier branch forward to the split group and inversely to the totally definite carrier; a second primitive packet lattice makes the final definite occurrence formal, without either integral Jacquet--Langlands comparison. The remaining seed hypothesis includes the moving presentation, auxiliary-prime lifting data, certified target edges, and the bottom Book 173 structural ledger; the path-compatible carrier is formal after the target packet and its actual extra set are known. Extend the two-place trace argument to give parity-complete basic carriers and all-coefficient attachment for every elementary candidate, including odd-degree fixed fields. Use equivariant conductor and invariant-Frobenius independence, the distinguished irreducible raw member, and Book 128, Lemma 3.2 to construct a common unramified algebraic Galois pair for every elementary packet away from two; the split unitary good cover treats the old coefficient prime. The later non-common-norm PEL-exact parahoric comparison for raw SP geometry and packet-carrier ambient semisimplicity remain inputs. The stronger automorphic principal/dihedral type theorem remains unproved and first needs an actual tameness ledger: cyclic principal quotient characters can be wild. The volume does not make its downstream compatible family unconditional.

183. **Brauer Induction for Automorphy Families** — Construct the elementary fixed-field packet array, overlap identities, integral Brauer classes, common polynomials, and virtual Frey-special local record from a supplied controlled top datum. Retain the common unramified algebraic Galois pair for every elementary packet away from two and show that finite twisting and Weil induction form the auxiliary local Mackey objects. Prove that ambient semisimplicity of the actual curve carriers is a noncircular uniform raw-to-global preservation certificate for the whole finite array. Book 41 reduces that certificate completely to finite isogeny classes and proves the descent and cancellation parts of the uniform source; the potential-semistability, ramified Hodge-base-change, toroidal-compactification, and metrized theta--Hodge interfaces and the logarithmic-Hilbert and theta upper-slope certificates remain. Effectivity is deliberately deferred, and neither those interfaces nor the missing controlled seed is supplied.
...
185. **Compatible Systems over the Base Field** — Assemble the effective constituents into a coefficient-descended compatible family over the base field, carrying the rational signed special record at two. All-place assembly retains Book 184's characteristic-zero absolute-irreducibility hypothesis at coefficient places above two; without it, the construction still covers every coefficient place away from two, including those above three. Apply algebraic twisting and Weil induction to the elementary unramified Galois pairs, then use the actual positive--negative direct-sum identity and Krull--Schmidt cancellation to prove $(\mathrm{AUX}_{\mathrm{all}})$ and $(\mathrm{AUX}_\nu)$. Hence $(\mathrm{Cond}_2)$ and $(\mathrm U)$ hold: every member is unramified outside two and its coefficient characteristic, and the selected three-adic member is unramified outside $\{2,3\}$. The local conductor formula gives an independent check. Book 182's stronger automorphic principal/dihedral descent complexes, type/exchange lines, tameness, and return maps remain unproved but are not needed for the Galois AUX comparison or clean support.

186. **Changing the Coefficient Prime while Keeping the Frey Special Place** — Select the coefficient-three member of the away-from-two compatible collection, retain Book 185's clean support outside $\{2,3\}$, and transport the full rational special Weil--Deligne record at two. This route is independent of the coefficient-two hypothesis $(\mathrm{AI}_2)$. Hand the named member to Book 187's integral construction and Book 161's actual category/classification input as applied by Book 188; do not infer crystallinity, finite flatness, or residual monodromy primitivity.

187. **The Fixed-Three Integral Local Theory** — Starting from Book 186's cleanly supported selected member, prove that a flat companion $(\mathrm{IC}_3)$ yields crystallinity of Hodge type $\{0,1\}$. Construct the CM-unitary common-multiplier PEL group, its proper good model, and its finite full-component cover of every positive compact division-quaternion carrier after the harmless central saturation of its trivial-character packet level, over a field unramified at $3\ell$. Construct the positive elementary good-abelian carriers and $(\mathrm{IC}_3)$ by product Jacobian, Weil restriction, oldvector embedding, saturation, and finite-flat closure without integral packet idempotents or integral Brauer cancellation. Import Book 182's split evaluated $\mathbf Q_3$ packet and parity-complete basic carriers, and prove that the formerly proposed direct common-norm PEL cover is impossible rather than silently using it. Construct the named all-level Schoof tower using Book 185's proved Galois AUX comparison and clean support. The stronger automorphic-type parameters remain optional and unproved, not an input to the tower. The result does not require $(\mathrm{AI}_2)$.
...
188. **Hardly Ramified $3$-adic Representations** — Verify that Book 187's signed-special tower
   ...
   certificates are constructed, while the controlled rational top remains conditional.
