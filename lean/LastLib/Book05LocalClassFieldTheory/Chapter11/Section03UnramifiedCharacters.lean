...
/- SOURCE_ISSUE: the source says, "any allowed `α` lies in a compact
  procyclic subgroup".  The subgroup generated by an infinite-order element
  need not itself be compact; the mathematically correct object is its compact
  closure. -/
