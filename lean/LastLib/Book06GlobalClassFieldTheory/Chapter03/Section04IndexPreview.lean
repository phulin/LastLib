...
bijectivity proof belongs to the global reciprocity/class-formation argument; the quotient API
is made explicit here so later chapters can use the canonical map rather than restating it.
-/
...
noncomputable def chapter03ArtinQuotientMap
  ...
  QuotientGroup.lift (chapter03NormClassSubgroup N) (chapter03ClassArtin A) (by sorry)
...
noncomputable def chapter03ClassNormQuotientEquivGalois
    {K L : Type*} [Field K] [Field L] [NumberField K] [NumberField L]
    [Algebra K L] [FiniteDimensional K L] [IsGalois K L]
    [IsMulCommutative (Gal(L / K))]
    {S_K : Chapter03FieldIdeleData K} {S_L : Chapter03FieldIdeleData L}
    ...
    (A : Chapter03ArtinReciprocityData N) :
    Chapter03ClassNormQuotient N ≃* Gal(L / K) :=
  MulEquiv.ofBijective (chapter03ArtinQuotientMap A) (by sorry)

theorem chapter03_abelian_classNorm_index_preview
    {K L : Type*} [Field K] [Field L] [NumberField K] [NumberField L]
    [Algebra K L] [FiniteDimensional K L] [IsGalois K L]
    [IsMulCommutative (Gal(L / K))]
    {S_K : Chapter03FieldIdeleData K} {S_L : Chapter03FieldIdeleData L}
    (N : Chapter03NormData S_K S_L) :
    (chapter03NormClassSubgroup N).index = Module.finrank K L := by
  sorry

theorem chapter03_abelian_classNorm_quotient_identified_with_galois
    {K L : Type*} [Field K] [Field L] [NumberField K] [NumberField L]
    [Algebra K L] [FiniteDimensional K L] [IsGalois K L]
    [IsMulCommutative (Gal(L / K))]
    {S_K : Chapter03FieldIdeleData K} {S_L : Chapter03FieldIdeleData L}
    {N : Chapter03NormData S_K S_L}
    (A : Chapter03ArtinReciprocityData N) :
    Nonempty (Chapter03ClassNormQuotient N ≃* Gal(L / K)) :=
...
theorem chapter03_nonabelian_norm_limitation
    {K L : Type*} [Field K] [Field L] [NumberField K] [NumberField L]
    [Algebra K L] [FiniteDimensional K L] [IsGalois K L]
    ...
    chapter03NormClassSubgroup T.norm_M_over_K =
      chapter03NormClassSubgroup T.norm_L_over_K := by
  sorry

theorem chapter03_nonabelian_classNorm_index_preview
    {K L : Type*} [Field K] [Field L] [NumberField K] [NumberField L]
    [Algebra K L] [FiniteDimensional K L] [IsGalois K L]
    {S_K : Chapter03FieldIdeleData K}
    {S_L : Chapter03FieldIdeleData L}
    {S_M : Chapter03FieldIdeleData (Chapter03MaximalAbelianSubfield K L)}
    (T : Chapter03NormTowerData S_K S_M S_L) :
    (chapter03NormClassSubgroup T.norm_M_over_K).index =
      Module.finrank K (Chapter03MaximalAbelianSubfield K L) := by
  sorry
