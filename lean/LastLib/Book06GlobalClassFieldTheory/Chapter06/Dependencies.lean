...
/- The topological idele class group interface used by the chapter. -/
class Chapter06IdeleClassGroup (K : Type*) [Field K] where
  carrier : Type*
...
structure Chapter06IdeleClassCharacter
    (K : Type*) [Field K] [Chapter06IdeleClassGroup K] where
  hom : Chapter06C K →* ℂˣ
  continuous' : Continuous hom
  finite_order : ∃ n : ℕ, 0 < n ∧ ∀ c, hom c ^ n = 1
...
/- The degree used in the chapter's index formula. -/
