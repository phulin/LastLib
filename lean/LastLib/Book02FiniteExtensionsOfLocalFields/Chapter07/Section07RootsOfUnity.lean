...
theorem chapter07_padic_prime_to_p_roots_of_unity
  ...
  /-
  Proof roadmap.  The statement is mathematically sufficiently specified, but
  no single earlier interface constructs all the fields in the target.  The
  proof should add the two focused construction helpers described below before
  assembling this structure.

  1. Write

       horder = ⟨hp, hm, hd, hpm, hdiv, hmin⟩.

     Install `NeZero d` from `hd` and put

       F := FiniteField.Extension (ZMod p) p d.

     (`Mathlib/FieldTheory/Finite/Extension.lean`.)  The declarations
     `FiniteField.finrank_extension` and `FiniteField.natCard_extension`,
     together with `ZMod.card`, identify its degree and cardinality with `d`
     and `p ^ d`.  Obtain `g : Fˣ` from `IsCyclic.exists_generator`.  The
     generator-order identity
     `orderOf_eq_card_of_forall_mem_zpowers` and `Nat.card_units` give
     `orderOf g = p ^ d - 1`.  If `a := (p ^ d - 1) / m`, then `hdiv` and
     `hm.ne'` allow `orderOf_pow_orderOf_div` to show

       orderOf (g ^ a) = m.

     Therefore `IsPrimitiveRoot.iff_orderOf` gives a primitive residue root
     `zetaBar : F`.  To avoid proving that all of `F` is generated by this
     element, set

       l := IntermediateField.adjoin (ZMod p) ({zetaBar} : Set F).

     Use the subtype root in `l`; generation is definitional (convert with
     `IntermediateField.adjoin_eq_top_of_algebra`) and
     `chapter07_finite_field_primitive_root_degree` in this file, instantiated
     with `q := p`, proves `Module.finrank (ZMod p) l = d`.  Package this as a
     local helper returning `l : Type uC`, its field/algebra/finite-dimensional
     instances, the primitive root, generation, and this finrank equality.
     Use `ULift` and `ULift.algEquiv` when `uC` is not `0`; transport
     `IsPrimitiveRoot`, `Algebra.adjoin`, and finrank across that equivalence.

  2. Add a finite-etale lift helper whose input is precisely the package from
     step 1 and whose output keeps one coherent field, valuation branch, and
     residue presentation.  Its useful (non-cyclotomic) conclusion should be

       ∃ (L : Type uC) (_ : Field L) (_ : Algebra ℚ_[p] L)
         (_ : FiniteDimensional ℚ_[p] L),
         ∃ (B : Type uC) (_ : CommRing B) (_ : HenselianLocalRing B)
           (resB : B →+* l) (vL : AddValuation L (WithTop ℤ))
           (hext : (Padic.addValuation (p := p)).IsEquiv
             (vL.comap (algebraMap ℚ_[p] L))),
           -- `B` is the valuation ring/integral closure inside `L`,
           -- `resB` is onto with maximal-ideal kernel, and all canonical
           -- Chapter10 residue maps and identifications commute.

     Construct it as follows.  Let `gbar := minpoly (ZMod p) zetaBar`.
     Its monicity/irreducibility are `minpoly.monic` and
     `minpoly.irreducible`; separability follows from
     `PerfectField.ofFinite`.  Lift it to a monic `P : ℤ_[p][X]` using
     `Polynomial.map_surjective`, `ZMod.ringHom_surjective PadicInt.toZMod`,
     and `Polynomial.lifts_and_natDegree_eq_and_monic` from
     `Mathlib/Algebra/Polynomial/Lifts.lean`.  Take

       L := Chapter07LiftedPolynomialQuotient ℚ_[p]
         (chapter07LiftedPolynomial P),
       alpha := chapter07LiftedRoot P.

     `chapter07_monic_lift_irreducible`,
     `chapter07_lifted_polynomial_quotient_is_field`, and
     `chapter07_lifted_polynomial_quotient_degree` from
     `Section03ConstructingTheUnramifiedLift.lean` supply the field,
     generation, and degree.  Build `B` as the integral closure of `ℤ_[p]`
     in this `L`, with its quotient map to `l`.  Prove it is the local DVR
     above the p-adic maximal ideal and that the quotient map is surjective
     with maximal-ideal kernel.  Then
     `chapter07_construct_unramified_lift_with_residue_maps` (same file)
     supplies the unramified local profile and its residue compatibility.
     The helper must additionally retain `HenselianLocalRing B`; this follows
     from the finite local algebra over the henselian ring `ℤ_[p]`, and is
     required by the next step.

  3. Apply
     `chapter07_prime_to_residue_characteristic_root_lifts_uniquely` in this
     file to `resB` and `zetaBar`.  The unit hypothesis is obtained from
     `hpm` and `PadicInt.ker_toZMod` (equivalently, show the image of `m` in
     `ZMod p` is nonzero, then use `IsLocalRing.isUnit_iff` in `B`).  This
     gives `theta : B` with `theta ^ m = 1` and
     reduction `zetaBar`.  Regard it in `L`.  To prove its exact order, use
     `IsPrimitiveRoot.mk`: the lifted equation supplies the `m`th-power
     clause, and if `theta ^ n = 1`, applying `resB` makes
     `zetaBar ^ n = 1`, so the primitive residue root implies `m ∣ n`.
     (The theorem `chapter07_reduction_preserves_primitive_root_order` proves
     the opposite implication direction and cannot establish primitivity of
     the lift.)  This fills `root_primitive`.  Generation is
     proved by reducing the intermediate algebra generated by `theta`: the
     residue root generates `l`, so the finite-etale Nakayama lemma
     `IsLocalRing.adjoin_residue_eq_top_iff_adjoin_eq_top` from
     `Mathlib/RingTheory/LocalRing/Etale.lean` gives generation of `B`, and
     passing to fraction fields gives
     `Algebra.adjoin ℚ_[p] ({theta} : Set L) = ⊤`.  Consequently
     `chapter07_finite_field_primitive_root_degree` and the quotient-degree
     theorem both identify `Module.finrank ℚ_[p] L` with `d`.

  4. For the valuation part, first use
     `chapter10_valuation_extension_exists` and
     `chapter10_contracts_to_is_equiv` from
     Book01/Chapter10/Section02ExistenceByMaximalDomination.lean.  Normalize
     the value group of this finite extension of the complete discretely
     valued field `ℚ_[p]` to `WithTop ℤ`; the uniqueness theorem
     `chapter10_henselian_unique_extension` in
     Book01/Chapter10/Section06FiniteExtensionsOfCompleteFields.lean proves
     that the normalized valuation has the same valuation ring `B`.  This is
     the missing normalization component of the helper in step 2; the raw
     extension theorem alone has a heterogeneous value group and cannot fill
     the target's `valuation` field.

     Apply `chapter10_unramified_lift_profile` from
     Book01/Chapter10/Section07ConcreteFiniteExtensions.lean (or the p-adic
     wrapper `chapter07_padic_irreducible_residue_lift` in
     Section05ExamplesBothCharacteristics.lean) to `P`, `alpha`, and this
     normalized valuation.  Use `PadicInt.residueField` and
     `PadicInt.toZMod_eq_residueField_comp_residue` for its base-residue
     hypothesis.  The result supplies `extensionData`, `profile`, realization,
     degree, ramification index one, residue degree `d`, and
     `profile_unramified`.

  5. Take `residueField := l`.  The extension-residue equivalence is induced
     by the quotient map `resB`; the base equivalence is the valuation-ring
     quotient equivalence followed by `PadicInt.residueField`.  Set
     `residueMap := Chapter10ResidueFieldMap _ _` after installing
     `Valuation.HasExtension := ⟨hext⟩`; then
     `residueMap_is_canonical` is `rfl`.  The quotient-map commutative square
     from the helper proves `residueMap_compatible`.  Fill
     `residueDegree` from step 1 and `residue_separable` from
     `PerfectField.ofFinite`, and assemble
     `Chapter07PadicCyclotomicExtension` with `profile_realized` supplied by
     `Chapter10ProfileRealizedByData`.

  Known dead ends.  `chapter07_finite_residue_unramified_exists_for_degree`
  in Section04 returns an abstract model, but it neither relates a chosen
  residue primitive root to the lifted field nor stores a normalized valuation
  or the canonical Chapter10 residue maps.  Also,
  `chapter07_construct_unramified_lift_with_residue_maps` only verifies an
  already supplied integral/residue presentation, and
  `chapter10_unramified_lift_profile` consumes an already supplied valuation.
  Merely chaining those declarations leaves exactly the `root`, `valuation`,
  and residue-identification fields unresolved; the two helpers above must
  construct and retain those witnesses rather than choosing unrelated models.
  -/
  sorry
