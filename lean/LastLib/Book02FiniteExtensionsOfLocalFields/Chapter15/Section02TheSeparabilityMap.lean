...
-/
structure Chapter15UnramifiedTotallyRamifiedFactorization
...
/-- Book 2, §15.2: finite complete degree formulas do not require separability
once a perfect residue field supplies defectlessness. -/
-- SOURCE_ISSUE: §15.2 repeats the false unconditional equality `[L : K] = e f`; the corrected interface assumes a perfect base residue field and completeness, which imply defectlessness without adding separability.
...
/-- Book 2, §15.2: in the separable case trace and norm have embedding formulas. -/
...
/-- Book 2, §15.2: trace nondegeneracy is a separable-extension statement. -/
...
/-- Book 2, §15.2: Galois means both normal and separable, and then the group has degree order. -/
...
/-- Book 2, §15.2: the foundational unramified condition includes residue separability. -/
...
/-- Book 2, §15.2: total ramification is the endpoint `f = 1`. -/
...
/-- Book 2, §15.2: a separable residue extension admits the two-stage factorization interface. -/
...
theorem separable_residue_extension_has_unramified_then_totally_ramified_factorization
    ...
    [Algebra.IsSeparable (IsLocalRing.ResidueField vK.valuationSubring)
      (IsLocalRing.ResidueField vL.valuationSubring)] :
    Nonempty (Chapter15UnramifiedTotallyRamifiedFactorization vK vL) := by
  sorry

/-- Book 2, §15.2: finiteness of the residue field implies perfection. -/
theorem finite_residue_field_is_perfect
    (k : Type*) [Field k] [Fintype k] : PerfectField k := by
  infer_instance

/-- The finite-residue-field consequences listed in Book 2, §15.2. -/
...
/- A finite-dimensional extension of a finite residue field is finite; this
bridge supplies the `Fintype l` instance used by the finite-field APIs. -/
theorem finite_residue_extension_of_finite_base_is_finite
    (k l : Type*) [Field k] [Field l] [Algebra k l]
...
/-- Book 2, §15.2: perfection supplies separability of finite algebraic residue extensions. -/
