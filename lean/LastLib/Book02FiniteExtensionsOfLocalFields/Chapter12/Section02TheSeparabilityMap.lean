...
normed overfield.  The empty set represents an element with no distinct
conjugate, so its infimum is `⊤` in `ENNReal`. -/
def chapter12KrasnerDistanceSet
    {K L : Type*} [Field K] [NormedField L] [Algebra K L]
    (α : L) : Set ENNReal :=
  {r | ∃ α' : L, IsConjRoot K α α' ∧ α ≠ α' ∧
...
theorem chapter12_krasner_radius_pos
    ...
    (hseparable : (minpoly K α).Separable)
...
theorem chapter12_krasner_proximity_iff_radius
    {K L : Type*} [Field K] [NormedField L] [Algebra K L]
    (α β : L) (hfinite : Set.Finite (chapter12KrasnerDistanceSet (K := K) α)) :
    chapter12KrasnerProximity (K := K) α β ↔
      ENNReal.ofReal ‖β - α‖ < chapter12KrasnerRadius (K := K) α := by
  sorry

/-- Replacing the norm by any positive natural power does not change the
...
theorem chapter12_conjugate_lies_on_krasner_boundary
    {K L : Type*} [Field K] [NormedField L] [Algebra K L]
    {α β : L} (hconj : IsConjRoot K α β) (hne : α ≠ β) :
    ENNReal.ofReal ‖β - α‖ ∈ chapter12KrasnerDistanceSet (K := K) α := by
  exact ⟨β, hconj, hne, rfl⟩
