...
class Chapter07InitialFormData (A : Type u) [CommRing A] [IsLocalRing A] where
  initialForm : Chapter07LocalEffectiveCartierCurve A →
    MvPolynomial (Fin 2) (ResidueField A)
...
def Chapter07InitialForm {A : Type u} [CommRing A] [IsLocalRing A]
...
def Chapter07TangentDirection (A : Type u) [CommRing A] [IsLocalRing A] :=
  Projectivization (ResidueField A) (Fin 2 → ResidueField A)
...
criterion. -/
def Chapter07SmoothAt {A : Type u} [CommRing A] [IsLocalRing A]
...
theorem chapter07_transverse_smooth_pair_is_regular_parameters
    ...
    IsRegular A [C.equation, D.equation] ∧
      Nonempty (A ⧸ Chapter07IntersectionIdeal C D ≃+* ResidueField A) := by
  sorry
