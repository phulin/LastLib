...
noncomputable def chapter02ProjectiveBundleUniversalEquiv
  ...
  Classical.choice (chapter02_projective_bundle_universal_equiv_exists P f)
...
projective-morphism predicate uses Mathlib's `IsClosedImmersion` and the book's standard projective
space package.
-/
def Chapter02ProjectiveMorphism {S X : Scheme.{u}} (f : X ⟶ S) : Prop :=
  ∃ r : ℕ, ∃ P : Chapter02ProjectiveSpaceData S (Chapter02ProjectiveSpaceIndex r),
    ∃ i : X ⟶ P.bundle.scheme,
      i ≫ P.bundle.projection = f ∧ IsClosedImmersion i
