...
def chapter12InducedFiniteImageRepresentation
    {Γ H G V : Type*} [Group Γ] [Group H] [Fintype H] [Group G]
    [AddCommGroup V] [Module ℚ V] [FiniteDimensional ℚ V]
    (ι : H →* G) (W : Chapter12FiniteImageRepresentation Γ H V) :
    Representation ℚ G (Representation.IndV ι W.representation) :=
...
theorem chapter12_frobenius_reciprocity_for_character_pairing
    {G H V : Type*} [Group G] [Fintype G] [Group H] [Fintype H]
    [AddCommGroup V] [Module ℚ V] [FiniteDimensional ℚ V]
    (ι : H →* G) (AG : G → ℚ) (hconj : chapter12IsClassFunction AG)
...
theorem chapter12_regular_character_pairing
    {H V : Type*} [Group H] [Fintype H]
    [AddCommGroup V] [Module ℚ V] [FiniteDimensional ℚ V]
    (ρ : Representation ℚ H V) :
    chapter12CharacterPairing (chapter12RegularCharacter H) ρ.character =
      (Module.finrank ℚ V : ℚ) := by
  sorry

theorem chapter12_induction_pairing_expansion
    {H V : Type*} [Group H] [Fintype H]
    [AddCommGroup V] [Module ℚ V] [FiniteDimensional ℚ V]
    (AH : H → ℚ) (p : Chapter12LocalExtensionNumbers)
    (ρ : Representation ℚ H V) :
    chapter12CharacterPairing
        (fun h => (p.residueDegree : ℚ) * AH h +
          (p.discriminantExponent : ℚ) * chapter12RegularCharacter H h)
        ρ.character =
      (p.discriminantExponent : ℚ) * (Module.finrank ℚ V : ℚ) +
        (p.residueDegree : ℚ) * chapter12CharacterPairing AH ρ.character := by
  sorry
...
theorem chapter12_induction_conductor_formula
    {G H V : Type*} [Group G] [Fintype G] [Group H] [Fintype H]
    [AddCommGroup V] [Module ℚ V] [FiniteDimensional ℚ V]
    (ι : H →* G) (AK : Chapter12ConductorProfile G)
    (AM : Chapter12ConductorProfile H)
    (p : Chapter12LocalExtensionNumbers) (ρ : Representation ℚ H V)
    (hoff : ∀ h, h ≠ 1 →
    ...
    AK.artinConductor _ (chapter12InducedRepresentation ι ρ) =
      chapter12InductionRHS p (Module.finrank ℚ V) (AM.artinConductor _ ρ) := by
  sorry
...
theorem chapter12_induction_conductor_formula_using_discriminant_exponent
    ...
    (hone : AK.artinCharacter (ι 1) -
      (p.residueDegree : ℚ) * AM.artinCharacter 1 =
        (Nat.card H : ℚ) * (p.discriminantExponent : ℚ)) :
    AK.artinConductor _ (chapter12InducedRepresentation ι ρ) =
      p.discriminantExponent * Module.finrank ℚ V +
...
theorem chapter12_induction_formula_in_galois_realization
    {K N Γ V : Type*} [Field K] [Field N] [Algebra K N]
    ...
    (ρ : Representation ℚ R.subgroup V)
    (hoff : ∀ h, h ≠ 1 →
    ...
    AK.artinConductor _ (chapter12InducedRepresentation R.subgroup.subtype ρ) =
      chapter12InductionRHS hKM.numbers (Module.finrank ℚ V)
