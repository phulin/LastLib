...
/-- If a valuation ring is generated by one element over coefficients fixed by
an automorphism, congruence on that generator controls congruence on the whole
ring.  This is the algebraic step behind the uniformizer test. -/
theorem chapter01_higher_congruence_of_fixed_coefficients_and_generator
    {R K L : Type*} [CommRing R] [Field K] [Field L] [Algebra K L]
    (A : ValuationSubring L) [Algebra R A]
    (π : A) (n : ℕ) (σ : chapter01DecompositionGroup K A)
    (hπ : σ • π - π ∈ (IsLocalRing.maximalIdeal A) ^ n)
    (hcoeff : ∀ r : R, σ • algebraMap R A r = algebraMap R A r)
    (hgen : Algebra.adjoin R ({π} : Set A) = ⊤) :
    LastLib.Book02FiniteExtensionsOfLocalFields.Chapter05.chapter05HigherCongruence
      A n σ := by
  intro x
  have hx : x ∈ Algebra.adjoin R ({π} : Set A) := by
    rw [hgen]
    trivial
  ...
  | mem x hx =>
      ...
      exact hπ
  | algebraMap r =>
      rw [hcoeff r, sub_self]
      exact (IsLocalRing.maximalIdeal A ^ n).zero_mem
  | add x y _ _ hx hy =>
      rw [smul_add, add_sub_add_comm]
      exact (IsLocalRing.maximalIdeal A ^ n).add_mem hx hy
  | mul x y _ _ hx hy =>
      rw [smul_mul']
      have hproduct : σ • x * (σ • y) - x * y =
          (σ • x) * (σ • y - y) + (σ • x - x) * y := by
        ring
      rw [hproduct]
      exact (IsLocalRing.maximalIdeal A ^ n).add_mem
        ((IsLocalRing.maximalIdeal A ^ n).mul_mem_left (σ • x) hy)
        ((IsLocalRing.maximalIdeal A ^ n).mul_mem_right y hx)

/-- With fixed coefficient lifts and a monogenic uniformizer presentation,
testing the first higher congruence on the uniformizer is sufficient. -/
theorem chapter01_higher_congruence_two_iff_uniformizer
    {R K L : Type*} [CommRing R] [Field K] [Field L] [Algebra K L]
    (vL : Valuation L ℤᵐ⁰) [IsDiscreteValuationRing vL.valuationSubring]
    [Algebra R vL.valuationSubring]
    (π : vL.valuationSubring) (σ : chapter01InertiaGroup K vL.valuationSubring)
    (hcoeff : ∀ r : R,
      (σ : chapter01DecompositionGroup K vL.valuationSubring) •
          algebraMap R vL.valuationSubring r = algebraMap R vL.valuationSubring r)
    (hgen : Algebra.adjoin R ({π} : Set vL.valuationSubring) = ⊤) :
    LastLib.Book02FiniteExtensionsOfLocalFields.Chapter05.chapter05HigherCongruence
        vL.valuationSubring 2
        (σ : chapter01DecompositionGroup K vL.valuationSubring) ↔
      chapter01FirstInfinitesimalCongruence vL π σ := by
  constructor
  · intro h
    exact h π
  · intro h
    exact chapter01_higher_congruence_of_fixed_coefficients_and_generator
      vL.valuationSubring π 2
      (σ : chapter01DecompositionGroup K vL.valuationSubring) h hcoeff hgen
...
/-- Under an explicit fixed-coefficient monogenic presentation, the character
kernel is wild inertia.  Such a presentation is supplied classically by the
maximal unramified stage under the usual defectless local-extension hypotheses. -/
theorem chapter01_tame_character_kernel_eq_wild_inertia
    {R K L : Type*} [CommRing R] [Field K] [Field L] [Algebra K L]
    (vL : Valuation L ℤᵐ⁰) [IsDiscreteValuationRing vL.valuationSubring]
    [Algebra R vL.valuationSubring]
    (π : vL.valuationSubring) (hπ : chapter01IsUniformizer vL π)
    (hcoeff : ∀ σ : chapter01InertiaGroup K vL.valuationSubring, ∀ r : R,
      (σ : chapter01DecompositionGroup K vL.valuationSubring) •
          algebraMap R vL.valuationSubring r = algebraMap R vL.valuationSubring r)
    (hgen : Algebra.adjoin R ({π} : Set vL.valuationSubring) = ⊤) :
  ...
  ext σ
  rw [chapter01_tame_character_mem_kernel_iff]
  change chapter01FirstInfinitesimalCongruence vL π σ ↔
    LastLib.Book02FiniteExtensionsOfLocalFields.Chapter05.chapter05HigherCongruence
      vL.valuationSubring 2
      (σ : chapter01DecompositionGroup K vL.valuationSubring)
  exact (chapter01_higher_congruence_two_iff_uniformizer
    vL π σ (hcoeff σ) hgen).symm
