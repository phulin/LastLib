...
class Chapter11CompleteDVR (A : Type*) [CommRing A] [IsDomain A]
...
theorem chapter11_generic_fiber_is_the_field_extension
    (A B K L : Type*) [CommRing A] [CommRing B] [IsDomain B] [Field K] [Field L]
...
theorem chapter11_global_ideal_factorization
    ...
    (he : ∀ i, e i = (P i).ramificationIdx A) :
    Ideal.map (algebraMap A B) m = ∏ i, (P i) ^ e i := by
  sorry
...
theorem chapter11_branch_ideals_pairwise_comaximal
    ...
    (P : ι → Ideal B) (hmax : ∀ i, (P i).IsMaximal) :
...
abbrev chapter11PrimePowerLayer (B : Type*) [CommRing B] (P : Ideal B) (i : ℕ) : Type _ :=
  chapter11SubmoduleQuotient
    (Submodule.comap (P ^ i : Submodule B B).subtype
      (P ^ (i + 1) : Submodule B B))
...
theorem chapter11_prime_power_layer_is_a_residue_line
    ...
    [P.IsPrime] [P.IsMaximal] (e i : ℕ) (hi : i < e) :
    Nonempty
...
theorem chapter11_sum_ramification_times_inertia_is_degree
    ...
    {ι : Type*} [Fintype ι] (m : Ideal A) (P : ι → Ideal B)
    (hbranch : ∀ i, chapter11Branch A B m (P i)) :
