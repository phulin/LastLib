...
Finally, twisting a group and twisting a representation are different operations. An inner form of $\operatorname{PGL}_n$ always exists for a central simple algebra of degree $n$. Its standard $n$-dimensional projective representation descends, but a genuine $n$-dimensional linear representation need not. Such a representation would split the algebra. Corestriction later supplies a larger representation when the small one is obstructed.
...
The isomorphism is not canonical until a model for the tensor-induced space is chosen, but the split algebra is canonical up to unique algebra isomorphism.
...
This converse uses the local-global classification and the supply of nonsplit auxiliary places. Over a general field, vanishing of transfer need not imply that a quaternion algebra descends.
...
Over a normal splitting field,

$$
W\otimes_E L\cong\bigotimes_{\tau:F\hookrightarrow L}V_\tau,
$$

where each $V_\tau$ is a two-dimensional splitting module for ${}^\tau B$. The group $\operatorname{Res}_{F/E}B^\times$ acts projectively on this tensor product. A splitting of the corestriction upgrades the action to a linear action on $W$, subject to a central character. Thus the algebra splitting is precisely the descent condition for the tensor representation.
...
The kernel of the tensor action must still be audited. Over $k^s$, scalar tuples with product one act trivially. One obtains faithfulness by either quotienting this central kernel or adjoining a faithful character representation. The direct trace-pairing representation on $B$ avoids this kernel and is often preferable when faithful algebra action is part of the moduli data.
...
For Hamilton quaternions, a pure unit quaternion provides such a $J$, with the sign of $\delta$ chosen consistently. At a split real place, positivity selects one of the two connected conjugacy classes of complex structures. The algebraic alternating identity alone does not choose that class.
...
The real homomorphism must give only the two weights appropriate to a weight-one polarized structure. Equivalently, after complexification the representation decomposes into two conjugate subspaces on which the Deligne torus acts through $z$ and $\bar z$. Positivity requires $\psi(x,h(i)y)$ to be definite with the chosen sign. These conditions constrain the real ramification and cannot be recovered from the finite-place Brauer class alone.
...
| trace-pairing nondegeneracy | finite separable $F/k$, central simple $B/F$, invertible pure $\delta$ | nondegenerate alternating $k$-form |
...
| minimal tensor representation | split $\operatorname{Cor}_{F/k}(B)$ | $k$-space of dimension $2^{[F:k]}$ |
