...
**Proof strategy.** The cotangent calculation has already produced the determinant. To connect it
to congruences, compare the Koszul resolutions on the relation sequence and on the coordinate
sequence. The comparison matrix is a divided-difference matrix. Its top exterior power both
produces the Jacobian determinant and generates the annihilator of the augmentation ideal.

**Proof.** Put $S=\mathcal O[[X_1,\ldots,X_n]]$ and $I=(X_1,\ldots,X_n)A$. Since every
$f_i$ vanishes at the augmentation, choose coefficients $h_{ij}\in S$ such that

$$
f_i=\sum_{j=1}^nh_{ij}X_j.
$$

Write $H=(h_{ij})$. The matrix $H$ gives a morphism from the Koszul complex on the
$f_i$ to the Koszul complex on the $X_j$; in degree $n$ it is multiplication by
$\det H$. The first complex resolves $A$ because the $f_i$ form a regular sequence.
After tensoring it with $\mathcal O=S/(X_1,\ldots,X_n)$, every differential is zero, so

$$
\operatorname{Tor}_n^S(A,\mathcal O)\simeq\mathcal O.
$$

Computing the same group with the Koszul resolution on $X_1,\ldots,X_n$ gives

$$
\operatorname{Tor}_n^S(A,\mathcal O)
=H_n(X_1,\ldots,X_n;A)
=\operatorname{Ann}_A(I).
$$

Under these two descriptions, the class of $1$ on the first side maps to the class of
$\det H$ on the second. Hence $\operatorname{Ann}_A(I)$ is free of rank one over
$\mathcal O$, generated by the image of $\det H$. Evaluating at the augmentation gives

$$
\eta_A=(\lambda(\det H))=(\det H(0)).
$$

But $H(0)=J_\lambda$, because $h_{ij}(0)$ is the coefficient of $X_j$ in the linear
part of $f_i$. Therefore $\eta_A=(\det J_\lambda)$. The square presentation of
$\Phi_A$ gives the two length formulas. This argument also covers $n=0$: then
$A=\mathcal O$, both determinants are $1$, and both numerical modules vanish. $\square$
...
can be used without importing a theory of differents. Introduce a second set of variables and
write
...
in $S\widehat\otimes_{\mathcal O}S$. In $A\otimes_{\mathcal O}A$, the left side is zero. If
$H=(h_{ij})$, multiplication by $\operatorname{adj}(H)$ gives
...
Apply the augmentation to the $Y$-factor. The resulting element

$$
d_g=\det(H(X,0))\in A
$$

annihilates every $X_j$, hence annihilates $I$. Applying the augmentation also to the
$X$-factor gives

$$
\lambda(d_g)=\det(H(0,0))=\det J_g(0).
$$

Therefore every maximal minor belongs to
$\lambda(\operatorname{Ann}_A I)=\eta_A$.
...
For the equality case, it is useful to spell out the standard reduction argument. After a faithfully flat extension of complete DVRs with ramification index one, if necessary, the residue field may be assumed infinite. This extension need not be finite; a finite unramified extension of a finite residue field would still have finite residue field. One can then choose $g_1,\ldots,g_n\in J$ simultaneously so that their reductions generate an $\mathfrak m$-primary ideal in $S/(\varpi)$ and so that their evaluated differential determinant has the least possible valuation, namely the valuation of $\operatorname{Fitt}^0(\Phi_A)$. These are simultaneous generic conditions on the coefficient matrix used to combine a finite generating set of $J$: the first avoids the finitely many proper linear conditions coming from the special-fiber minimal primes, and the second asks that a maximal minor generating the determinant lattice have unit coefficient. Put $Q=(g_1,\ldots,g_n)$ and $B=S/Q$.
Flatness identifies the base-changed conormal module with $\Phi_A\otimes\mathcal O'$.
It also identifies the base change of $\operatorname{Ann}_A(I_A)$ with the new annihilator:
write the annihilator as
$\operatorname{Hom}_A(A/I_A,A)$ and use finite presentation of $A/I_A$ to commute
$\operatorname{Hom}$ with flat base change. Thus both the Fitting ideal and the congruence
ideal extend to $\mathcal O'$, and faithful flatness reflects their equality. The reduction to an
infinite residue field therefore neither assumes nor loses the desired equality.
...
The quotient $B\twoheadrightarrow A$ has kernel $L=J/Q$. We use the following filtered
determinant assertion, whose proof is part of the argument rather than an additional hypothesis:
...
Put $\mathfrak x=(X_1,\ldots,X_n)B$. The Koszul complex on the $g_i$ resolves $B$ over
$S$, and the Koszul complex on the $X_i$ computes the augmentation homology. A comparison of
these two complexes has in degree one the matrix of evaluated differentials and in degree $n$
its top exterior determinant. Filter the comparison by the $\mathfrak x$-adic filtration. The
filtration induced on $L$ is separated, and Artin--Rees identifies it, from some degree onward,
with the filtration $\mathfrak x^rL$. Thus $L\ne0$ has a first nonzero initial class in the
associated graded comparison.

We now follow that class through the filtered Koszul complex. Before its first degree, the two
complexes agree. In its first degree, the class is a new relation and its Koszul boundary is
nonzero; otherwise it would lie one step deeper in the induced filtration, contrary to the
choice of the initial class. Exterior multiplication carries this boundary to top degree. The
top comparison is multiplication by the determinant of the evaluated conormal map, so the new
top class is represented in the quotient of the two annihilator lines

$$
\operatorname{Ann}_A(I_A)/\operatorname{Ann}_B(I_B).
$$

Here the denominator denotes the injective image of $\operatorname{Ann}_B(I_B)$ in $A$.
It is injective because this annihilator is the rank-one lattice $\eta_Be_\lambda$ and its
nonzero augmentation prevents it from meeting $L\subseteq I_B$. The augmentation identifies
the two annihilator lines with $\eta_B\subseteq\eta_A$, so it identifies their quotient with
$\eta_A/\eta_B$.

The top class therefore gives a nonzero class in $\eta_A/\eta_B$. To see that the class cannot
die at a later filtration stage, lift a hypothetical later boundary through the Rees comparison.
The defining boundary equation would give $b\in B$ and $c>0$ with
$\varpi^cb\in L$ but $b\notin L$. This is impossible: $A=B/L$ is $\mathcal O$-flat, so $L$ is
$\varpi$-saturated in $B$. Induction on the filtration degree therefore preserves the first top
class. This proves (7.3.1). This is the determinant-defect step: it is the filtered form of the
top-degree Koszul comparison, and it uses both Artin--Rees and $\mathcal O$-flatness.
...
Notice that the implication compares actual ideals, not a signed Euler characteristic. If
...
Here is the precise Nakayama step. If the chosen lifts are $x_1,\ldots,x_n$, then

$$
I_R=(x_1,\ldots,x_n)+I_R^2
\subseteq (x_1,\ldots,x_n)+\mathfrak m_RI_R.
$$

The ideal $I_R$ is a finite $R$-module because $R$ is Noetherian, so Nakayama gives
$I_R=(x_1,\ldots,x_n)$ as an $R$-ideal. Equivalently, the classes of the $x_i$ span the
...
**Proof strategy.** The stronger theorem already proved applies directly. The source
complete-intersection hypothesis also gives the historical length interpretation: a quotient can
only shorten the cotangent module and enlarge the congruence ideal, so sharpness leaves no room for
either loss.
...
Thus the branch defect $\eta_T/\eta_R$ vanishes. For kernel elimination, apply the
Wiles--Lenstra theorem of Section 8.2: its source cotangent finiteness follows from the displayed
source equality, and all its other hypotheses are part of the present statement. It gives
$R\simeq T$. The target therefore inherits the complete-intersection structure. $\square$
...
The useful operation occurs before reaching relative dimension zero. Suppose $A$ is an augmented
complete Noetherian Cohen--Macaulay $\mathcal O$-algebra of relative dimension $r$, meaning
$\dim A=r+1$, and $z_1,\ldots,z_r\in I_A$ is an $A$-regular sequence such that

$$
\overline A=A/(z_1,\ldots,z_r)
$$

is finite flat. Put $L=(z_1,\ldots,z_r)$. The exact conormal sequence is

$$
0\longrightarrow\frac{L}{L\cap I_A^2}
\longrightarrow\Phi_A
\longrightarrow\Phi_{\overline A}
\longrightarrow0.
$$

Regularity gives $L/L^2\simeq\overline A^r$, but it does not by itself identify the left
term above with $\mathcal O^r$. The necessary extra equality is

$$
L\cap I_A^2=I_AL.
$$

Under this equality the classes of the $z_i$ identify the left term with $\mathcal O^r$. If
their image splits off a free direct summand of $\Phi_A$, quotienting removes exactly those
free relative directions, and the remaining torsion module is $\Phi_{\overline A}$. Without
the intersection equality, a linear dependence can be lost before the quotient and this
description is false.

There is no equally formal formula for the congruence ideal. It must be computed in
$\overline A$, or obtained from a proved comparison of the two branch-idempotent lattices.
Regularity of the $z_i$ alone controls the Koszul complex, not the denominator of the selected
generic idempotent.

In comparisons, take compatible regular sequences in $R$ and $T$, require their images to agree,
require regularity on both rings, verify the displayed intersection equality wherever free
directions are to be removed, and verify that the quotients are finite flat. Complete-intersection
presentations then pass to the quotients. The numerical inequality itself does not follow from
regularity: one must recompute $\Phi$ and $\eta$ after quotienting, or prove exact comparison
formulas for both. Once that quotient-level bound is known, the numerical criterion applies. If a
sequence is regular only on the source, its image can be a zerodivisor on the target and even this
comparison fails.
...
DVRs of ramification index $e$, and put $B=A\otimes_{\mathcal O}\mathcal O'$. If $B$ is local,
put $A'=B$. If $B$ is semilocal, let $A'=B_{\mathfrak m'}$ at the maximal ideal selected by the
extended augmentation and assume this localization is branch preserving in the sense of
Section 11.2. Then
...
To deduce relative self-duality without importing a separate duality theorem, pass to the special
fiber. Flatness of $A$ says that $\varpi$ is regular after quotienting by the $f_i$. Hence

$$
f_1,\ldots,f_n,\varpi
$$

is an $S$-regular sequence. Permutability puts $\varpi$ first, so the reductions
$\overline f_1,\ldots,\overline f_n$ form a regular sequence in
$P=k[[X_1,\ldots,X_n]]$. Thus

$$
\overline A=P/(\overline f_1,\ldots,\overline f_n)
$$

is a zero-dimensional complete intersection. Compute
$\operatorname{Tor}_n^P(\overline A,k)$ in two ways. The Koszul resolution on the
$\overline f_i$, tensored with $k$, has zero differentials and gives one copy of $k$ in top
degree. The Koszul resolution on the $X_i$, tensored with $\overline A$, gives

$$
\operatorname{Tor}_n^P(\overline A,k)
=H_n(X_1,\ldots,X_n;\overline A)
=\operatorname{Soc}(\overline A).
$$

The special-fiber socle is therefore one-dimensional. The criterion of Section 15.2 now gives
$A^\vee\simeq A$. At the augmented point, the top Koszul comparison of Section 6.3 is
multiplication by $\det J_\lambda$; this is why the same determinant presents $\Phi_A$ and
generates the annihilator branch line. In a monogenic algebra, this generator is the
complementary factor $g(X)$ in $f=Xg$, whose augmentation is $f'(0)$.
