...
The reason these particular groups occur is arithmetic, not merely categorical. Book 143 constructs on the generic fiber of $J_0(N)$ a constant cuspidal subgroup
...
Book 143 establishes the following prime-level generic package. The cuspidal divisor
...
For $N=2,3$, Book 143 proves that the Jacobian, cuspidal group, and component group under discussion are trivial, and $n_N=1$. For
...
The prime $3$ is exceptional in formulas involving the denominator $12$ and the elliptic point of order three on the modular curve. Those exceptional automorphisms have already been incorporated into the exact integer $n_N$ by Book 143. Once $3\mid n_N$ is fixed, the local finite-flat theory at $3$ is not at the low-ramification boundary:
...
Book 46 supplies the exact category of finite-flat commutative group schemes, represented faithfully flat quotients, rank multiplication, finite étaleness when the order is invertible, and schematic closure over DVRs and Dedekind domains. We apply the Dedekind closure theorem to $\mathbf Z[1/N]$, and we never treat an arbitrary nonflat kernel as admissible.

Book 47 supplies exact Cartier duality, the exchange of constant and diagonalizable groups, annihilators, closure–annihilator compatibility, and the Tate-twisted generic module dual. Every use occurs in an admissible finite-flat sequence or for a finite-flat subgroup.

Book 28 supplies the fppf Kummer sequence in every characteristic, the calculation
