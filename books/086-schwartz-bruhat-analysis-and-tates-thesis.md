...
Here is the archimedean distribution argument rather than an appeal to a classification theorem. If a distribution $T$ is supported at zero, continuity on the test functions supported in one fixed compact neighborhood bounds it by finitely many derivative seminorms, say through order $N$. Taylor's formula with a cutoff writes

$$
\phi(x)=\sum_{r=0}^N\frac{\phi^{(r)}(0)}{r!}x^r\eta(x)+R_N(x)
$$

over $\mathbf R$, where every derivative of $R_N$ through order $N$ vanishes at zero. Shrinking a cutoff around zero shows $T(R_N)=0$, since the derivative bounds tend to zero there and the part away from zero is killed by the support condition. Thus $T$ is a finite linear combination of the derivatives $\delta_0^{(r)}$. The identical two-variable Taylor argument over $\mathbf C$ expresses a point-supported distribution as a finite sum of $\partial_z^r\partial_{\bar z}^t\delta_0$. Direct substitution under $x\mapsto ax$ shows that these derivatives have distinct integral radial degrees and the indicated parity or angular weights. They can match the prescribed covariance only at a discrete set of $s$.

On $F^\times$, twist a covariant distribution by the inverse of the smooth character $\chi(x)|x|^s$. The result is invariant under multiplication. An invariant distribution on either orbit is a scalar multiple of multiplicative Haar measure: choose a compactly supported smooth $h$ of Haar integral one; invariance and Fubini give $T(f*h)=T(f)$, while translating $h$ inside the convolution gives $T(f*h)=T(h)\int f\,d^\times x$. Over $\mathbf C^\times$ there is one orbit. Over $\mathbf R^\times$ there are the positive and negative orbits, and covariance under $-1$ relates their two scalars by the prescribed parity. Hence restriction to $F^\times$ leaves exactly one scalar. Away from the discrete point-supported parameters this proves one-dimensionality. The archimedean zeta families are meromorphic by the Taylor-subtraction argument, so continuation across those parameters completes the uniqueness proof there.
