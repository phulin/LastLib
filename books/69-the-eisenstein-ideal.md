...
12. [The exported theorem ledger](#12-the-exported-theorem-ledger)
    ...
    - [The exact interface with Book 70](#124-the-exact-interface-with-book-70)
...
The incoming convention for correspondences is the convention of Books 53 and 68. In particular, if a correspondence is displayed as
...
are handled separately. Book 67 proves that $X_0(p)$ has genus zero at precisely these prime levels, and Book 68 gives
...
Book 68 proves independently that $c$ has exact order $n$. It also proves, with the incoming convention,
...
Thus Book 28's congruence module is exactly the cyclic module in (4.5). The algebra $A$ is finite flat over $\mathbf Z_\ell$, and its generic fiber
...
The exact index has three geometric companions, already constructed in Book 68. We recall their independent origins because the later intersection and torsion proofs use their maps, not only their orders.
...
At $p$, Book 68 identifies the toric character lattice and its monodromy pairing as
...
The two groups in (6.3) and (6.4) are not dual copies of one calculation. Their coefficient twists, their connected--étale orientations at $\ell$, and their monodromy conditions at $p$ differ. Book 17's local duality explains their orthogonality, while Books 18 and 19 assemble the stated local permissions into honest global Selmer structures.
...
The devissage is where extension classes become a statement about the whole group scheme. It does not assume that every generic extension is finite flat, and it does not infer a subgroup from a semisimplified trace. These are precisely the tasks supplied by Book 24's exact finite-flat category and Mazur's admissibility theorem.
...
At $2$, the constant and cyclotomic characters coincide, $\mu_2(\mathbf Q)$ is nontrivial, and the quadratic point (5.9) lies simultaneously in the known constant and multiplicative subgroups. The residual group scheme need not split into two distinguishable character lines. In addition, Book 24's low-ramification theorem assumes $\ell>2$: the weight interval $[0,1]$ is not inside a strict Fontaine--Laffaille full-faithfulness range at $2$, and trace-zero splittings also fail.
...
The proof works with connected--étale closures over $\mathbf Z_2$ and the actual Cartier geometry of the modular stack. It is not an application of Book 24: Raynaud's odd low-ramification full faithfulness and the Fontaine--Laffaille interval used there do not apply at $2$. Nor is the result a claim that the whole local Hecke block has multiplicity one.
...
Only now do we conclude that rational torsion is Eisenstein. Book 68's explicit Hecke calculation on the generator $c$ gives
...
The same holds with $0$ in place of $\infty$. This odd good-prime cusp-disk rigidity is the clean interface exported to Book 70.
...
| Mazur--Raynaud odd multiplicity one | $J[\mathfrak m]=\mathbf Z/\ell\oplus\mu_\ell$ | Raynaud uniformization, Book 24 finite-flat classification, the two Selmer orientations, cyclotomic class groups, and admissible-group-scheme devissage |
...
The formal dependency graph has direct arrows from Books 37 and 68. Book 37 supplies faithful image algebras, localization, Hecke duality, and the congruence-versus-cotangent discipline; Book 68 supplies the prime-level cusp, Shimura subgroup, component group, monodromy lattice, and incoming variance. The proof architectures expose their earlier foundations rather than silently strengthening them: Book 9 for covariant Tate modules, Books 15--19 for continuous and local--global Galois cohomology, Book 24 for odd finite-flat closure and classification, Book 28 for the numerical inequality, Book 53 for correspondence adjoints, and Books 67--68 for integral modular geometry. No conclusion from Book 70 or any later book enters.

The index theorem does not use rational torsion. Its upper bound is the two-cusp theorem, and its lower bound is only the already known cusp class. The augmented fiber product is derived after the index theorem. Book 28 then gives only the inequality (4.7); no equality of cotangent and congruence lengths is asserted without another theorem.
...
### 12.4 The exact interface with Book 70

Book 70 may use the following implication without adding an Eisenstein hypothesis. Let $p$ be a positive-genus prime level, let $r$ be an odd prime with $r\ne p$, and let $x\in X_0(p)(\mathbf Q)$. If the integral moduli argument of Book 67 shows that $x$ reduces to $0$ or $\infty$ modulo $r$, then the winding quotient and formal immersion give
...
If the only available cusp disk has residue characteristic $2$, Book 70 must prove a quotient-level specialization-kernel theorem before making the same deduction. It may not infer that theorem from

$$
J_0(p)(\mathbf Q)_{\mathrm{tors}}=C.
$$

The final classification of prime degrees, the treatment of noncuspidal reductions, and the exceptional prime list remain in Book 70.
