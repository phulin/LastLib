...
9. [Dualizing sheaves](#9-dualizing-sheaves)
   ...
   - [Discharging the residue previews](#95-discharging-the-residue-previews)
   - [Proper pushforward, Stein factorization, and connected fibers](#96-proper-pushforward-stein-factorization-and-connected-fibers)
...
(Theorems 2.7 and 2.10); coherent exhaustion of a quasi-coherent sheaf on a noetherian scheme
...
The sequence $x_1,x_2$ is regular. Indeed $R$ is a domain, so $x_1$ is a non-zero-divisor. Every
minimal prime over $(x_1)$ has height one by the principal ideal theorem (it cannot have height
zero because $x_1\ne0$), while the maximal ideal has height two; hence $R/x_1R$ has dimension one.
Its maximal ideal is generated by the image of $x_2$, so its embedding dimension is at most one
and therefore equals its dimension. Thus $R/x_1R$ is regular local of dimension one, hence a
discrete valuation ring and in particular a domain, so the image of $x_2$ is a non-zero-divisor
there. Consequently the Koszul complex
...
The next two global results are previews, not results available at this point in the dependency
order. Their detailed arguments are recorded here to keep the residue calculation together, but
they use the duality and trace theorems of Chapter 9. Nothing in Sections 9.1--9.4 uses either
preview. Section 9.5, after all of those inputs have been proved, explicitly discharges the two
arguments and promotes the previews to Theorems 8.8 and 8.9. Until then only Sections 8.1--8.4 may
be cited.

**Deferred statement 8.8 (residue theorem).** If $X$ is a smooth proper geometrically connected curve over $k$
...
**Deferred argument for statement 8.8 (discharged in Section 9.5).** Let $\underline{\Omega}$ denote the constant sheaf on $X$ with value
...
**Deferred statement 8.9 (local trace formula).** For every closed point $x\in X$ and every
...
**Deferred argument for statement 8.9 (discharged in Section 9.5).** The argument is the one already used inside the proof of Theorem 8.8, now applied to
...
residue theorem enters any argument through Section 9.4. The deferred arguments recorded in
Sections 8.5--8.6 are therefore discharged only in Section 9.5, after the needed trace and the
identification of the smooth dualizing sheaf have become available. The resulting comparison is
refined in Section 12.2.
...
consume those hypotheses. Section 9.6 uses (i) and (ii) in that greater generality.
...
### 9.5 Discharging the residue previews

**Theorems 8.8 and 8.9 (global residues and finite trace).** The two deferred statements in
Sections 8.5 and 8.6 hold, with the normalizations and functoriality asserted there.

**Proof.** The complete arguments were recorded with the statements. Their only inputs beyond
Sections 8.1--8.4 are absolute curve duality and its finite trace (Theorem 9.1 and (9.8)) and the
identification $\omega_X=\Omega^1_{X/k}$ for a smooth curve (Section 9.4). All of those inputs now
precede this result, while Sections 9.1--9.4 used none of the deferred statements. Reading the two
recorded arguments with these proved inputs first gives Theorem 8.8 and fixes the residue
normalization of the duality trace, then gives Theorem 8.9. Thus there is no circular or forward
proof dependency. $\square$

From this point onward Theorems 8.8 and 8.9 are available for citation.

### 9.6 Proper pushforward, Stein factorization, and connected fibers
