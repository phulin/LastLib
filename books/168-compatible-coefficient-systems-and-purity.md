...
7. [Local Weil--Deligne compatibility](#7-local-weil--deligne-compatibility)
   ...
   - [Cuspidal irreducibility and uniform passage](#75-cuspidal-irreducibility-and-uniform-passage)
...
For the uniform packet array there is a narrower route than ambient Tate-module semisimplicity. A cuspidal parallel-weight-two packet cannot have a reducible semisimple Galois realization once its one-dimensional geometric constituents are known to be locally algebraic at the coefficient prime: global class field theory turns such constituents into algebraic Hecke characters, and isobaric multiplicity one contradicts cuspidality. This volume proves that reduction below. Book 47, Proposition 9.1 proves the subquotient passage, and Book 6, Section 11.5 turns a locally algebraic constituent into an algebraic Hecke character. The rank-one Hodge--Tate-to-locally-algebraic theorem, which the uniform route also needs, is proved in no volume of this library: Book 6, Section 11.5 defers it to Book 24a, _Tate--Sen Theory and $\mathbf C_\ell$ Period Foundations_, which is not yet written. So the **uniform all-coefficient statement** rests on two p-adic Hodge inputs, neither available: that deferred theorem and the smooth-proper comparison $(\mathrm{HT}_{\mathrm{curve}})$ isolated in Book 47, (9.7). The final fixed-three chain is narrower still: Theorem 7.6C obtains local algebraicity directly from the good-reduction all-level finite-flat tower and Book 54, Proposition 14.2. Thus its selected member does not require $(\mathrm{HT}_{\mathrm{curve}})$. Ambient semisimplicity remains a sufficient alternative, not the recommended FLT route.
...
This proposition concerns a raw calculation on the basic carrier itself. For the semisimple
packet system ultimately consumed by FLT, Proposition 7.6A below gives a weaker and sufficient
route whenever $B$ is split at every dyadic factor: calculate on Book 122's unitary auxiliary
carrier and compare semisimple systems. That route does not require the finite integral
comparison (10.5) between the two curve models.
...
### 7.5 Cuspidal irreducibility and uniform passage

The application needs semisimplicity only for each rank-two multiplicity representation, not
for the whole Jacobian Tate module. The following precise local-algebraicity input is enough.
For every selected packet carrier, every coefficient place $\lambda$, every finite extension of
$E_\lambda$, and every one-dimensional Jordan--Holder constituent $\xi$ of the raw curve
multiplicity representation after that extension, impose

$$
\boxed{(\mathrm{HT}_1)\qquad
\xi|_{G_{F_w}}\text{ is locally algebraic for every }w\mid\ell(\lambda).}
\tag{7.9}
$$

The smooth-proper comparison $(\mathrm{HT}_{\mathrm{curve}})$ of Book 47, (9.7), does not by
itself yield $(\mathrm{HT}_1)$; it yields the Hodge--Tate condition, and one further theorem is
needed to convert that condition into local algebraicity. On the first half, Book 47,
Proposition 9.1 passes its Hodge--Tate decomposition to every Hecke multiplicity subquotient and
every one-dimensional Jordan--Holder factor. The second half is the rank-one Hodge--Tate
algebraicity theorem, that a one-dimensional Hodge--Tate character of a local Galois group is
locally algebraic. No volume of this library proves it: Book 6, Section 11.5 constructs the
$\ell$-adic avatar from local algebraicity and expressly defers the converse to Book 24a,
_Tate--Sen Theory and $\mathbf C_\ell$ Period Foundations_, which is not yet written.
So $(\mathrm{HT}_1)$ follows from $(\mathrm{HT}_{\mathrm{curve}})$ together with that deferred
theorem, and from no combination of results presently available.
Betti--de Rham comparison alone does not identify the restriction of the $\ell$-adic
representation at a place above $\ell$. The library has isolated but proved neither input, so
the smooth-proper p-adic comparison and the rank-one algebraicity theorem are together the
explicit arithmetic input behind $(\mathrm{HT}_1)$.

**Theorem 7.6 (cuspidal packet irreducibility from $(\mathrm{HT}_1)$).** Let $\pi$ be one of
the selected parallel-weight-two packets in the curve-attachment range whose split
$\mathrm{GL}_2$ transfer is cuspidal. If
$(\mathrm{HT}_1)$ holds for its raw multiplicity representations, then, for every coefficient
place $\lambda$, the semisimple attached representation $\rho_{\pi,\lambda}$ is absolutely
irreducible. Consequently the raw representation is already absolutely irreducible and

$$
\widetilde\rho_{\pi,\lambda}\simeq\rho_{\pi,\lambda}. \tag{7.10}
$$

Thus (1.6) holds at every $v\nmid\ell(\lambda)$. In particular, every constructed dyadic
special operator, its kernel line, and its Frobenius sign survive global semisimplification at
every coefficient place of residue characteristic different from two.

**Proof.** Suppose that $\rho_{\pi,\lambda}$ becomes reducible over an algebraic closure of
$E_\lambda$. After a finite coefficient extension it is a direct sum

$$
\rho_{\pi,\lambda}\otimes_{E_\lambda}E'_{\lambda}
=\xi_1\oplus\xi_2. \tag{7.11}
$$

The raw representation is a curve-cohomology subquotient and is unramified outside a finite
set, so the same is true of $\xi_1$ and $\xi_2$. Hypothesis $(\mathrm{HT}_1)$ makes them
locally algebraic above $\ell(\lambda)$. Book 6, Section 11.5 therefore supplies algebraic
Hecke characters $\psi_1,\psi_2$ whose $\lambda$-adic avatars are $\xi_1,\xi_2$. Book 6 uses
arithmetic Artin reciprocity, whereas (1.1) uses geometric Frobenius. Thus at an unramified
place $v$,

$$
\psi_i(\varpi_v)=\xi_i(\operatorname{Frob}^{\mathrm{arith}}_v)
=\xi_i(\Phi_v)^{-1}.
$$

Consequently the correctly normalized two-character isobaric sum is

$$
\Psi=\psi_1^{-1}\boxplus\psi_2^{-1}.
$$

At every common good place it has the same quadratic Hecke polynomial as
$\rho_{\pi,\lambda}$ and hence, after the chosen
$\overline{\mathbf Q}_\ell\simeq\mathbf C$ comparison, as $\pi$. The
isobaric multiplicity-one theorem of Book 109, Section 9.5 identifies automorphic objects from
this full Euler tail. It cannot identify the cuspidal representation $\pi$ with a
two-character isobaric sum: their isobaric decompositions have respectively one rank-two
cuspidal constituent and two rank-one constituents. This contradiction proves absolute
irreducibility.

If the raw two-dimensional representation had a proper nonzero global subrepresentation, its
semisimplification would have two nonzero Jordan--Holder factors, contradicting the absolute
irreducibility just proved. Hence the raw representation is irreducible and (7.10) follows.
Book 128, Lemma 9.1 now gives (1.6). Restriction of the global isomorphism retains the actual
local representation, so its Weil--Deligne operator, kernel line, and Frobenius action are not
lost. No bad-place datum was used to prove irreducibility, so the argument is noncircular.
$\square$

The proof uses neither residual irreducibility nor semisimplicity of the ambient Jacobian. It
also explains the exact counterboundary: cuspidality and good Frobenius polynomials alone do
not turn arbitrary continuous one-dimensional $\ell$-adic constituents into Hecke characters.
Local algebraicity is the missing bridge; omitting it would silently assume the conclusion of
a p-adic Hodge theorem.

**Proposition 7.6A (semisimple CM-unitary SP bypass).** Let $F$ be totally real of degree at
least two, assume that $2$ splits completely in $F$, let $B/F$ be a quaternion division
algebra split at exactly one real place and at every dyadic place, and let $\Pi_B$ be the
one-split transfer of a selected trivial-character
cuspidal parallel-weight-two packet $\pi$. Assume that $\pi_v$ is selected special at every
$v\mid2$. Choose an imaginary quadratic field $K/\mathbf Q$ which is split at two and
nonsplit at every finite place ramified in $B$, put $L=FK$, and form the unitary
common-multiplier curve of Book 122, Proposition 10.2. Assume $(\mathrm{HT}_1)$ for its
unitary raw packet factor.

Then, for every coefficient place of residue characteristic different from two, the
semisimple system attached to $\pi$ has the full signed special pair at every $v\mid2$.
This conclusion does not require the integral finite comparison (10.5) of Book 122.

**Proof.** The local conditions on $K$ imply $L\hookrightarrow B$ by Book 118, Section 8.5.
The existence of such a $K$ is part of the statement. When the finite ramified places lie over
rational primes split completely in $F$, as in the controlled use of prescribed auxiliary
places, Book 6's exponent-two Grunwald theorem constructs it by prescribing nonsplit local
square classes there, the split class at two, and the imaginary real sign. Without that
splitting qualification, one must separately check that one quadratic $\mathbf Q_q$-class
remains nonsplit after every relevant completion $F_w$; this is not automatic. Central
saturation is harmless for the trivial-character packet. Apply the generic
common-adjoint construction in the proof of Book 118, Corollary 10.3A, protecting the
one-step Iwahori factors at two and shrinking at auxiliary odd places. The coset-removal
argument permits this: a nontrivial quadratic obstruction is detected at an odd place outside
any prescribed finite set, while a rational derived coset can be excluded by first shrinking
its derived level at such an auxiliary place. No unramified dyadic component-field conclusion
of that corollary is invoked. The resulting generic cover pulls the nonzero $\Pi_B$-block into the
cohomology of the unitary curve after a finite scalar extension. Automorphic decomposition
and component routing in Book 125 therefore select a noncharacter unitary packet
$\Pi^{\mathrm u}$ with the same derived local factors, the same good Hecke polynomials, and
the same raw $U_v$ new-line scalar at every $v\mid2$. This is a characteristic-zero
occurrence statement; it does not extend the finite map over a dyadic integral model.

At a dyadic place the two protected levels have the same oriented Iwahori image in the common
adjoint group. The pulled-back packet has trivial action by the saturated derived central
kernel, so the corresponding right double coset acts by the same unnormalized scalar on its
one-dimensional special new line. This retains $\beta_v$, not merely its square; no
determinant choice is being used to recover a sign.

Because $B$ is split at every dyadic place and $K$ is split at two, the unitary curve is in
the exact dyadically tensor-split range of Book 122, Proposition 10.2. Its intrinsic
parahoric model, packet projector, and residue correspondence satisfy Theorem 9.1 of that
book: the raw unitary multiplicity factor has rank-one monodromy and local pair
$\operatorname{St}(\beta_v)$, with $\beta_v$ the selected $U_v$ scalar. The corresponding
base change $\pi_L$ is cuspidal: otherwise cyclic base-change classification would make
$\pi$ dihedral from $L/F$, but at a place where $L/F$ is split such an induction is principal,
contradicting the selected special dyadic factor. Thus Theorem 7.6 applies to this unitary raw
factor. Hence its raw representation is already its globally semisimple realization and
the special pair survives there at every allowed coefficient place.

Let $r_{\pi,\lambda}$ be the semisimple representation obtained from the basic carrier and
let $r^{\mathrm u}_{\lambda}$ be the unitary one over its reflex field $L$. At the
density-one set of places of $L$ having relative residue degree one over $F$, their
Frobenius polynomials are respectively the restriction of the polynomial of $\pi$ and the
same polynomial on $\Pi^{\mathrm u}$. Chebotarev and Brauer--Nesbitt, exactly as in
Proposition 10.3, give

$$
r^{\mathrm u}_{\lambda}\simeq
r_{\pi,\lambda}|_{G_L}.
\tag{7.11a}
$$

For each $v\mid2$, choose the prime $u$ of $L$ determined by one of the two split factors of
$K\otimes\mathbf Q_2$. Then $L_u=F_v$, so their decomposition groups and geometric
Frobenius elements are literally the same. Restricting (7.11a) at $u$ transfers the entire
unitary special pair, including nonzero $N$, its kernel line, and $\beta_v$, to
$r_{\pi,\lambda}|_{G_{F_v}}$. No raw-to-raw comparison, ramified local base change, or
integral extension of the common-adjoint cover is used. $\square$

The proposition bypasses (10.5), but not quaternionic parity. It applies only after an
automorphic carrier split at every dyadic factor exists. In odd degree, $W=\varnothing$
...
**Proposition 7.7 (ambient uniform raw-to-global passage).** Under
...
For each $C_i$, Book 36, (10.12) identifies the representation in (7.12) with
...
This alternative is uniform over all embeddings above a fixed coefficient characteristic and
...
If one of these packages is available at every finite $v$ and (1.6) holds at every nonzero-monodromy bad $v$, the semisimple family is strictly compatible away from the varying coefficient residue characteristic. Corollary 7.5 supplies (1.6) at a coefficient place with absolutely irreducible residual reduction. Theorem 7.6 supplies it uniformly for a cuspidal packet from $(\mathrm{HT}_1)$; Proposition 7.7 supplies the stronger optional route from $(\mathrm{SS}_{\mathrm{array}})$. At a bad finite-monodromy place, purity of weight one also needs the separate return-map weight hypothesis of Theorem 8.2; the constructed special pair is already pure by Section 8.2.
...
| Weil-number calculus and good-place purity of curve $H^1$                     | Book 37, _Weights and Weil Bounds for Curves and Abelian Varieties_                                               | smooth proper curve over a finite field; geometric Frobenius; correspondence summand                                                                                                                                          | it supplies no bad-place return-map weights                                                                                                                                                                                                                                                                                                                                                            |
| good-polynomial field, conjugate packets, and saturated eigenlattices         | Book 97, _Algebraicity and Integral Structures of Weight-Two Packets_                                             | one absolute $\mathbf Q$-algebraic conjugation-stable finite Hecke module; raw $T_v,S_v$ normalization; isolating data; intersection with a fixed integral module                                                             | a relative model gives only embeddings over its base field; the full packet center can exceed the field of good values; good eigenvalues can name only a near-equivalence block                                                                                                                                                                                                                        |
| eligible one-split packet                                                     | Book 104, _Global Jacquet--Langlands_                                                                             | $F$ totally real of degree $d\ge2$; $(d-1)+\lvert W\rvert$ even; every place in $W$ is selected discrete series                                                                                                               | in even degree $W$ must be odd and nonempty; in odd degree $W=\varnothing$ is allowed                                                                                                                                                                                                                                                                                                                  |
| curve realization and multiplicity                                            | Book 125, _Automorphic Decomposition of Shimura-Curve $H^1$_                                                      | actual one-split compact curve; full component-routing orbit; full finite Hecke image; separating component idempotent; multiplicity two                                                                                      | no surface or modular-curve substitute is supplied; the raw representation need not be semisimple                                                                                                                                                                                                                                                                                                      |
| raw rank-two representations                                                  | Book 126, _Galois Representations from Weight-Two Shimura-Curve Cohomology_                                       | full finite Hecke block, splitting field, common curve carrier, multiplicity two                                                                                                                                              | oldspace dimension is not Galois rank; absolute irreducibility is not supplied                                                                                                                                                                                                                                                                                                                         |
...
| uniform nonzero-monodromy raw-to-global passage for selected cuspidal packets | Theorem 7.6, using Book 6, Section 11.5, Book 47, Proposition 9.1, Book 109, Section 9.5, and Book 128, Lemma 9.1 | $(\mathrm{HT}_1)$ for one-dimensional geometric constituents at each coefficient place                                                                                                                                        | the geometric-subquotient passage is proved, and Book 6 converts a locally algebraic constituent into an algebraic Hecke character; the rank-one local-algebraicity theorem itself is not proved anywhere and is deferred to Book 24a, so $(\mathrm{HT}_1)$ requires that theorem together with smooth-proper $(\mathrm{HT}_{\mathrm{curve}})$ of Book 47, (9.7); no Faltings semisimplicity is needed |
...
Three load-bearing assertions remain logically separate. First, Book 122 does construct $(\mathrm B_v)$ and the raw special pair in its exact split one-step Iwahori range, whereas Book 128's tame principal, scalar-special, and dihedral rows merely recognize data that must already have been supplied. Second, Book 128 identifies the raw and globally semisimplified members formally when $N=0$ and, for nonzero monodromy, at a coefficient place with absolutely irreducible residual reduction. Theorem 7.6 proves all remaining formal reductions for a selected cuspidal packet. Book 6 proves the rank-one local-algebraicity step and Book 47 proves inheritance by curve subquotients, so the exact unsupplied premise is now only the smooth-proper comparison $(\mathrm{HT}_{\mathrm{curve}})$ of Book 47, (9.7), which supplies $(\mathrm{HT}_1)$. No Faltings-height or ambient Tate-semisimplicity theorem belongs to this route. Third, finite-monodromy return maps have not in general been proved to have the complex absolute values required for bad-place purity; the special row has no such gap because its shape and determinant force a finite-order kernel scalar.

For the FLT SP carrier problem, another dyadic special place supplies a compact basic curve
...
The boundary is exact. When the eligible carrier and complete split-Iwahori ledger are available, the special pair is geometrically constructed, and absolute residual irreducibility proves its passage to the semisimple member at the coefficient place where that residual condition holds. Theorem 7.6 proves passage at every allowed coefficient embedding from $(\mathrm{HT}_1)$. Its proof closes the class-field, rank-one avatar, geometric-subquotient, isobaric-recognition, absolute-irreducibility, and raw-to-global steps; what remains is $(\mathrm{HT}_1)$ itself, and supplying it needs both smooth-proper $(\mathrm{HT}_{\mathrm{curve}})$ and the rank-one Hodge--Tate algebraicity theorem deferred to Book 24a. Finite-monodromy bad purity still requires the return-map weight. Rational compatibility does not synchronize integral lattices, and coefficient-residue-characteristic comparisons require separate hypotheses. Subject to these boundaries, coefficient embeddings, polarization scalars, splitting fields, and eligible curve carriers alter presentations but not the semisimple good compatible data.
