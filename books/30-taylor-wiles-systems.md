...
by adjoining the fixed number of auxiliary eigenvalue and frame coordinates dictated by the chosen convention. Under the complete, effective, and relative Selmer-complex compatibility hypotheses of Book 25, there is a presentation
...
This is the relaxed-at-the-actual-base dual dimension of the enlarged problem. It is the Book 25 relative obstruction number for $\mathcal D_{Q_N}$ with the full local-base set $U_N=T\cup Q_N$. It is distinct from the old-base-only number
...
Choose an infinite subsequence on which the precision-one diagram, with its first named labels, is constant up to isomorphism. Inside it choose a further infinite subsequence on which the enhanced precision-two diagram and its reduction to precision one are constant. Continue, retaining the reduction identification at every stage, and take the diagonal subsequence. This finitely branching construction permits representatives $\mathscr D_d$ to be chosen so that reduction of $\mathscr D_{d+1}$ is identified with $\mathscr D_d$. It gives genuine transition maps
...
between the chosen finite models.
