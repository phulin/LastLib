...
Thus a division uniformizer $\Pi$ has $v_D(\Pi)=1$, whereas a scalar $z$ has $v_D(z)=2v(z)$. This is the convention of the quaternionic representation theory recalled from Book 82.
...
Every noncentral element of $D^\times$ generates a quadratic field when that field is separable. In residue characteristic two there can be inseparable quadratic subfields, so we retain the adjective regular semisimple and exclude those elements. This is exactly the boundary already isolated in Book 82.
...
For a quaternionic dihedral representation $\rho(E,\theta)=\mathrm{c\text{-}Ind}_{J_D}^{D^\times}\Lambda_D$, the finite-index character formula from Book 82 gives
...
Let $D_k$ be the full-group discrete series of Book 99, $k\ge2$, with trivial real-power twist for simplicity. It has elliptic character
...
Fix a nontrivial additive character $\psi$ of $F$. Book 80 normalizes its conductor by declaring the largest fractional ideal in its kernel to be $\mathfrak p^{-n(\psi)}$ and proves
