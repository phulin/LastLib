...
If $f=[l:k]$, then $F^f$ lies in inertia, so for a uniquely determined residue class
$a\pmod e$ one has
...
Thus the Galois group is generated by $F$ and $\tau$ with
$\tau^e=1$, $F\tau F^{-1}=\tau^q$, and $F^f=\tau^a$; compatibility also gives
$q^f\equiv1\pmod e$. The exponent $a$ depends on the chosen Frobenius lift. Geometric Frobenius replaces $F$ by $F^{-1}$ and changes the conjugation exponent accordingly. The ramification filtration itself remains
...
There is no inertia and hence no positive ramification. If $k$ is finite and the extension is
nontrivial, arithmetic Frobenius generates $G$ and does not lie in $G_0=1$; it is visible only
at level $-1$. For the trivial extension, the identity is the sole element of both $G_{-1}$ and
$G_0$. This placement is conceptually important: nontrivial Frobenius measures residue motion,
while the ramification filtration measures motion invisible in the residue field.
...
normal-form condition. Replacing $y$ by $y-z$ changes the right side by the Artin--Schreier
coboundary $z^p-z$; a term at a $p$-divisible exponent can be removed in this way only when its
coefficient has the required $p$th root (in particular, this works over a perfect coefficient
field).
...
perfect, Hasse--Arf therefore forces $p\mid b-a$, as proved in the next section.
...
The divisibility needed in the proof is Sen's congruence for the ramification numbers of
the powers of one cyclic automorphism. We prove it directly.

**Lemma 5.2A (Sen's cyclic congruence).** Let $E/F$ be a totally ramified cyclic extension
of degree $p^m$, where the common residue field has characteristic $p$, and let $\tau$ generate
$\operatorname{Gal}(E/F)$. For a uniformizer $\pi$ and an integer $k$ not divisible by $p^m$ put

$$
c_k=v_E\!\left(\frac{\tau^k(\pi)}{\pi}-1\right).
$$

Then $c_k$ depends only on $v_p(k)$. Moreover, if $k_1,k_2$ are not divisible by $p^m$ and
$v_p(k_1-k_2)\geq a$, where $0\leq a<m$, then

$$
c_{k_1}\equiv c_{k_2}\pmod {p^{a+1}}.                 \tag{5.3a}
$$

**Proof.** If $k=ip^j$ with $p\nmid i$, put $\rho=\tau^{p^j}$. The identity

$$
\rho^i-1=(\rho-1)(1+\rho+\cdots+\rho^{i-1})
$$

shows on the first nonzero graded piece of the valuation filtration that
$v_E(\rho^i(\pi)-\pi)=v_E(\rho(\pi)-\pi)$: the second factor acts there as multiplication
by the nonzero residue of $i$. Thus $c_k=c_{p^j}$. The same factorization with $i=p$
shows $c_{p^{j+1}}\geq c_{p^j}$, because the $p$ leading residues sum to zero in
characteristic $p$.

We need one way to realize the same number on an element of any prescribed valuation. For
$k>0$ set

$$
\alpha_k=\prod_{i=0}^{k-1}\tau^i(\pi).
$$

Then $v_E(\alpha_k)=k$ and

$$
\frac{\tau(\alpha_k)}{\alpha_k}
=\frac{\tau^k(\pi)}{\pi},
\qquad
v_E(\tau(\alpha_k)-\alpha_k)=k+c_k.          \tag{5.3b}
$$

For negative $k$ use $\alpha_k=\alpha_{-k}^{-1}$. Multiplying $\alpha_k$ by a unit of
$F$ changes its leading residue arbitrarily and preserves both equalities, because the residue
extensions agree.

It is enough to prove successively

$$
p^n\mid c_{p^{n-1}}-c_{p^n}\qquad(1\leq n<m),          \tag{5.3c}
$$

for the first paragraph and the triangle relation then give (5.3a). We argue by induction on
$n$. Assume (5.3c) below $n$ and suppose, for a contradiction, that it fails at $n$. Put

$$
d=c_{p^{n-1}}-c_{p^n}.
$$

The induction hypothesis applied to $\tau^p$ shows that $p^{n-1}\mid d$; failure at $n$
therefore says $v_p(d)=n-1$. Formula (5.3b), now for the automorphism $\tau^p$ and an element
$\alpha$ of valuation $d$, gives

$$
v_E(\tau^p(\alpha)-\alpha)
=d+c_d(\tau^p)=d+c_{pd}(\tau)=d+c_{p^n}=c_{p^{n-1}},   \tag{5.3d}
$$

where the first paragraph applies because $v_p(pd)=n$.

Let

$$
\beta=\alpha+\tau(\alpha)+\cdots+\tau^{p-1}(\alpha).
$$
...
$$
v_E(\tau(\beta)-\beta)=v_E(\tau^p(\alpha)-\alpha)=c_{p^{n-1}}.       \tag{5.3e}
$$

Completeness and the last sentence following (5.3b) give an expansion
$\beta=\sum_{k\geq v_E(\beta)}\beta_k$, convergent in $E$, in which either $\beta_k=0$
or $v_E(\beta_k)=k$ and, unless $p^m\mid k$,

$$
v_E(\tau(\beta_k)-\beta_k)=k+c_k.                    \tag{5.3f}
$$

Construct it recursively by matching the leading residue of the current remainder with a
suitable $F$-unit multiple of $\alpha_k$; the valuation of the remainder strictly increases,
so completeness gives the sum. Terms with $p^m\mid k$ are fixed by $\tau$ and disappear from
$\tau(\beta)-\beta$.

The numbers $k+c_k$ with $v_p(k)<n$ are pairwise distinct. If two $k$ have the same
$p$-adic valuation, their $c_k$ agree and the $k$ differ. If their valuations differ, equality
would give

$$
v_p(k_1-k_2)=v_p(c_{k_2}-c_{k_1})
\geq v_p(k_1-k_2)+1
$$

by the induction hypothesis, an impossibility. None equals $c_{p^{n-1}}$: when
$v_p(k)=n-1$ one has $c_k=c_{p^{n-1}}$ and $k\ne0$, while for $v_p(k)<n-1$ the same
valuation comparison gives a contradiction.

Consequently terms with $v_p(k)<n$ cannot cancel to produce the valuation in (5.3e), and the
terms with $v_p(k)\geq n$ must do so. But every such term satisfies

$$
k+c_k\geq v_E(\beta)+c_{p^n}>d+c_{p^n}=c_{p^{n-1}},
$$

contradicting (5.3e). This proves (5.3c) and the lemma. $\square$

**Theorem 5.2 (Hasse--Arf).** Assume that $k$ is perfect. If
...
**Lemma 5.3 (cyclic Hasse--Arf lemma).** Let $E/F$ be a
...
**Proof.** The summands are integers, but divisibility by $|C|$ is not termwise. If $C_1=1$,
then $b=0$ and the sum is empty. Assume
...
Apply Lemma 5.2A to the totally ramified cyclic extension $E/E^P$ and its generator
$\sigma$. Since

$$
c_{p^r}=v_E\!\left(\frac{\sigma^{p^r}(\pi_E)}{\pi_E}-1\right)=q_r,
$$

the lemma with $k_1=p^r$, $k_2=p^{r-1}$, and $a=r-1$ gives

$$
q_r\equiv q_{r-1}\pmod {p^r}\qquad(1\leq r<s).       \tag{5.4}
$$
...
calculation will be used in Chapter 11, together with the finite-group character argument proved
there, to prove integrality of the total Swan conductor even when individual breaks of a
...
An abelian representation over a perfect residue field has integral breaks by Hasse--Arf. A
general representation can have rational breaks, yet its total Swan conductor is an integer by
the integral character argument in Chapter 11. That stronger fact follows from Artin's character
theorem rather than from breakwise integrality. With only separability of one chosen residue
extension, the decomposition and the rational conductor formula remain valid, but this book does
not claim the same integrality theorem.
...
For the extension of Section 3.3---namely $K=k((t))$ with $\operatorname{char}k=p>0$,
$m>0$, $p\nmid m$, and $L/K$ the nontrivial degree-$p$ extension defined by
$y^p-y=t^{-m}$---the displayed equation is not an integral monogenic presentation because $y$
has negative valuation. Hilbert's formula is therefore more natural than differentiating that
equation: it immediately gives $(m+1)(p-1)$. A change to an integral generator would recover the
same derivative exponent but obscure the transparent Galois action.
...
- A one-dimensional tamely ramified character that is nontrivial on inertia has conductor $1$;
  a nontrivial unramified character still has conductor $0$.
...
The last statement, together with the integrality of character conductors proved in
...
characters induced from cyclic subgroups is not enough: denominators in such a formula would
survive in the conductor pairing. We therefore prove the integral statement before applying it.

Write $R(G)$ for the free abelian group on the irreducible complex characters of a finite group
$G$. A group is **$p$-elementary** if it is a direct product $C\times P$, with $C$ cyclic of
order prime to $p$ and $P$ a $p$-group.

**Proposition 11.1B (integral monomial induction).** For every $\chi\in R(G)$ there are
subgroups $H_j\leq G$, one-dimensional characters $\lambda_j$ of $H_j$, and integers $n_j$
such that
...
**Proof.** We first prove induction from arbitrary characters of elementary subgroups. Fix a
prime $p$, write $|G|=p^rm$ with $(m,p)=1$, and let $V_p\subseteq R(G)$ be generated by
$\operatorname{Ind}_E^G R(E)$ for all $p$-elementary $E$. The tensor identity and the Mackey
double-coset formula show that $V_p$ is an ideal of $R(G)$: intersections of conjugates of
$p$-elementary groups are again $p$-elementary.

Let $n=|G|$, let $\zeta$ be a primitive $n$-th root of unity, and put
$\mathcal O=\mathbf Z[\zeta]$. Character values lie in $\mathcal O$. Since
$\mathcal O\cap\mathbf Q=\mathbf Z$, a basis of $\mathcal O$ may be chosen beginning with
$1$; consequently

$$
(\mathcal O\otimes V_p)\cap R(G)=V_p.
\tag{11.1b}
$$

For a cyclic subgroup $C$, define $\vartheta_C(x)=|C|$ when $x$ generates $C$ and $0$
otherwise. Summing induced functions and evaluating at $g\in G$ counts each conjugate of $g$
once, hence
...
Induction on $|C|$, applied to the same identity inside $C$, proves
$\vartheta_C\in R(C)$: isolate the term $C$ and use the already known proper subgroups. It
follows in particular that an integer-valued class function whose values are divisible by $n$
lies in $\mathcal O\otimes V_p$. Indeed, write it as $nf_0$, multiply (11.1c) pointwise by
$f_0$, and use

$$
f_0\operatorname{Ind}_C^G\vartheta_C
=\operatorname{Ind}_C^G(\operatorname{Res}_C f_0\,\vartheta_C).
$$

The factor on the right has values divisible by $|C|$; its inner products with the linear
characters of $C$ therefore lie in $\mathcal O$, so it belongs to
$\mathcal O\otimes R(C)$. A cyclic group is $p$-elementary after separating its $p$-part, so
the assertion follows.

We now construct an element of $\mathcal O\otimes V_p$ which is everywhere prime to $p$.
Every $g$ has a unique commuting decomposition $g=g_{p'}g_p$ into its prime-to-$p$ and
$p$-power parts; both are powers of $g$. If an integer-valued element
$\eta\in\mathcal O\otimes R(G)$ is restricted to $\langle g\rangle$, write it as an
$\mathcal O$-linear sum of linear characters and let $q$ be the order of $g_p$. Each linear
character has the same $q$-th power at $g$ and $g_{p'}$; the freshman's-dream congruence in
$\mathcal O/p\mathcal O$, followed by $a^q\equiv a\pmod p$ for the two integer values, gives

$$
\eta(g)\equiv\eta(g_{p'})\pmod p.
\tag{11.1d}
$$

Choose representatives $x_i$ of the conjugacy classes of $p$-regular elements. Put
$C_i=\langle x_i\rangle$ and choose a Sylow $p$-subgroup $P_i$ of $C_G(x_i)$. On
$E_i=C_i\times P_i$, take the integer-valued function which is $|C_i|$ on
$\{x_i\}\times P_i$ and zero off that subset. The preceding cyclic inner-product calculation
puts it in $\mathcal O\otimes R(E_i)$. Its induction $\eta_i$ to $G$ vanishes on every
$p$-regular class except the class of $x_i$, while

$$
\eta_i(x_i)=|C_G(x_i)|/|P_i|\not\equiv0\pmod p.
$$

After multiplying the $\eta_i$ by suitable integers modulo $p$ and summing, we obtain
$\eta\in\mathcal O\otimes V_p$, integer-valued on $G$, with
$\eta(x_i)\equiv1\pmod p$ for every $i$. Formula (11.1d) then gives
$\eta(g)\equiv1\pmod p$ for every $g$.

Let $N=\varphi(p^r)$. Euler's theorem gives $\eta(g)^N\equiv1\pmod {p^r}$ for every $g$.
Therefore $m(\eta^N-1)$ is integer-valued and divisible pointwise by $p^rm=n$, so the preceding
divisibility argument puts it in $\mathcal O\otimes V_p$. The ideal property also puts
$m\eta^N$ there, and subtraction gives $m\mathbf1_G\in\mathcal O\otimes V_p$. By (11.1b),
...
Since $V_p$ is an ideal, $mR(G)\subseteq V_p$. Summing $V_p$ over the primes dividing $n$
therefore gives all of $R(G)$: the integers $n/p^{v_p(n)}$ have greatest common divisor $1$.
Thus every character is an integral sum of characters induced from elementary subgroups.

It remains to make the inducing characters one-dimensional. If $P$ is a $p$-group and $W$ an
irreducible representation, first quotient by its kernel; a monomial expression in the quotient
pulls back to one in $P$. If the resulting faithful $P$ is nonabelian, choose a nontrivial cyclic
subgroup of order $p$ in $Z(P/Z(P))$ and let $A$ be its
inverse image. Then $A$ is abelian and normal and properly contains $Z(P)$. The isotypic
decomposition of $W|_A$ has more than one summand: otherwise $A$ would act by scalars and,
by faithfulness, would lie in $Z(P)$. The group $P$ permutes these summands transitively.
If $D<P$ stabilizes one summand $W_0$, irreducibility of $W$ makes $W_0$ irreducible as a
$D$-representation, and the map

$$
\operatorname{Ind}_D^P W_0\longrightarrow W,
\qquad g\otimes w\longmapsto gw,
$$

is surjective and has equal-dimensional source and target, hence is an isomorphism. Induction on
$|P|$ and transitivity of induction show that $W$ is induced from a linear character. For
$E=C\times P$, irreducibles are tensor products of a linear character of $C$ with an
irreducible of $P$, so the same conclusion holds. Replacing every elementary inducing character
above by its monomial expression proves (11.1a) with integers and one-dimensional
$\lambda_j$. $\square$

**Theorem 11.1 (integrality).** Under the
...
We now prove that every irreducible coefficient of $A_G$ is integral. Proposition 11.1B writes the
...
complex setting of Proposition 11.1B. This proves the result for arbitrary characteristic-zero
coefficients. In coefficient characteristic dividing $|G|$, one must instead use an appropriate
modular theory. Perfectness of the residue field is the standing field hypothesis for
Hasse--Arf; Lemma 5.2A proves the ramification-number congruence, while Proposition 11.1B makes
the passage from cyclic characters integral.
...
For a wild cyclic group of order $p$ with unique lower break $m$, let $\chi$ be a nontrivial one-dimensional character and let $V_\chi$ be the one-dimensional representation it affords. Then $V_\chi^{G_i}=0$ for $0\leq i\leq m$ and equals $V_\chi$ afterward. Since $G_i=G_0$ throughout that interval,
...
- Hasse--Arf and one-dimensional conductor integrality hold here under the stronger hypothesis
  that $k$ is perfect. Lemma 5.2A supplies the cyclic congruence, and Proposition 11.1B supplies
  the integral character reduction needed for Artin and Swan conductors of arbitrary
  characteristic-zero representations. The rational fixed-space formulas still make sense under
  the preceding separable-residue hypothesis.
