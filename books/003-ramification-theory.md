...
The divisibility needed in the proof is a statement about all the iterates of one automorphism.
It is important not to replace those iterates by unrelated deformations of their fixed schemes.
The following filtration calculation keeps the single cyclic action throughout.

**Lemma 5.2A (cyclic fixed-ideal congruence).** Let $R$ be a complete DVR with residue field
$k$ of characteristic $p>0$, and let $u$ be a continuous local automorphism inducing the
identity both on $k$ and on $\mathfrak m/\mathfrak m^2$. Put

$$
J_d=(u^d(x)-x:x\in R).
$$

If $u^{p^r}\ne1$, then every $J_{p^j}$ for $0\leq j\leq r$ is a nonzero principal ideal and

$$
p^r\ \bigm|\
\operatorname{length}_R(R/J_{p^r})-
\operatorname{length}_R(R/J_{p^{r-1}}).
\tag{5.3a}
$$

**Proof.** Write $\Delta=u-1$ as an additive operator. Since $u$ is the identity on the residue
field and on the cotangent line,
$\Delta(\mathfrak m^a)\subseteq\mathfrak m^{a+1}$ for $a\geq0$. If $w$ is any nonidentity
power of $u$, the ideal generated by the values of $w-1$ is therefore
$\mathfrak m^{c(w)}$, where

$$
c(w)=\min_{x\in R}v((w-1)x).
$$

Thus all the asserted fixed ideals are nonzero and principal, and their colengths are the
integers $c(u^{p^j})$.

We record the filtered-operator calculation which controls those integers. If $w$ is
residue-trivial, $w^{p^a}\ne1$, and $c_j=c(w^{p^j})$, then

$$
c_j-c_{j-1}\in p^j\mathbf Z\qquad(1\leq j\leq a).
\tag{5.3b}
$$

Indeed, in the ring of additive endomorphisms,

$$
w^{p^j}-1=(1+(w-1))^{p^j}-1
=\sum_{h=1}^{p^j}\binom{p^j}{h}(w-1)^h.
\tag{5.3c}
$$

For $h=p^tq$ with $p\nmid q$, cancellation in
$\binom{p^j}{h}=(p^j/h)\binom{p^j-1}{h-1}$ gives
...
because $\binom{p^j-1}{h-1}\equiv(-1)^{h-1}\pmod p$. Filter (5.3c) by the least valuation
of the values of an operator and pass successively to the first nonzero associated-graded map.
The terms with $v_p(h)=t$ form packets permuted by $w^{p^t}$. Every packet before the last
finite difference has $p^{j-t}$ identical residues and hence cancels; iterating through
$t=0,\ldots,j-1$ shows that the first surviving degree changes from that of
$w^{p^{j-1}}-1$ by

$$
\sum_{t=0}^{j-1}p^j n_{j,t}
$$

for integers $n_{j,t}\geq0$. Taking an element on which the first surviving graded map is
nonzero identifies these two degrees with $c_j$ and $c_{j-1}$. This proves (5.3b). The point
of using (5.3c) is that every packet comes from an iterate of the one operator $w$; no
independent deformation of fixed schemes has entered.

Apply (5.3b) with $w=u$ and $j=r$. Since
$\operatorname{length}(R/\mathfrak m^c)=c$, the resulting congruence is precisely (5.3a).
$\square$

The same packet calculation gives the familiar Moebius form whenever all the relevant fixed
ideals have finite colength:

$$
n\mid\sum_{d\mid n}\mu(n/d)\operatorname{length}_R(R/J_d).
$$

Prime by prime, the sum collects packets of exact period $d$, and each such packet has total
multiplicity divisible by $d$. Only the proved prime-power case (5.3a) is needed below.
...
$\operatorname{Frac}(A)$ which preserves $A$, induces the identity on $\kappa$ and on
$\mathfrak m_A/\mathfrak m_A^2$, and for which
...
We derive the congruence from Lemma 5.2A. This fixed-point formulation is important in
...
Apply Lemma 5.2A to $R=A$, $u=\theta$, and the exponent $r$. The automorphism $\theta$ is
local and continuous because it preserves the valuation filtration, and the two trivial-action
hypotheses are part of the ramification-number statement. Every divisor of $p^r$ is
...
the lemma have now been checked. Formula (5.3a) gives
