...
The purpose of this chapter is to keep those arrows separate. The geometric construction of Book 59 and the arithmetic existence theorem of Book 58 will provide a field and an abelian variety. Books 45, 47, and 48 control the known automorphic side under restriction. Book 56 supplies the two lifting steps. Book 54 is the indispensable bridge between an automorphic representation and its different coefficient-adic realizations. None of these inputs can replace another.
...
- every away-from-$\ell$ local component of the target belongs to the controlled nonminimal range of Book 56;
- the two-prime realization datum of Book 59 has genuine local points on one smooth geometrically connected fine component; and
- the integral automorphic infrastructure required by the two applications of Book 56 is available over the fields on which it is invoked.
...
3. for each real place $\tau$ and a corresponding complex conjugation $c_\tau$,
   ...
   $$
4. for every $v\mid\ell$, the lattice $\mathcal O^2$ is finite flat of type $(0,1)$ in the integral sense used in Book 56; and
...
Fix $v\mid\ell$. The clean local theorem in Book 56 assumes $F_v/\mathbf Q_\ell$ unramified and $\ell>2$. We therefore assume $\ell$ unramified in $F$ and later force $v$ to split completely in the potential-modularity field. This keeps the local field literally unchanged:
...
At $v\nmid\ell$, the target is allowed the controlled conditions established in Book 56. Typical cases are:
...
4. A finite set of away-from-$\ell$ local prescriptions, each represented by a named component in the controlled range of Book 56.
...
The last clause is not circular. It does not assume the target is automorphic. It assumes the independent level-raising, level-lowering, type-lattice, local--global compatibility, determinant-line, and component-support theorems on which Book 56 is conditional. An application must identify which version supplies them.
...
There is no virtue in choosing $q$ small. In the clean theorem one chooses it outside the finite exceptional sets coming from the target, the auxiliary character, coefficient fields, local type denominators, and residual image. If a proposed $q$ fails its finite-flat local calculation, one changes $q$ before constructing the moduli space.

The order of choices prevents circularity. First choose the CM extension $K/F$ and a characteristic-zero algebraic Hecke character with the desired determinant and with prescribed local behavior away from its future coefficient prime. Next exclude the finite set of rational primes at which its two conjugates become equal after reduction, its stable lattice is not well behaved, a required type denominator is nonunit, or the intended residue field cannot be embedded in the coefficient datum. Choose $q$ outside that set with the desired splitting in the coefficient fields. Only then inspect the local avatars at places above $q$ and construct the finite-flat comparison objects.

If that final local inspection fails, return to the choice of character or $q$. One may not alter the $q$-adic local characters after reducing them while pretending to retain the same global Hecke character. Once $q$, $\bar\sigma$, its pairing, and all local $q$-models are fixed, choose the totally real coefficient field $M$ and the prime $\mathfrak q$. The moduli space comes last.
...
A robust choice is dihedral. Let $K/F$ be a totally imaginary quadratic extension and let $\Theta$ be an algebraic Hecke character of $K$ whose infinity type at every CM pair is $(1,0)$ in the normalization of Book 45, up to the common determinant normalization, so that its automorphic induction has parallel weight two and cyclotomic Galois determinant. Choose a $q$-adic coefficient field $E_{\Theta,q}$ whose ring of integers $\mathcal O_{\Theta,q}$ has residue field $k_q^{\mathrm{in}}$, enlarging the coefficient field before fixing the input if necessary. Let
...
Fix a fractional polarization ideal $\mathfrak c$, a positivity orientation, and a neat auxiliary level $\mathfrak n$, all prime to $\ell q$ and to the relevant different. A point of the untwisted PEL space is a tuple
...
Add the paired frames (4.2). After splitting both residual modules, this is a finite etale frame cover of the fine PEL space. Twisting by the product cocycle produces
...
The fine-space theorem of Book 59 applies only after all of the preceding PEL and level data have been fixed. It gives a quasi-projective scheme with a universal tuple. Smoothness is checked on the characteristic-zero generic fiber by deforming the Hodge filtration. After splitting $M$, one has
...
Let $Y^\circ$ be the selected untwisted PEL component. Its simultaneous paired frame cover has deck group
...
| PEL or level-bad $v$ | point on the actual generic moduli component | every rigidification and a verified analytic neighborhood |
...
These equalities use the selected generator of the PEL polarization line. If a nontrivial coefficient or polarization character is retained, it must occur identically in the target pairings, the automorphic central character, and both deformation problems.
...
of cyclic extensions to which Books 47--48 apply. Iterated cyclic base change gives an isobaric representation
...
Suppose Moret--Bailly produces a field whose normal closure has nonsolvable Galois group. Books 47--48 do not define a general base change of $\pi_0$ to that field. The statement
...
$H^0(F',\operatorname{ad}^0\bar r_{A,q})=0$ by Schur's lemma and the trace splitting. Thus both vanishing clauses in the standing Book 56 ledger have been checked.
...
NM-I supplies the paired Taylor--Wiles modules and exact augmentation; NM-III identifies determinant lines and proves that patched support contains the component through $x_A$. The applicable full, reduced, or componentwise $R=T$ theorem therefore makes $x_A$ factor through the Hecke side. If only reduced equality is known, the factorization still holds because the target coefficient ring is a domain and kills nilpotents.
...
The central character is trivial in the cyclotomic-determinant normalization. At every finite place, the local component of $\Pi_A$ has the conductor, inertial type, ordered branch, and monodromy prescribed by the actual local representation of $A$ within the range of NM-II.
...
Book 54 attaches to $\Pi_A$, at a place $\widetilde\lambda\mid\ell$ of its coefficient field, a continuous representation
...
Hodge--Tate weights alone would not verify this clause. The finite-flat deformation ring in Book 56 classifies integral torsion systems, and (11.2) is checked against that ring through the supplied models.
...
6. NM-I--NM-III and the curve-realization input are available over each field where they are invoked.
...
be the cyclic tower. The top representation $\Pi_E$ is invariant because its Galois representation is the restriction of one defined over $L$. Cyclic descent gives at least one cusp form $\pi_{s-1}$ over $L_{s-1}$. Its Galois representation and $\rho|_{G_{L_{s-1}}}$ have isomorphic irreducible restrictions to $G_E$.
...
The preceding descent applies to $E/L$ because that extension was deliberately chosen solvable Galois and because $\rho|_{G_L}$ supplies compatible extension data. The Moret--Bailly extension $L/F$ is usually neither Galois nor solvable. Books 47--48 therefore do not provide a route from $L$ to $F$.
...
**Proof.** Clauses 5--7 and the theorem of Book 59 give the fine variety and all nonempty local opens. Apply the simultaneous totally real, disjoint theorem of Book 58 using $D$. This produces $F'$ and a point $x\in X(F')$ satisfying Sections 6.2--6.3. Pulling back the universal family gives $A$ and (13.2). Lemma 6.1 and clause 8 preserve both residual images.

Clause 10 gives a cuspidal residual automorphic source for $A[\mathfrak q]$. The local integral models, clause 9, image preservation, and clause 11 verify every input of the first controlled nonminimal lifting theorem, which gives (9.7). The reduced Frobenius polynomial of $A$ is independent of its coefficient prime, as in (10.1). Comparing it with the Hecke polynomial of $\Pi_A$, constructing $\rho_{\Pi_A,\widetilde\lambda}$, and applying Chebotarev yields (13.3). Reduction gives (13.4). $\square$
...
2. the residual automorphic system (13.4) can be lowered and raised to the precise target datum by the independent integral theorems of Book 56;
...
The theorem is conditional exactly where Book 56 is conditional: the integral Hilbert or quaternionic level/type control, local Hecke-valued representation, determinant-line comparison, and component support must be established for the selected fields and components.

The logical provenance can be summarized without blurring it:

| Part of the argument | Status in this book |
|---|---|
| fine twisted elliptic or HBAV space, smoothness, product monodromy | imported from Book 59 after its hypotheses are verified here |
| totally real split point with disjoint residue field | imported from Book 58 after the local-open ledger is verified here |
| dihedral automorphic induction | imported from Book 45 and reapplied over $KF'/F'$ |
| cyclic and solvable base change or descent | imported from Books 47--48 only on the stated towers |
| automorphic Galois representations at each coefficient prime | imported from Book 54 in its curve-realizable range |
| the two nonminimal lifting steps | imported from Book 56 with NM-I--NM-III and component support |
...
**Proof.** Clauses 1--4 are the simultaneous split, real, and disjoint conclusions of Book 58 applied in Section 6. Clauses 5--6 follow from Lemma 6.1 and from membership of every conjugate local point in $\Omega_v$. The negative statements are not limitations of the proof's exposition: they are absent from the arithmetic existence theorem. $\square$
...
**Corollary 13.4 (elliptic-curve variant).** Suppose $C/F$ admits a prime $\ell$ for which $C[\ell]$ is absolutely irreducible, adequate, totally odd, and satisfies the global adjoint and local Schur hypotheses. Suppose its local representations have the controlled types just described, and suppose there is a lifting-admissible two-prime realization datum for $C[\ell]$. Then there exist a finite totally real field $F^{\mathrm{pm}}/F$, a parallel-weight-two cuspidal representation $\Pi$ over $F^{\mathrm{pm}}$, a coefficient place $\widetilde\lambda\mid\ell$ of $\Pi$, and a finite extension $C_\ell/\mathbf Q_\ell$ receiving $E_{\Pi,\widetilde\lambda}$ such that
...
If (13.8) has no solution, one needs a PEL moduli problem whose multiplier line carries the exact character $\bar\psi\bar\chi_\ell$. Existence and local solvability of such a moduli problem are additional hypotheses; an arbitrary quadratic twist cannot repair the determinant because it changes a rank-two determinant by a square.
...
Suppose the residue field $F'$ of the moduli point has normal closure with Galois group $S_5$. This is compatible with Moret--Bailly's theorem. There is no solvable tower to which Books 47--48 can be applied, so an arbitrary modular residual representation over $F$ is not known from those books to remain modular over $F'$.
...
at all good places. Book 61 can organize the representations attached to $\Pi_\rho$ at its different coefficient primes into a compatible system. The input from this book is:
...
Book 63 may choose a more convenient coefficient prime in the compatible system. It receives the common polynomials (16.1), but it must freshly verify:
...
For later arithmetic applications, and in particular for the modular-method synthesis of Book 71, export the following checklist.
