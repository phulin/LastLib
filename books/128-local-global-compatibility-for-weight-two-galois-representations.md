...
9. [Raw cohomology and global semisimplification](#9-raw-cohomology-and-global-semisimplification)
   ...
   - [9.4 Reduction to semisimplicity of the ambient curve cohomology](#94-reduction-to-semisimplicity-of-the-ambient-curve-cohomology)
...
### 9.4 Reduction to semisimplicity of the ambient curve cohomology

There is a uniform route to $(\mathrm R_u)$ which is independent of residual
irreducibility. It must be stated at the ambient cohomological level, because that is where the
one genuinely new theorem is needed.

Let $C/E$ be the actual smooth projective curve carrier at one coefficient characteristic
$\ell$. After the selected center embedding and finite splitting field have been fixed, write
$H_{\Pi,\lambda}$ for the selected full-Hecke block in
$H^1_{\mathrm{et}}(C_{\overline E},L_\lambda)$. Book 126, Theorem 3.1 gives a
$G_E$-equivariant evaluation isomorphism

$$
P_{\Pi,\lambda}\otimes_{L_\lambda}
W^{\mathrm{raw}}_{\Pi,\lambda}
\xrightarrow{\sim}H_{\Pi,\lambda},
\tag{9.4}
$$

where $G_E$ acts trivially on the finite-dimensional Hecke module
$P_{\Pi,\lambda}$.

**Proposition 9.4 (ambient-semisimplicity reduction).** Suppose
$H^1_{\mathrm{et}}(C_{\overline E},\mathbf Q_\ell)$ is semisimple as a
$\mathbf Q_\ell[G_E]$-module. Then every raw multiplicity representation extracted from
$C$ after finite coefficient extension is globally semisimple. Consequently
...
and $(\mathrm R_u)$ holds simultaneously for every $u\nmid\ell$ and every coefficient
embedding displayed from that carrier in residue characteristic $\ell$.

**Proof.** Semisimplicity survives finite scalar extension in characteristic zero. Indeed,
the finite-dimensional algebra generated by $G_E$ on a semisimple faithful module has zero
radical, and a finite-dimensional semisimple algebra over the perfect field
$\mathbf Q_\ell$ remains semisimple after finite scalar extension. The rational central
packet idempotent and the selected split central idempotent commute with $G_E$, so
$H_{\Pi,\lambda}$ is a direct summand of that scalar extension and is semisimple.

Put $d=\dim P_{\Pi,\lambda}$. After choosing a vector-space basis of the first factor,
(9.4) is an isomorphism of $G_E$-modules

$$
\left(W^{\mathrm{raw}}_{\Pi,\lambda}\right)^{\oplus d}
\simeq H_{\Pi,\lambda}.
$$

The raw multiplicity space is a direct summand of the left side, hence is semisimple. It is
therefore isomorphic to its global semisimplification, proving (9.5). Restriction to a
decomposition group and formation of the Weil--Deligne pair preserve this global isomorphism,
so $(\mathrm R_u)$ follows at every $u\nmid\ell$. No local monodromy assertion was used to
prove semisimplicity. $\square$

Global semisimplicity here does not say that the restriction to a decomposition group is a
semisimple local representation. A globally irreducible representation may have unipotent
inertia and nonzero $N$. What matters is that the global raw representation itself, rather than
only its semisimplification, is the member being restricted; its local extension and monodromy
are therefore not discarded.

For a curve, Book 36, (10.12) identifies the ambient representation with
$V_\ell(\operatorname{Jac}C)(-1)$ in the covariant convention, equivalently with the dual
through the principal polarization. Twists and duals preserve semisimplicity. Thus the
following exact theorem is sufficient for the hypothesis of Proposition 9.4:
...
embedding above $\ell$. A display in a larger algebraically closed coefficient field is a
further scalar extension of that finite model, and separability gives the same conclusion. It
also applies when $\ell=2$ as a statement about the global raw
representation. It does **not** create a dyadic Weil--Deligne comparison when the base place
$u\mid2$ is also a coefficient-prime place. The prime-to-coefficient special comparison still
has the necessary range $u\nmid\ell$.
