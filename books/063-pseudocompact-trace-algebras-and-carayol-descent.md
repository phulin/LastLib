...
2. If $m_1,\ldots,m_s$ span $M/\overline{\mathfrak m_BM}$, then the closed submodule generated by the $m_i$ is all of $M$.
...
For the second assertion, let $N$ be the closed submodule generated by the $m_i$. The quotient $M/N$ is pseudocompact, and the spanning hypothesis gives

$$
M/N=\overline{\mathfrak m_B(M/N)}.
$$

The first part makes $M/N=0$. $\square$

The conclusion is topological generation. It becomes algebraic generation when the image of a finite free module is known to be closed. For finite modules over a complete Noetherian local ring, that closed-image property follows from the induced adic topology. Before Noetherianity is known, silently replacing “topologically generates” by “generates” is invalid.
...
Let $B$ be pro-Artinian and let

$$
f:B^m\longrightarrow B^n
$$

be $B$-linear. Its image need not be closed for an arbitrary matrix $f$. The special maps in the descent argument come with a finite-level inverse or a unit determinant. If $m=n$ and $\det f\in B^\times$, the adjugate formula gives a continuous $B$-linear inverse, so $f$ is a homeomorphism. More generally, if $f$ admits a continuous $B$-linear retraction, then its image is the kernel of the continuous complementary projection and is therefore closed.

The maps used below fall into these safe classes. A basis with invertible reduced determinant gives a homeomorphism of finite free modules. An idempotent endomorphism has image equal to the kernel of its complementary idempotent. An isomorphism detected modulo the maximal ideal has unit determinant and is strict.

This observation prevents a circularity. We do not infer closedness merely from finite generation over a nonnoetherian ring. We prove it from an explicit inverse, a direct-summand decomposition, or a unit determinant. Once a closed trace ring happens to be Noetherian, the broader finite-module closedness theorem becomes available, but the core descent proof does not depend on reaching that stage.
...
is scalar, say $aI$. Express $I$ and $z$ in the $A$-basis $x_i$. Since the coordinates of $z$ lie in $B$, comparison with $aI$ shows that $a$ is constrained by those coordinates; more conceptually, the Azumaya isomorphism identifies central endomorphisms with scalar ones. Thus

$$
Z(E)=B.
$$

The centralizer of $E$ in $M_d(A)$ is $A$. Indeed $E$ contains $\rho(G)$, and Section 5.5 computed the centralizer of that group as the scalar matrices. This scalar-centralizer statement will make conjugating matrices unique modulo $A^\times$.
