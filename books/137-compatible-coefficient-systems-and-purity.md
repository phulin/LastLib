...
6. [Purity at the good places](#6-purity-at-the-good-places)
   ...
   - [Purity after surface cancellation](#63-purity-after-surface-cancellation)
   - [Consequences for Hecke eigenvalues](#64-consequences-for-hecke-eigenvalues)
...
12. [The surface-cancellation boundary](#12-the-surface-cancellation-boundary)
    - [The honest surface system](#121-the-honest-surface-system)
    - [Global cancellation](#122-global-cancellation)
    - [Local cancellation](#123-local-cancellation)
    - [Failures prevented by the boundary](#124-failures-prevented-by-the-boundary)
...
Third, the low-dimensional weight theorem proves that smooth proper curve $H^1$ is pure of weight one, while smooth proper surface $H^2$ is pure of weight two. Purity passes to correspondence summands. A rank-two factor removed from a surface is weight one only when a genuine weight-one cancellation factor has been identified.
...
9. Keep the four-dimensional surface block separate unless global and local cancellation have both been supplied.
...
**Proposition 2.1 (finite common field).** There is a finite extension $E/E_0$ over which every simple packet factor, every finite inertial type in the stated local range, and every supplied cancellation algebra splits. The field $E$ may be chosen normal over $\mathbf Q$, so every embedding of $E_0$ extends to an automorphism of $E$ and all coefficient conjugates occur inside it.
...
For a surface carrier, (4.1) is replaced by the cancellation Hom only under the hypotheses of Chapter 12. Without those hypotheses the family has the dimension of the full surface packet and is not the rank-two family discussed in Chapters 4--11.
...
### 6.3 Purity after surface cancellation

A smooth proper surface has pure $H^2$ of weight two. Its full packet block therefore has roots of modulus $q_v$. If a descended cancellation datum gives

$$
V^{\mathrm{surf}}_{\Pi,\lambda}
\simeq R_{\Pi,\lambda}\otimes
W^{\mathrm{surf}}_{\Pi,\lambda}, \tag{6.2}
$$

and $R_{\Pi,\lambda}$ is pure of weight one, then $W^{\mathrm{surf}}_{\Pi,\lambda}$ must also be pure of weight one, provided the tensor factors are individually defined with algebraic Frobenius polynomials.

To justify the last statement, let the roots of Frobenius on $R$ be $r_1,r_2$ and those on $W$ be $w_1,w_2$. Every $r_iw_j$ has modulus $q_v$. Since $R$ is pure of weight one, $|r_i|=q_v^{1/2}$ under every embedding. Hence $|w_j|=q_v^{1/2}$. The algebraic splitting and descended factorization ensure that the $w_j$ are algebraic numbers attached to $W$, not formal quotients chosen separately at each embedding.

**Proposition 6.3 (cancellation purity).** Under the global cancellation datum, its pairing factorization, and the good-place tensor identity, the rank-two surface Hom factor is pure of weight one. Without a pure weight-one cancellation factor, only weight-two purity of the full surface block is established.

No half Tate twist can replace cancellation. Tate twists change weights by even integers, so an integral twist cannot turn weight two into weight one.

### 6.4 Consequences for Hecke eigenvalues
...
**Corollary 6.4.** For every good $v$, $t_v$ satisfies the weight-two Weil bound (6.4), $s_v$ has finite order, and the roots of $P_v$ are algebraic integers.

**Proof.** The bound and unit modulus follow from (6.3). Algebraic integrality of the roots follows because $P_v$ is monic with algebraic-integer coefficients: $t_v$ is integral by the stable Hecke lattice, and $q_vs_v$ is integral. $\square$
...
Assume in the surface case additionally the global and local cancellation data of Chapter 12. Then the weakly compatible packet family is strictly compatible in the sense of Definition 3.2. The common $D_v$ is respectively unramified, special, finite type, or tame dihedral as described above.

**Proof.** At a good place the smooth proper comparison and (4.6) give (7.4) for every $\lambda$ away from $v$. Proposition 7.1 treats the two special cases. Proposition 7.2 treats finite and tame dihedral types. These identifications are made from the same algebraic model, correspondences, type data, and Frobenius return maps, so they hold for every allowed coefficient place. This is exactly (3.6). The surface assertion follows only after local cancellation identifies the Hom Weil--Deligne pair with the desired rank-two factor. $\square$
...
Two curve realizations of the same full packet data can produce the same rank-two object because their multiplicity constructions are identified by the packet correspondences and the common adjoint pairing. A curve realization and a surface realization require more care: the surface first gives a four-dimensional weight-two block.

When both a curve factor and a cancellable surface factor exist, the local comparison theorem identifies each of their verified local Weil--Deligne pairs with the same automorphic pair $D_v$. Thus their compatible-system data agree place by place wherever both local packages apply. If an actual descended correspondence identifies their multiplicity objects, this gives an isomorphism of the global representations without any density argument.
...
**Proposition 10.3 (carrier independence at the level proved).** A change of carrier which is related by a descended multiplicity or cancellation identification leaves the rank-two system unchanged up to isomorphism. If only common Hecke polynomials and local parameters are known, the two constructions define the same compatible-system data; no canonical comparison of their ambient cohomology or integral lattices is asserted.
...
- A rational geometric carrier is interchangeable only when the required descended comparison or cancellation data exist.
...
5. integral monodromy logarithms, especially the term $U^2/2$ for a surface at $\ell=2$;
6. nonprimitive restriction or Gysin images.
...
## 12. The surface-cancellation boundary

### 12.1 The honest surface system

Let $S/F$ be a compact unitary surface in the established geometric range, and let $V_{\Pi,\lambda}^{\mathrm{surf}}$ be its packet part of $H^2$. At full minimal orientation level it is generally four-dimensional, has Hodge numbers $(1,2,1)$, carries a pairing with twist $(-2)$, and is pure of weight two at good places.

The good characteristic polynomial has degree four. It is coefficient-place independent because projected traces and surface purity isolate the packet factor. This gives an honest compatible family, but not the rank-two weight-one family of a parallel-weight-two $\mathrm{GL}_2$ packet.
...
inside a complex Hodge decomposition does not solve the problem. Those lines need not be defined over $E$, need not be Galois stable, and have not been cut out by an algebraic correspondence.

### 12.2 Global cancellation

A global cancellation datum consists of a descended split algebra

$$
B_\Pi\simeq M_2(E_\lambda),
$$

a two-dimensional simple $B_\Pi$-module $R_{\Pi,\lambda}$, compatible Galois actions normalizing the algebra action, and an evaluation isomorphism

$$
R_{\Pi,\lambda}\otimes
W_{\Pi,\lambda}^{\mathrm{surf}}
\xrightarrow{\sim}V_{\Pi,\lambda}^{\mathrm{surf}}, \tag{12.1}
$$
...
(R_{\Pi,\lambda},V_{\Pi,\lambda}^{\mathrm{surf}}). \tag{12.2}
$$

The action on the Hom factor is

$$
(g f)(r)=g\bigl(f(g^{-1}r)\bigr). \tag{12.3}
$$

It remains $B_\Pi$-linear because Galois acts compatibly on the algebra and module. If $R$ is pure of weight one and its alternating pairing removes one twist $(-1)$ from the symmetric surface pairing, the remaining $W$ is rank two, pure of weight one, and has pairing target $E_\lambda(-1)\otimes\eta$.

Splitness alone is insufficient. The algebra, module, action, tensor factorization, and pairing must descend algebraically. A matrix algebra visible only after choosing complex Hodge coordinates has no arithmetic force.

### 12.3 Local cancellation

At a bad place, global factorization (12.1) does not determine the monodromy on $W$. The nearby-cycle complex of the surface must factor compatibly into the cancellation complex and desired packet complex. If

$$
N_V=N_R\otimes1+1\otimes N_W, \tag{12.4}
$$

then differentiating (12.3) gives

$$
(N_Wf)(r)=N_V(f(r))-f(N_Rr). \tag{12.5}
$$

Equation (12.5) is the local cancellation of monodromy. It is not obtained by dividing the rank of $N_V$ by two.

**Theorem 12.1 (surface boundary theorem).** The full surface packet forms a pure weight-two compatible family in the established surface range. It yields a rank-two strictly compatible weight-one family only if:

1. a descended global cancellation datum gives (12.1)--(12.3);
2. the pairings factor with the required twist and finite character;
3. at every bad place under consideration, the nearby-cycle complexes and Frobenius actions satisfy the local tensor factorization giving (12.4)--(12.5);
4. the desired Hom factor satisfies one of the local comparison packages of Theorem 7.3.

**Proof.** The first assertion is purity and coefficient independence of the correspondence summand of surface $H^2$. Under global cancellation, Morita evaluation gives a two-dimensional continuous Hom representation; the pairing gives its determinant and Section 6.3 gives weight one. Under local cancellation, (12.5) identifies its finite inertia, nilpotent operator, and Frobenius with the desired packet complex. Theorem 7.3 then proves strict compatibility. $\square$

### 12.4 Failures prevented by the boundary

Four tempting shortcuts are invalid.

**Half twisting.** An integral Tate twist changes weight by two, so no twist changes a weight-two surface block into weight one.

**Taking a Hodge half.** Hodge summands are archimedean and need not define subrepresentations of arithmetic Galois.

**Dimension division.** A four-dimensional module for a split matrix algebra has a rank-two multiplicity only when the descended algebra action and module structure have been supplied.

**Monodromy division.** A surface may have $N^3=0$ and length-three blocks. The rank-two factor has $N^2=0$ only after (12.5) subtracts the auxiliary monodromy.

Consequently the surface boundary is not an expository qualification. It is the algebraic condition separating an honest four-dimensional system from a genuine rank-two one.
...
2. It has a compact Shimura-curve cohomological realization whose full finite Hecke block admits multiplicity extraction of rank two, or it has a unitary-surface realization equipped with all the global cancellation data of Section 12.2.
3. The adjoint packet and component descent give the perfect alternating multiplicity pairing with finite character $\eta$.
4. A finite extension $E/E_0$ splits the simple Hecke factors and contains the finitely many local types and cancellation data.
...
In the surface case every rank-two conclusion above is conditional on both global and local cancellation. Without those data the unconditional object is the full, generally four-dimensional, pure weight-two surface packet system.

### 13.2 Proof

The finite algebraic Hecke module gives $E_0$, integrality of $t_v$ and $s_v$, and the occurrence of every coefficient conjugate. A finite splitting extension $E$ exists by Proposition 2.1. At each $\lambda$, multiplicity extraction from curve $H^1$ gives the rank-two Hom space (4.1); in the surface case global cancellation gives (12.2). Closedness of the Hom equations proves continuity, and Jordan--Holder gives the canonical semisimplification. This constructs (13.1).
...
The same special fiber is a smooth proper curve over a finite field. Weight-one purity of its $H^1$ passes through the correspondence summand and multiplicity factor, proving good-place purity. For a surface Hom factor, the pure weight-two ambient block and the pure weight-one cancellation module give weight one to the remaining factor. The pairing also removes exactly one twist, confirming the normalization.

At a bad place, the local comparison theorem computes the packet part of the equivariant nearby-cycle complex. For split strict semistability it identifies nonzero graph monodromy and the bad-correspondence scalar, giving $\operatorname{St}(\beta_v)$. Finite descent supplies a common inertia representation and Frobenius return maps. In the tame dihedral case these data are the quadratic extension and inducing character. All are defined algebraically over $E$ and commute with scalar extension, so Propositions 7.1 and 7.2 give (13.4). In the surface case local cancellation formula (12.5) is needed before applying the same recognition statements.
...
| purity of a surface packet | the same weight theory | smooth proper surface, or controlled interior cohomology; algebraic projector | ordinary cohomology of an uncontrolled open or singular surface is not substituted |
...
| rank-two surface factor | the same extraction theory | descended split cancellation algebra and compatible pairing | the honest surface block is generally four-dimensional and weight two |
...
| surface local parameters | the same local comparison theory | local nearby-cycle tensor factorization and Hom logarithm | global cancellation alone does not cancel local monodromy |
...
11. Surface $H^2$ has weight two; rank two and weight one require descended cancellation, including at bad places.
12. A residue-degree change raises Frobenius roots to the corresponding power.
...
The theory also draws its own boundary sharply. Rational compatibility does not canonically synchronize integral lattices. A surface contributes its honest four-dimensional weight-two block until a descended algebraic factor cancels globally and locally. At the coefficient residue characteristic, additional comparison conditions are required. Within these boundaries, however, the result is genuinely uniform: coefficient embeddings, polarization scalars, splitting fields, and verified geometric carriers alter presentations but not the compatible arithmetic data.
