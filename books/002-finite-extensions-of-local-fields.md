...
Theorem 8.3 is local: it produces a generator over a complete base. Chapters that work over a global or noncomplete base need to know how monogenicity behaves under localization, and the correct principle is that equality of integral rings is detected at the height-one primes.

**Lemma 8.4 (monogenicity is detected at height one).** Let $A$ be a Dedekind domain with fraction field $K$, let $B$ be a finite $A$-algebra, and let $\alpha\in B$. Then $B=A[\alpha]$ if and only if

$$
B_{\mathfrak p}=A_{\mathfrak p}[\alpha]
\quad\text{for every height-one prime }\mathfrak p
\text{ of }A.
$$

**Proof.** Since $A[\alpha]\subseteq B$ and $B$ is finite over $A$, the quotient $B/A[\alpha]$ is a finite $A$-module, and localization is exact, so for each prime $\mathfrak p$ the localization $(B/A[\alpha])_{\mathfrak p}$ identifies with $B_{\mathfrak p}/A_{\mathfrak p}[\alpha]$. A finite module over a domain vanishes if and only if it vanishes after localization at every maximal ideal; in a Dedekind domain the maximal ideals are exactly the height-one primes. Hence $B=A[\alpha]$ holds exactly when all the localized equalities hold. $\square$

Combined with the unramified generation theorem, this converts the étale case into a height-one computation. Suppose $B$ is finite over $A$ and _étale at every height-one prime_: each localization $B_{\mathfrak p}$ is finite étale over the discrete valuation ring $A_{\mathfrak p}$. If moreover the completion of $A_{\mathfrak p}$ is used — or $A_{\mathfrak p}$ is otherwise henselian — then each $B_{\mathfrak p}$ is monogenic over $A_{\mathfrak p}$ by lifting a primitive element of its separable residue extension, exactly as in Section 7.3. Lemma 8.4 then reduces global monogenicity to the compatibility of these local generators: an $\alpha\in B$ generates $B$ precisely when its image generates every height-one localization.

One boundary of this principle must be recorded honestly, because downstream chapters are heavy consumers of it. Local monogenicity at every height-one prime does _not_ by itself produce a single global generator: the local generators live in different localizations and there is no Chinese-remainder device that glues generators across primes, since a localization $B_{\mathfrak p}=A_{\mathfrak p}[\alpha_{\mathfrak p}]$ need not be generated by the image of any fixed global element with the same denominator behavior at the other primes. Thus the henselian hypothesis (or completeness) at the local level, plus a compatible choice of residue generators, is genuinely part of the input; a blanket assertion "locally monogenic at all height-one primes implies monogenic" is false, and any argument that needs the global statement must supply the missing compatibility from elsewhere — for instance by working after completion, where Theorem 8.3 applies directly, or over a base whose height-one localizations all complete to the same henselian ring.
