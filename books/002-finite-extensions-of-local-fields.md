...
**Proof.** Because only finitely many $\alpha_i$ occur, choose a positive radius $r$ so small that
the closed balls of radius $r$ about the $\alpha_i$ lie in the prescribed $V_i$ and are pairwise
...
$$
|g(\alpha_i)|<|g'(\alpha_i)|^2
$$

and that the Newton estimate place the resulting root within distance $r$ of $\alpha_i$. More
explicitly, first keep $|g'(\alpha_i)|=|f'(\alpha_i)|$ by making
$|g'(\alpha_i)-f'(\alpha_i)|<|f'(\alpha_i)|$, and then make $|g(\alpha_i)|$ smaller than both
$|f'(\alpha_i)|^2$ and $r|f'(\alpha_i)|$.

The field $M$ is finite over $K$ and hence complete. Hensel's lemma in $M$ gives a root
$\beta_i\in M$ satisfying

$$
|\beta_i-\alpha_i|
\leq\frac{|g(\alpha_i)|}{|g'(\alpha_i)|}<r.
$$

The balls are disjoint, so the $\beta_i$ are distinct. A degree-$d$ polynomial with $d$ distinct
roots has no others, proving the factorization and the assertion of uniqueness in each ball.
$\square$
...
Here unramified extensions count as tame. When $p=1$, this is Theorem 13.1. When $p>1$, it excludes
the wild families responsible for failure.
...
The polynomial $T^p-T-t^{-m}$ is separable because its derivative is $-1$. It defines a cyclic
extension of degree $p$. To see irreducibility, recall that an Artin--Schreier polynomial
$T^p-T-a$ has a root in $K$ exactly when $a=x^p-x$ for some $x\in K$. If $v_K(x)<0$, then
...
order $m$ prime to $p$, it is not of this form.

The extensions $L_m$ are pairwise nonisomorphic over $K$. Degree-$p$ Artin--Schreier extensions
correspond to one-dimensional $\mathbf F_p$-subspaces of

$$
K/\{x^p-x:x\in K\}.
$$

If the fields defined by $t^{-m}$ and $t^{-n}$ were the same, there would be a
$c\in\mathbf F_p^\times$ such that

$$
t^{-n}-ct^{-m}=x^p-x
$$

for some $x\in K$. When $m\ne n$, the left side has pole order $\max\{m,n\}$, which is not
...
Suppose $E=F(\alpha)$ and the chosen integral equation has good reduction at $\mathfrak p$: its
