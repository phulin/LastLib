...
|---|---|---|---|
| generic unramified-to-special change | $q_v\not\equiv\pm1\pmod\ell$ | separated old roots and a signed one-prime Gram block | conditional level raising and integral curve--definite comparison from Book 159 |
| scalar tame twist | $q_v\equiv1\pmod\ell$ | enhanced line, sign, global ray quotient $\Delta_v$, and $\Lambda_v=\mathcal O[\Delta_v]$ | Book 165's strict type complex under $({\rm TIC}_v)$ and $({\rm TIH}_{\mathscr F})$, support under $({\rm SUP}_v)$, and exact twist augmentation under the effective ray hypothesis; constant Ihara and primitive residue are separately required only for the corrected component filtration and extreme residue |
...
definite lattice is a further comparison: it separately requires Book 159's
...
retained finite level, Book 161 then supplies a map
...
before using Book 158's corrected constant-coefficient filtration. If type incidence or
...
Book 158 proves, after the component and central routing has been performed, the exact formula
...
For the flag resolution used by Book 165, one needs a finite family of such statements. If
...
Book 158 also identifies the orthogonal new lattice with $Q^\vee$, computes the Gram operator
(2.4), and relates the graph monodromy pairing to the Néron component group. Book 159 adds the
...
Book 159 and specialize the central operator to $S_v=1$; the general central branch is obtained
...
away-from-$v$ residue character and that the $U_v$ action and local type have Book 159's
...
projector is unavailable. Book 165 replaces it by an actual line-enhanced coefficient system.
...
Book 165 does not transfer endpoint injectivity from constant coefficients by a formal
...
character calculation of Book 158 does not prove (5.0). The constant family
...
primitive. Book 165 assumes $({\rm PRI}_{\rm type})$: after localization and residual
...
$({\rm TIC}_v)$, $({\rm TIH}_{\mathscr F})$, and $({\rm SUP}_v)$, Book 165 proves that the
...
2. $({\rm TYPE}_v)$ means $({\rm TIC}_v)$ and
   ...
   tuple.
3. $({\rm SUP}_v)$ is the five-clause one-boundary datum of Book 165: marked finite shadows,
...
4. $({\rm REC}_v)$ is the Book 161 reciprocity package on the actual finite carrier: integral
...
$({\rm AIH}_{\mathscr F})$ for Book 158's corrected constant rows,
...
Under $({\rm TYPE}_v)$, Book 165 gives a strict primary--companion complex whose new
...
finite levels. Under $({\rm REC}_v)$, Book 161 gives at finite level a canonical surjection
...
At patched level, Book 165 still gives
...
Book 165 proves full support again at each retained finite level. Theorem 6.2 can therefore
...
gives the selected local packet, while Book 161's Hecke-valued representation and the
...
proved for that exact problem. Books 158 and 165 do not turn those other regimes into the
...
Automorphy over $L$ does not descend merely from Galois invariance. Book 97 applies through a
...
|---|---|---|
| Book 97 | recursive cyclic solvable descent selected by a representation already defined over the lower field | Galois invariance alone is not descent; irreducibility, image criteria, and full local antecedents are rechecked |
| Book 155 | neutral minimal modularity lifting and its coefficient-prime range | its equality is not applied to the conductor-one line-special ring with Kummer directions |
| Book 157 | enhanced special lines, primary and companion lattices, tame twists, and exceptional local charts | local lattices do not prove type incidence, type Ihara, primitive residue, or global support; primitive wild types are excluded |
| Book 158 | exact tree kernel (4.3), and conditional saturation, duality, Gram, component, and branch sequences | residual injectivity depends on $({\rm AIH}_{\mathscr F})$ at every actual constant flag level; it is not a type-Ihara theorem |
| Book 159 | conditional one-prime integral level change, node/definite comparison, equality of faithful orders, and signed Gram calculation | also requires node uniformization and generic support; proves no unconditional several-prime theorem; keeps the branch Smith correction separate |
| Book 161 | order-valued Galois representation, factorization through represented local conditions, and the surjection (3.5) | requires every generic branch, integral coefficient-prime realization, reduced generic order, exact trace generation, and the conditional Book 159 comparison whenever used |
| Book 165 | enhanced one-prime type complex, scalar $q\equiv1$ family, primitive residue, one-prime support, and exact fixed-prime augmentation | $({\rm TIC}_v)$, $({\rm TIH}_{\mathscr F})$, $({\rm PRI}_{\rm type})$, $({\rm NU}_{\rm type})$, and $({\rm SUP}_v)$ are separately named hypotheses; support is topological and only one-place |
| Book 166 | finite-level reduced one-prime comparison and pointwise characteristic-zero modularity in the scalar family | full finite-level $R=T$ requires global reducedness; no patched reduced comparison follows without reducedness of the patched image, and no finite-set theorem is supplied |

The exact direct-dependency row is

$$
156\mid 97,155,157,158,159,161,166,165.
\tag{10.0}
$$

Within Books 154--166, Books 160, 162, 163, and 164 enter transitively through the type-support
and one-prime patching packages. Book 154 is a downstream arithmetic synthesis, not a proof
...
3. independently, $({\rm AIH}_{\mathscr F})$ turns each constant tree kernel into residual
   injectivity and supplies Book 158's corrected filtration;
...
$({\rm NU}_{\rm type})$, and Book 159's node-uniformization, generic-support, and
