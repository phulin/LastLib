...
Two examples fix the convention. The Cartier dual of $\mathbf Z/n\mathbf Z$ is $\mu_n$, while the Cartier dual of $\mu_n$ is $\mathbf Z/n\mathbf Z$. _(Conditional remark._ The following example anticipates a duality construction proved later in the library: for an elliptic curve $A$ and $n$ invertible in $K$, the Weil pairing identifies the Cartier dual of $A[n]$ with the $n$-torsion of the dual elliptic curve, and a principal polarization gives a self-duality. Nothing below depends on it; the coefficient pairings used in this book are constructed from Cartier duality alone.*) These are exactly the coefficient pairings that later induce local pairings on first cohomology.
