...
is onto. Indeed, local surjectivity follows from $H^1(F_v,\mathbf G_m)=0$, and at almost every place an integral projective point has an integral lift, so the local lifts form a restricted product. The same argument over $F$ identifies $H(F)$ with $D^\times(F)/F^\times$. Choose a compact open $U\subset D^\times(\mathbf A_f)$ whose image lies in $K_f$. Book 83 proves, by the order-ideal argument and without using definiteness at infinity, that
...
At a split place, reduced norm is determinant and elementary matrices generate $\operatorname{SL}_2(F_v)$, the kernel of determinant. At a finite division place, Book 79 proves that the commutator subgroup is the reduced-norm-one group. At a ramified real place,
...
**Proof.** The compact-quotient argument of Theorem 4.1 first gives the required global admissibility: after fixing a finite compact open, an archimedean compact type, and the infinitesimal character of $\pi$, the corresponding vectors form a finite-dimensional space. Book 95 proves the restricted-product factorization lemma for any irreducible admissible module over
...
This time Poisson summation is available, but it must be invoked with its complete normalization. Tensoring Book 86's one-dimensional Fourier theorem over an $F$-basis of $D$, and using the trace pairing
...
Nothing in Proposition 7.2 asserts a representation correspondence. Its inputs are the compact trace formula, the rank-one simple split trace formula, and orbital matching. In particular, it neither cites Book 104 nor assumes the theorem to be proved below.
...
The corresponding fixed-level, fixed-type, bounded window on the split cuspidal side is finite dimensional by the cuspidal compactness theorem of Book 90. Outside $S$, the spherical Hecke operators and their adjoints generate a commuting finite-dimensional $*$-algebra on the direct sum of the quaternionic window and this split window. Only finitely many characters of that algebra occur. For every character different from the full tail of $\pi$, choose one good place where it differs; finitely many such places and Lagrange interpolation then produce the exact projector onto

$$
\lambda^S=(\lambda_{\pi_v})_{v\notin S}. \tag{7.12}
$$

Apply this projector in (7.9), retaining arbitrary separating test functions at the places in $S$. On the split side, Book 92's global Whittaker theorem gives

$$
m_{\mathrm{GL}_2}(\sigma)=1
$$

for every cuspidal $\sigma$, and Book 94's Rankin--Selberg strong multiplicity-one theorem says that at most one such $\sigma$ has the good local data (7.12).
...
Here are the two downstream interfaces in exact form. Book 104 may begin its compact spectral comparison with $\sum_\pi m_D(\pi)\operatorname{tr}\pi(f)$ in order to track origins, but it must replace $m_D(\pi)$ by one using (7.10); neither Book 104's representation correspondence nor its later global theorem is used to obtain that value. For the one-split-real-place division algebra of Book 125, Matsushima's summand is
...
with action $\rho$. Book 83 used $W$-valued functions satisfying
...
The same space has the finite description of Book 83. Choose
...
Suppose first that every split finite factor of $\pi$ is infinite dimensional and generic. Let $a_v(\pi)$ be its $K_1$ conductor at a split place and the transfer-facing exponent $a_D(\pi_v)$ of Book 79 at a division place. Then define the selected conductor ideal
...
The conductor convention prepared in Book 79 is
...
The separate unramified character clause matters: its congruence exponent is zero, but its selected special-packet conductor is one. For a higher-dimensional level-zero representation, $a_D=2$. Thus selected conductor is not identical to first principal-congruence depth; it is the normalization fixed in Book 79 for the later local comparison.
...
Keep the Hurwitz algebra but impose principal level at $3$. The level-three calculation of Book 83 gives
