...
Finite projectivity of $A^\vee$ follows from the dual-basis theory of Book 10. Its rank equals that of $A$. The proof also shows exactly what fails outside the stated category: without finite projectivity the tensor-dual comparison can fail, and without cocommutativity the convolution algebra need not be commutative.
...
Constant and diagonalizable groups were introduced in Books 11 and 12 as opposite standard families. Cartier duality explains that opposition. Let $M$ be a finite abelian group. Over $S=\operatorname{Spec}R$, write
...
Duality should turn a quotient into a subgroup and a subgroup into a quotient. In the finite-flat category, those words have the precise meaning fixed in Book 12. A sequence
...
The exact Hopf-algebra criterion from Book 11 says that (7.3)--(7.4) satisfy the hypotheses of the lemma. In particular, the transpose
...
If $G$ is killed by $n$, then so is $G^D$. More generally, exactly the same integers annihilate $G$ and $G^D$, because $[n]_{G^D}=0$ dualizes back to $[n]_G=0$ under biduality. Thus they have the same exponent whenever that terminology is being used. The primary decomposition of Book 12 is preserved:
...
Let $k$ be a perfect field. Book 12 constructed
...
Let $R$ be a henselian local ring with perfect residue field $k$. For $G/R$ finite locally free and commutative, Book 12 gives
...
Cartier duality is a theory of finite affine groups, whereas an abelian scheme $A/S$ is proper and positive-dimensional. The connection is through multiplication kernels. For every $n\ge1$, Book 12 established that
...
This is the scheme-theoretic form of the contravariance familiar from Book 9.
...
When $n$ is prime to the characteristic of a field, taking geometric points recovers the pairing constructed in Book 9. A symplectic basis identifies its matrix with
...
On prime-to-characteristic geometric points this is the formula of Book 9. Scheme-theoretically it is an equality of morphisms obtained from the functoriality of the Poincaré biextension. For a prime $\ell$ invertible on the field, inverse limits yield
...
In characteristic $p$, consider the Frobenius sequence from Book 12,
