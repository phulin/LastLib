...
Recall the standard terminology. A Noetherian local ring $(R,\mathfrak m_R)$ with residue field $\kappa$ is **regular** if $\dim R=\dim_\kappa\mathfrak m_R/\mathfrak m_R^2$; equivalently, by Krull's height theorem, if $\mathfrak m_R$ can be generated by $\dim R$ elements. Such a generating family is a **regular system of parameters**. Krull's height theorem will be used throughout in its two familiar guises: a prime minimal over an ideal generated by $n$ elements has height at most $n$, whence on the one hand $\dim R\leq\dim_\kappa\mathfrak m_R/\mathfrak m_R^2$ for every Noetherian local ring, and on the other hand no ideal generated by fewer than $\dim R$ elements can have $\mathfrak m_R$ as a minimal prime. A family $x_1,\ldots,x_d\in\mathfrak m_R$ with $d=\dim R$ for which $R/(x_1,\ldots,x_d)$ has finite length is called a **system of parameters** of $R$. Unlike a regular system of parameters, which by definition exists only when $R$ is regular, a system of parameters exists in every Noetherian local ring; this is Lemma 13.28A below, and we prove it rather than take it on credit, because two arguments at the end of this section choose systems of parameters in rings that are not assumed regular.

A regular system of parameters generates the maximal ideal outright. A system of parameters asks for much less: its members need not generate $\mathfrak m_R$, only cut $R$ down to finite length, and it is exactly this weaker demand that can always be met with $\dim R$ elements. It is worth recording once what "finite length" means here in ideal-theoretic terms. For an ideal $I\subseteq\mathfrak m_R$ of a Noetherian local ring, the quotient $R/I$ is again Noetherian local, and a Noetherian local ring has finite length precisely when its dimension is zero, that is, when its maximal ideal is its only prime ideal. So $R/I$ has finite length if and only if $\sqrt I=\mathfrak m_R$, that is, if and only if $I$ is $\mathfrak m_R$-primary.

Two standard facts about Noetherian rings go into the existence proof. The first is that a Noetherian ring has only finitely many minimal primes, so that a finite list of primes to be avoided is available at all. The second is prime avoidance: an ideal contained in a finite union of prime ideals is contained in one of them. We shall also use repeatedly that every prime of $R$ contains a minimal prime, so that any chain of primes can be extended downwards until it begins at a minimal prime; in particular $\dim R$ is the largest of the numbers $\dim R/\mathfrak p$ as $\mathfrak p$ runs over the minimal primes of $R$.

**Lemma 13.28A (existence of systems of parameters).** Let $(R,\mathfrak m_R)$ be a Noetherian local ring of dimension $d$. Then there are elements $x_1,\ldots,x_d\in\mathfrak m_R$ such that $R/(x_1,\ldots,x_d)$ has finite length, that is, such that $(x_1,\ldots,x_d)$ is $\mathfrak m_R$-primary. Moreover no ideal generated by fewer than $d$ elements is $\mathfrak m_R$-primary, so a system of parameters is a shortest possible such family.

**Proof strategy.** The induction is on $d$, and the only delicate step is the choice of the first element. It must lie in $\mathfrak m_R$ and outside every minimal prime $\mathfrak p$ with $\dim R/\mathfrak p=d$, since those are precisely the primes responsible for the dimension being as large as it is; killing such an element then genuinely lowers the dimension. The induction hypothesis supplies parameters for the quotient, and Krull's height theorem, applied at the very end, shows that the dimension dropped by exactly one, so that the family produced has length $d$ rather than something longer.

**Proof.** Consider first the final assertion. If $I=(y_1,\ldots,y_n)$ is $\mathfrak m_R$-primary, then $\mathfrak m_R$ is the unique prime containing $I$ and is in particular minimal over $I$, so Krull's height theorem gives $d=\dim R=\operatorname{ht}\mathfrak m_R\leq n$.

Existence is proved by induction on $d$. If $d=0$, take the empty family: $R$ itself is Noetherian of dimension zero, hence of finite length, and the zero ideal is $\mathfrak m_R$-primary.

Suppose $d>0$ and the statement known for all Noetherian local rings of smaller dimension. Let $\mathfrak p_1,\ldots,\mathfrak p_r$ be those minimal primes of $R$ with $\dim R/\mathfrak p_i=d$; there are finitely many of them because $R$ is Noetherian, and there is at least one, since $\dim R$ is achieved by some minimal prime. No $\mathfrak p_i$ equals $\mathfrak m_R$, because $\dim R/\mathfrak m_R=0<d$; hence $\mathfrak m_R\not\subseteq\mathfrak p_i$ for every $i$, and prime avoidance gives

$$
\mathfrak m_R\not\subseteq\mathfrak p_1\cup\cdots\cup\mathfrak p_r .
$$

Choose $x_1\in\mathfrak m_R$ lying in none of the $\mathfrak p_i$.

Put $\bar R=R/(x_1)$, a Noetherian local ring with maximal ideal the image of $\mathfrak m_R$, and let $e=\dim\bar R$. We claim $e\leq d-1$. The primes of $\bar R$ are the images of the primes $\mathfrak q$ of $R$ containing $x_1$, and $\dim\bar R$ is the largest value of $\dim R/\mathfrak q$ over such $\mathfrak q$. So suppose $\mathfrak q$ contains $x_1$ and $\dim R/\mathfrak q=d$, and choose a minimal prime $\mathfrak p\subseteq\mathfrak q$ of $R$. A chain of $d+1$ distinct primes beginning at $\mathfrak q$ can be prefixed by $\mathfrak p$; were $\mathfrak p\neq\mathfrak q$, this would produce a chain of length $d+1$ in $R$, contradicting $\dim R=d$. Hence $\mathfrak q=\mathfrak p$ is itself a minimal prime with $\dim R/\mathfrak p=d$, so $\mathfrak q$ is one of $\mathfrak p_1,\ldots,\mathfrak p_r$ — and those do not contain $x_1$. This contradiction shows that every prime containing $x_1$ has $\dim R/\mathfrak q\leq d-1$, so $e\leq d-1$ as claimed.

By the induction hypothesis $\bar R$ has a system of parameters $\bar x_2,\ldots,\bar x_{e+1}$, that is, elements of its maximal ideal whose quotient has finite length. Lift them to elements $x_2,\ldots,x_{e+1}$ of $\mathfrak m_R$. Then

$$
R/(x_1,x_2,\ldots,x_{e+1})\cong\bar R/(\bar x_2,\ldots,\bar x_{e+1})
$$

has finite length, so the ideal $(x_1,\ldots,x_{e+1})$, generated by $e+1$ elements, is $\mathfrak m_R$-primary. By the assertion already proved, $d\leq e+1$. Together with $e\leq d-1$ this forces $e=d-1$, so the family $x_1,\ldots,x_d$ has exactly $d$ members and is a system of parameters of $R$. $\square$

The proof yields a little more than the statement asks for: applied to the family it constructs, the argument shows that the dimension drops by exactly one at each stage, so that $\dim R/(x_1,\ldots,x_i)=d-i$ for every $i$. This is a statement about dimension only, and asserts nothing about whether $x_i$ is a non-zero-divisor on the preceding quotient; a parameter may perfectly well be a zero divisor.
...
This is the form in which the structure theorem enters homological arguments: having a surjection $Q\twoheadrightarrow A$ from a complete regular local ring $Q$ lets one replace questions about the possibly very singular ring $A$ by questions about modules over the much better behaved ring $Q$. Looking ahead, the homological reason this is profitable is that a regular local ring has finite global dimension, so that every finitely generated module over $Q$ admits a finite free resolution; that theorem is not proved in this book, is used nowhere in it, and belongs to the homological algebra of local rings taken up in later volumes of this library — it is proved there as Book 10a, Chapter 2, Theorem 2.4 with Corollary 2.5, so any later volume invoking finite free resolutions over $Q$ may cite that chapter for it. What Corollary 13.31 supplies, and all that is claimed here, is the surjection itself. Nothing about $A$ needs to be assumed beyond completeness and Noetherianity.
...
The dimension hypothesis in the last statement is met by the natural choices. In equal characteristic, take $x_1,\ldots,x_d$ to be a system of parameters of $A$, which exists by Lemma 13.28A, so $s=d=\dim A$ and $\dim K[[X_1,\ldots,X_d]]=d$. In mixed characteristic, suppose $A$ is a domain of dimension $d$; then $p\neq0$, and
