...
**Proof.** We prove both directions of (3.3). Throughout we use the annular chart construction of Section 4.2, which builds the analytic quotient $T^{\mathrm{an}}/M$ by locally finite gluing of closed polyannuli; its verification is independent of, and does not use, the present theorem.

_Suppose first that $M$ is full._ Choose the bounded half-open parallelotope $P\subset N_\mathbf R$ of Section 3.3 meeting every coset of $\ell(M)$ once, and enlarge it slightly to a bounded closed rational polyannulus $P^+$ whose inverse image under tropicalization is an admissible affinoid cover of $T^{\mathrm{an}}$ modulo $M$; only finitely many translates $u(m)P^+$ meet $P^+$, because the tropical images $\ell(m)$ of periods leaving a compact enlargement of $P^+$ escape every bounded polytope. The images of these finitely many closed polyannuli form a finite affinoid cover of the quotient.

_Properness, valuative step._ Let $L/K$ be a complete valued extension field and consider a morphism from a punctured valuation disc over $L$ into $T^{\mathrm{an}}/M$; we must extend it over the puncture. Composing with $T^{\mathrm{an}}\to T^{\mathrm{an}}/M$ and passing to a finite extension of $L$, lift the map locally to $T^{\mathrm{an}}$: above each of the finitely many affinoid charts the covering $T^{\mathrm{an}}\to Q$ is a local isomorphism with discrete deck group $M$, so after refining the disc into finitely many sectors each sector lifts. The tropical coordinate of a lift is an element of $\operatorname{Hom}(X,\mathbf R)$ varying with the radial parameter; subtracting a suitable period displacement $\ell(m)$ moves it into the bounded range prescribed by $P^+$, which is possible precisely because $\ell(M)_\mathbf R=N_\mathbf R$, so every coset meets $P^+$. On the standard integral model of the closed polyannulus, obtained from the equations $X_iY_i=a_i$ together with bounding inequalities on coordinates (Book 59, Sections 7.1--7.2), the boundedness of all coordinate valuations says exactly that the lifted point extends over the puncture. Two such extensions agree on overlaps because the union of the graphs of the period translations inside $T^{\mathrm{an}}\times T^{\mathrm{an}}$ is closed, being a locally finite union of closed sets. This is the valuative criterion for properness of analytic spaces (Book 59, Section 12.1). Notice that a coordinate of nonzero valuation need not itself be a unit: it is the polyannular formal chart, rather than the affine torus model, that supplies the extension.

_Representability and separatedness._ For completeness, representability is not assumed but constructed. Cover a slight enlargement of $P$ by finitely many closed rational polyannuli. Glue any two along the finitely many rational subdomains related by a period translation. Local finiteness follows because a bounded polytope meets only finitely many translates of $\ell(M)$. The cocycle is the addition law of $M$. Affinoid gluing therefore constructs the quotient used above. Separatedness follows directly here because the locally finite union of the closed translation graphs is closed; Chapter 4 verifies the universal sheaf property in general.

_Conversely, suppose the real span of $\ell(M)$ is proper in $N_\mathbf R$._ Then the orthogonal complement $V$ of $\ell(M)_\mathbf R$ in $N_\mathbf R$ is nonzero whenever $\operatorname{rank}\ell(M)<r$, and we are done if $\operatorname{rank}\ell(M)<r$; choose a nonzero linear functional $h$ vanishing on $\ell(M)_\mathbf R$ and nonzero on $V$. Pick a cocharacter $\lambda':\mathbf G_m\to T$ with $\langle h,\operatorname{trop}\lambda'(t)\rangle$ unbounded as $v(t)\to+\infty$. The ray $t\mapsto\lambda'(t)$ for $v(t)\geq0$ defines a punctured valuation-disc map into $T^{\mathrm{an}}/M$. If it extended over the puncture, its image would meet some translate $u(m)P^+$, since the finitely many chart images cover the whole quotient; but translating back to $P^+$ changes the tropical coordinate only within the bounded set $P^+-P^+$, while $h$ applied to the ray is unbounded — a contradiction, because $h$ vanishes identically on all period displacements. Hence no extension exists and the quotient is not proper. Finally, if $\ell$ has nontrivial kernel, discreteness already fails by Lemma 3.1, so this case is excluded by hypothesis. $\square$
...
The algebraization step used here is not confined to curves. The earlier formal and rigid machinery — Book 59's formal-model theorem (its Theorem 9.2), extension of morphisms and coherent data to models after admissible blowup (its Proposition 13.1 and Section 13.2), invariance of generic fibers under admissible blowup (its Theorem 9.1), and the completed-localization comparison (its Section 7.3) — combined with Grothendieck's existence theorem for the proper scheme $\mathbf P^N_R$ (Book 58, Theorem 9.4.4, with the passage from projective to proper carried out in its Section 9.5) makes a direct proof possible. The proof below is written for an arbitrary closed analytic subspace; when it is applied to the theta embedding of a torus quotient, every model involved can be taken to be built from the polyannular toric charts of Sections 2–4, so no input beyond those charts and the cited theorems is consumed.

**Projective algebraization lemma.** A closed analytic subspace $Z\hookrightarrow\mathbf P_K^{N,\mathrm{an}}$ is the analytification of a unique closed subscheme of $\mathbf P_K^N$.

**Proof.** _Step 1: move the ideal onto a formal model._ Let $\mathcal I\subset\mathcal O_{\mathbf P^{N,\mathrm{an}}}$ be the coherent ideal of $Z$; coherence holds because a closed immersion is finite, affinoid-local coherence is Book 59's Theorem 6.1, and the pieces glue over the standard affinoid cover. Let $\mathfrak P=\widehat{\mathbf P}^N_R$ be the standard formal model; its generic fiber is all of $\mathbf P^{N,\mathrm{an}}$, since every analytic point of projective space has bounded homogeneous coordinates after scaling, which places it on one of the standard affine charts and hence in the generic fiber of the corresponding formal affine chart. By the lattice-extension mechanism of Book 59, Section 13.2 applied to $\mathcal I$ and the model $\mathfrak P$, there is an admissible blowup $\mathfrak P'\to\mathfrak P$ and a coherent sheaf $\mathfrak I'$ on $\mathfrak P'$ with $(\mathfrak I')_\eta\simeq\mathcal I$.

_Step 2: realize the blowup algebraically._ The blowup $\mathfrak P'$ is admissible, so its center is supported in the special fiber and contains a power of the uniformizer. If $J\subset\mathcal O_{\mathbf P^N_R}$ is a finitely generated ideal whose completion generates the blown-up ideal sheaf, then the algebraic blowup $Y:=\operatorname{Bl}_J(\mathbf P^N_R)$ is proper with $Y_K\simeq\mathbf P^N_K$, and its completion identifies with $\mathfrak P'$. On affine charts this identification is the statement that formation of the degree-zero part of the Rees algebra commutes with $\pi$-adic completion, which holds because completion is exact on finitely generated modules (Book 58, Sections 2.2 and 5.3). This is exactly the mechanism used in the curve case in Book 59's Theorem 14.1; nothing in it uses dimension one.

_Step 3: apply Grothendieck existence._ Now $Y$ is a proper $R$-scheme and $\widehat Y\simeq\mathfrak P'$, with $\mathfrak I'$ coherent on $\widehat Y$. Book 58's Theorem 9.4.4 — whose Section 9.5 extends it from the projective to every proper target by noetherian induction along a projective modification — algebraizes $\mathfrak I'$ uniquely to a coherent sheaf $\mathcal I_Y$ on $Y$ with completed sheaf $\mathfrak I'$.

_Step 4: cut out the scheme._ Let $W\hookrightarrow Y$ be the closed subscheme defined by $\mathcal I_Y$. Exactness of completion on coherent sheaves identifies $\widehat W$ with the closed formal subspace of $\mathfrak P'$ cut out by $\mathfrak I'$; passing to generic fibers and using the comparison between completed localization followed by inversion of $\pi$ and analytic tensor product (Book 59, Section 7.3), together with admissible-blowup invariance of the generic fiber (its Theorem 9.1), gives an isomorphism of rigid spaces

$$
W_K^{\mathrm{an}}\;\simeq\;Z.
$$

Moreover $W_K$ is a closed subscheme of $Y_K\simeq\mathbf P^N_K$, hence projective.

_Uniqueness._ If $W,W'\subset\mathbf P^N_K$ are two closed subschemes whose analytifications equal $Z$ as subspaces of $\mathbf P^{N,\mathrm{an}}$, then the completions of their ideal sheaves both cut out the same closed formal subspace of $\widehat{\mathbf P}^{\,N}_R$, namely the model of $Z$. The quotient $\mathcal I_W/(\mathcal I_W\cap\mathcal I_{W'})$-type comparison reduces to this: the two ideals have identical completions, and completion along the ideal of definition is exact on coherent sheaves (Book 58, Section 5.3) with zero-completed finite modules zero by Nakayama; hence $\mathcal I_W=\mathcal I_{W'}$ and $W=W'$. $\square$

For the application to Theorem 6.1, one consequence of the proof is recorded separately because the group-law descent uses it.

**Lemma 6.3.1 (algebraization of graphs).** Let $V_i$ be projective $K$-varieties, each realized as a closed subscheme of some projective space. For every analytic morphism $f:(V_1\times_KV_2)^{\mathrm{an}}\to V_3^{\mathrm{an}}$ whose graph is closed, there is a unique $K$-morphism $g:V_1\times_KV_2\to V_3$ with $g^{\mathrm{an}}=f$.

**Proof.** Compose the graph of $f$ with Segre embeddings: $\Gamma_f\subset(V_1\times V_2\times V_3)^{\mathrm{an}}$ is a closed analytic subspace — closedness holds because $V_3$ is proper, so the projection is closed — and the triple Segre embedding realizes $V_1\times V_2\times V_3$ as a closed subscheme of some $\mathbf P^M_K$, compatibly with analytification. Hence $\Gamma_f$ is a closed analytic subspace of $\mathbf P^{M,\mathrm{an}}$, and the projective algebraization lemma produces a closed subscheme $H\subset\mathbf P^M_K$, projective, with $H^{\mathrm{an}}=\Gamma_f$. The projections from $\Gamma_f$ transport to projections $p_{12}:H\to V_1\times V_2$ and $p_3:H\to V_3$ — they exist algebraically because $H$ maps into the Segre image of the triple product, and one composes with that product's projections. Analytically $p_{12}$ is an isomorphism with analytic inverse $h:f$-section transport; the inverse graph is again a closed analytic subspace of a product of projective varieties, so it algebraizes to a morphism $q:V_1\times V_2\to H$. Both composites $q\circ p_{12}$ and $p_{12}\circ q$ are morphisms between projective varieties whose analytifications are identity maps; two morphisms between projective varieties agreeing analytically agree, because their graphs are closed analytic subspaces that the uniqueness clause of the projective algebraization lemma forces to coincide, and a morphism is recovered from its graph by the projections. Hence $q$ is an inverse to $p_{12}$, and $g:=p_3\circ q$ satisfies $g^{\mathrm{an}}=f$. Uniqueness: if $g'^{\mathrm{an}}=f$ too, then $g$ and $g'$ agree analytically, hence agree by the same graph-uniqueness argument. $\square$

**Lemma 6.3.2 (connectedness transfer).** Let $V$ be a finite-type $K$-scheme. If $V^{\mathrm{an}}$ is connected, then $V$ is connected; if $V^{\mathrm{an}}$ is geometrically connected, then $V$ is geometrically connected.

**Proof.** If $V$ were disconnected, there would be a decomposition $V=W_1\sqcup W_2$ into nonempty open-and-closed subschemes. Analytification converts open immersions into admissible open immersions (Book 59, Section 7.3) and preserves coproducts, giving $V^{\mathrm{an}}=W_1^{\mathrm{an}}\sqcup W_2^{\mathrm{an}}$, a disconnection with both parts nonempty because each $W_i$ contributes points over some finite extension of $K$. Contradiction. For geometric connectedness, analytification commutes with base change (Book 59, Section 7.5), so $(V_L)^{\mathrm{an}}\simeq(V^{\mathrm{an}})\widehat{\otimes}_KL$ is connected for every extension $L/K$ whenever $V^{\mathrm{an}}$ is geometrically connected, and the first part applied to $V_L$ finishes the proof. $\square$

One genuine transfer remains outside what Books 58 and 59 prove, and it is flagged rather than smuggled.

**Standing input (smoothness transfer).** _Let $V$ be a connected finite-type $K$-group variety. If $V^{\mathrm{an}}$ is smooth, then $V$ is smooth._

_Provenance._ Smoothness of a group variety can be tested at the identity after base change to $\overline K$ (translations are algebraic automorphisms, and smoothness descends through field extensions). At an $\overline K$-rational point $x$, the completed local ring $\widehat{\mathcal O}_{V,x}$ and the local ring of $V^{\mathrm{an}}$ at the matching rigid point have canonically identified completions, both being completions of the same affinoid presentation; regularity of the latter therefore implies regularity of the former once finite-type algebras over the excellent field $K$ are granted excellence (Book 010a, Section 10.2) together with the reflection of regularity along the completion of an excellent local ring. The comparison between the two completed local rings is the only ingredient not yet carried out in this corpus at arbitrary dimension — Book 59 develops it in the affinoid setting of Section 7.3 and in dimension one in Chapter 11 — and it is exactly a GAGA-type locality statement.

_Consumer analysis._ Within this book the standing input is consumed only by the final sentences of Theorem 6.1 and of Section 9.3, each of which concludes "smooth" for an already constructed projective group variety whose analytification is the manifestly smooth quotient $Q=E/M$. No other section consumes it: Chapters 7–8 work with formal models directly, where smoothness is available from the chart construction, and the numerical statements of Chapter 11 use only dimensions and exact sequences. Downstream, Book 61 consumes uniformization only through Theorem 9.1, whose semistable-reduction conclusion rests on the formal-model construction of Section 9.3 and not on the standing input alone; should the standing input later be discharged, the flag can simply be removed.
...
Apply the projective algebraization lemma to the image: there is a projective variety $A/K$ with $A^{\mathrm{an}}\simeq Q$. The multiplication, inverse, and identity of the analytic group $Q$ are analytic morphisms with closed graphs — their targets are proper — so Lemma 6.3.1 algebraizes each to a $K$-morphism of $A$. The group axioms hold because pairs of algebraic morphisms between projective varieties agreeing analytically agree, as recorded in the proof of Lemma 6.3.1. Being projective, $A$ is proper; geometric connectedness transfers by Lemma 6.3.2. The analytic space $Q$ is smooth, so the standing input above makes the connected group variety $A$ smooth. Thus $A$ is an abelian variety. This descent argument uses an actual linearized tensor product, not merely a Galois-invariant numerical class, which by itself would not be effective descent data.

For the last assertion, let $f:Q_1^{\mathrm{an}}\to Q_2^{\mathrm{an}}$ be an analytic homomorphism between two quotients algebraized as $A_1,A_2$. Its graph is closed in the product of proper spaces, and Lemma 6.3.1 — applied after realizing both factors through their theta embeddings, so that the product lands in projective space — yields a unique $K$-morphism $A_1\to A_2$ analytifying to $f$; because composition agrees with composition analytically, the homomorphism property descends the same way. Uniqueness is the graph-uniqueness clause of Lemma 6.3.1. $\square$
...
Characterwise pushout of $\widehat{\mathcal G}_{n+1}\to B_{n+1}$ gives rigidified algebraically trivial line bundles. The Poincare bundle identifies them with $c_{n+1}:X\to B_{n+1}^\vee$, and its biextension law records additivity. Proposition 7.1 reconstructs the extension and identifies it with the completed group. A relatively ample symmetric bundle on $B_s$, replaced once by a fixed power, lifts with its cube structure through the system; Grothendieck existence — Book 58, Theorem 9.4.4, extended to proper targets in its Section 9.5, applied levelwise on the noetherian base with the uniform vanishing of its Proposition 9.3.4 supplying the bounds that are independent of the level — guarantees that the compatible system of coherent descent data is effective, and the inverse limit of the quotient schemes is a proper adic formal abelian scheme $\mathfrak B$ over $\operatorname{Spf}R$ whose special fiber is $B_s$. Passing to the inverse limit gives (8.2). Translated toric charts are then attached using $X$; two rational choices have a common subdivision. This proves existence, uniqueness up to unique admissible refinement, and compatibility with the original formal identity neighborhood. $\square$
...
The projective algebraization lemma of Section 6.3, applied to the theta embedding just constructed, produces a projective variety $A/K$ with $A^{\mathrm{an}}\simeq Q$. The analytic multiplication, inverse, and identity algebraize uniquely by Lemma 6.3.1. Smoothness follows from the smoothness-transfer standing input of Section 6.3 — consumed here exactly as recorded in its consumer analysis — and connectedness descends by Lemma 6.3.2, so $A$ is an abelian variety.
