...
uniquely over the henselian base. Cartier duality exchanges this condition
with the multiplicative unit condition. We retain that exact integral
duality even though the global proof below is left exact and needs no global
duality sequence.

Cartier duality makes (3.4) and (3.5) exact orthogonal complements under local flat duality. For the argument below we need only their exactness under admissible subgroups and quotients, which follows by applying fppf cohomology to finite-flat exact sequences over $\mathbf Z_\ell$.
...
$\sqrt{\widehat h_L(P)}$. This is the estimate behind (8.2).

For completeness, these facts imply finite generation as follows.  Choose
representatives $R_1,\ldots,R_t$ of $A(\mathbf Q)/mA(\mathbf Q)$.  The
quadratic height pairing and the parallelogram law give a constant $C$ such
that whenever $P=mQ+R_i$,

$$
\widehat h_L(Q)\leq m^{-2}\widehat h_L(P)+C. \tag{8.2}
$$

If the height of $P$ is large, (8.2) strictly decreases it.  Repeated
division modulo the finite representative set reaches a point of bounded
height.  Northcott makes the terminal set finite.  Induction back through
the decompositions shows that the terminal points together with the
$R_i$ generate $A(\mathbf Q)$.  Thus
...
To make the descent finite, choose $B$ so large that
$m^{-2}x+C<x$ for $x>B$. Starting from any $P$, write
$P=mQ+R_i$. If $\widehat h_L(P)>B$, then
$\widehat h_L(Q)<\widehat h_L(P)$. Since canonical heights need not be
integral, strict decrease alone is not a termination argument. Instead,
iterate (8.2): after $j$ steps,

$$
\widehat h_L(Q_j)
\leq m^{-2j}\widehat h_L(P)+\frac{C}{1-m^{-2}}. \tag{8.2a}
$$

For sufficiently large $j$ this lies under one fixed bound. Northcott then
gives a finite terminal set. Reversing the finitely many decompositions
expresses $P$ in the subgroup generated by that terminal set and the
$R_i$. This supplies the precise termination missing from a bare
...
on every order-$p$ factor. Thus $T_p-1$ acts as zero on all successive
...
**Proof.** The perfect Fourier pairing for the faithful prime-level Hecke
algebra realizes $\lambda$ in a normalized mod-$p$ cuspidal eigenform $f$.
The relations (9.8), Hecke multiplicativity, and the prime-power recurrence
determine every coefficient $a_m(f)$ with $N\nmid m$: it is the divisor sum
$\sigma_1(m)$ in $k$.

Compare $f$ with the normalized prime-level weight-two Eisenstein Fourier
series $E_N$. Its nonconstant coefficients away from $N$ are the same
divisor sums, and $U_NE_N=E_N$. The integral normalization theorem measures
its cuspidal congruence by the exact integer

$$
n_N=\frac{N-1}{\gcd(N-1,12)}. \tag{9.10}
$$

This formulation includes the denominators at $2$ and $3$; it does not use
the uncorrected constant $(N-1)/12$ as an integral order.

When $p\geq5$, the comparison with $E_N$ can be made by reducing its usual
normalized Fourier expansion directly. In characteristics $2$ and $3$, the
phrase “integral normalization” means the saturated Fourier functional used
in the exact index calculation: residues at the cusps are cleared before
reduction, while the first nonconstant coefficient remains $1$. The losses
of two powers at $2$ and one power at $3$ are exactly the saturation factors.
Thus $E_N$ in the following argument denotes this integral Fourier
functional in the small characteristics, not a rational series reduced
through a forbidden denominator.

For clarity, we spell out the Fourier argument that detects the sign. The
difference $h=f-E_N$ has zero coefficients $a_m(h)$ whenever $N\nmid m$.
If $u=\lambda(U_N)$, the $U_N$ recurrence gives

$$
a_{Nm}(h)=(u-1)\sigma_1(m)\qquad(N\nmid m). \tag{9.11}
$$

A weight-two form of level $N$ whose nonconstant Fourier expansion is
supported on powers $q^{Nm}$ lies in the image of the degeneracy operator
$V_N$ from level $1$. This follows by descending the invariant differential
along the quotient of the Tate curve by its canonical subgroup; the
Fourier expansion principle makes the descended differential unique. But
the level-one cuspidal space of weight two is zero, since $X(1)$ has genus
zero. In characteristics $2$ and $3$ a Hasse-type level-one form can account
for a constant Fourier term, but it still has no positive coefficient.
Consequently $a_N(h)=0$. Equation (9.11) with $m=1$ gives $u=1$.

The homomorphism $\lambda$ now kills every generator of $I$, so it factors
through the exact quotient
$\mathbb T/I\simeq\mathbf Z/n_N\mathbf Z$. A map from this quotient to a
field of characteristic $p$ exists only if $p\mid n_N$. $\square$
...
The descent assertion used in the Fourier proof can be checked on the two
standard charts of $X_0(N)$. On the Tate chart at infinity, support on
$q^{Nm}$ produces a series in $q^N$ and hence a differential pulled back by
$q\mapsto q^N$. At the other cusp, the Fricke transformation and
holomorphy show that the descended differential has no pole. The Fourier
expansion principle then glues it to a level-one form. Its positive
weight-two cuspidal part vanishes because $X(1)$ has genus zero. This proves
the step without assuming the desired sign in advance.
...
The uniqueness in (9.11) is worth emphasizing. Since
...
- Book 31 supplies local Tate duality, flat Cartier duality, Kummer local
  conditions, and the warning that the coefficient-prime condition is not
  ordinary unramified cohomology.
...
- Book 142 fixes the prime-level Hecke algebra including $U_N$, the
  Eisenstein ideal, and the exact quotient
...
The hypothesis matching is exact. Books 31 and 32 allow finite flat
coefficients at the residue characteristic, which is required at $v=\ell$.
