...
$D_{\mathrm{LL}}(\Pi_v)$ in Book 81. Untwisted weight-two curve cohomology
...
94 constructs the packet on the quaternion algebra ramified at every other
real place and exactly at $W$. Book 132 realizes that packet in the $H^1$ of
the compact one-split curve, and Book 134 extracts (1.13). Principal-series,
...
131 one must moreover choose this carrier with $v\notin W$, so that the
...
Book 94 supplies no one-split carrier that is split at $v$. The existence of
...
- the completely split odd and dyadically tensor-split one-step rank-two
  Iwahori places of Book 131, including the Steinberg branch;
...
Equation (1.10) is immediate. This is the special block in an uncentered notation that avoids choosing $q^{1/2}$. Book 81 uses the centered block
...
$|\cdot|^{\pm1}$, Book 81 matches the corresponding centered pair with the
...
in Book 81's admissible pair. Put
...
Book 81 and then shifted as in (1.5a). It consists of one of the following
packages.
...
- $p$ is odd and the complete good-place ledger of Book 130 holds;
...
one-step Iwahori ledger, Book 131 now supplies that equality by its
...
union of Book 131. At $v$, assume either its completely split odd ledger or
...
be Book 131's incoming right-coset operator. It is the raw, unnormalized
...
The centered Book 81 kernel root is $q^{-1/2}u_v$; after the weight-two shift
...
Book 131's bad-prime Eichler--Shimura theorem proves, after away-from-$v$
...
**Proof.** Book 131 gives graph multiplicity one and rank-one raw monodromy.
...
automorphic correspondence beyond the selected theorem of Book 81. None of
...
in the selected range. For Book 81's centered admissible-pair character
$\theta^{\mathrm{cen}}$, put
...
Book 81's rectifier is normalized precisely so that the determinant of the
...
Book 134 defines $\rho_{\Pi,\lambda}$ by global semisimplification and
...
Books 24, 40, 80, 130, and 131 are local or geometric inputs and do not
...
former special-line scalar $(\mathrm B_v)$ is now proved by Book 131 under
...
here. Integral packet lattices and finite-flat quotients belong to Book 138.
...
Weil side. Hence every local pair used by Book 137 is transported as one
...
$(\mathrm B_v)$ in Book 131 and does not prove $(\mathrm R_u)$ outside the
...
3. **Odd or dyadic split-Iwahori special comparison.** Suppose the complete
   packet and local-model ledger of Section 6.2 holds. Then Book 131 proves
...
Part 3 is Theorem 6.1: Book 131's oriented-edge calculation proves the former
...
**Nearby cycles and monodromy (Book 24).** The low-dimensional nearby-cycle
...
**Semistable abelian varieties (Book 40).** The semistable
...
**Weil--Deligne representations (Book 80).** The local parameter theory
...
**Selected local correspondence (Book 81).** This is the source of the three
...
characteristic two. This dependency is indispensable; Book 80 alone names
parameters but does not identify them with automorphic representations.

**Good integral models (Book 130).** The good-model theorem supplies smooth
...
**Semistable Shimura models (Book 131).** The parahoric model theorem supplies
...
**One-split carrier (Books 94 and 132).** Book 94's compact one-split inverse
theorem constructs the quaternionic packet only from an odd nonempty set of
selected special or selected tame-dihedral finite places. Book 132 realizes
that packet on the compact curve, proves multiplicity one and the rank-two
packet factorization, and supplies the good-level support used in Book 131.
...
**Cohomological extraction (Book 134).** The weight-two extraction theorem
...
supplied directly by Books 94 and 132, the raw extraction and determinant by
Book 134, and the cohomological normalization is fixed in Sections 1.1 and
...
$$
\boxed{136\mid24,40,80,81,94,130,131,132,134}.
$$

Book 81 is direct because it supplies the automorphic labels and their
centered normalization; Books 94 and 132 are direct because they supply the
...
| odd or dyadic split-Iwahori special identification | complete Book 131 tensor, packet, support, and raw $U_v$ ledger; $(\mathrm B_v)$ is then a theorem | determinant gives only the square of the kernel scalar |
| split-Iwahori carrier at $v$ | eligible odd nonempty $W$ for Book 94 with $v\notin W$, then the Book 132 curve packet | an eligible carrier ramified at $v$ does not supply split-Iwahori geometry there |
...
| any selected automorphic label | the matching row of Book 81 | parameter recognition alone is not an automorphic correspondence |
...
are closed under the dependencies above. Book 131 closes the former
...
one-step Iwahori place, Book 131 proves graph multiplicity one and identifies
...
by Book 137. This is the proved good and split-Iwahori special comparison,
