...
6. On every actual primary and companion shadow, $({\rm TYPE}_w)$ holds, meaning $({\rm TPE}_w)$ through $({\rm TIC}_w)$ together with $({\rm TIH}_{\mathscr F})$. The type lattices are finite free and perfectly paired at base, mixed, and Taylor--Wiles levels; every one-prime degeneracy and type map and its two-term complex has the asserted saturation, exactness, and coefficient-algebra Tor control. No several-place mixed exactness is included in this clause; that is part of the route in item 8. The package also includes $({\rm PRI}_{\rm type})$ whenever an extreme coefficient-one residue is used, and $({\rm NU}_{\rm type})$ together with Book 140's node-uniformization hypothesis whenever a carrier is switched to the definite side. Constant generic support and common-kernel equality are then conclusions of Book 140 from the corrected filtration and Book 125; under the preceding $({\rm AIH}_{\mathscr F})$, the displayed type and primitive-residue inputs, and Book 125 routing, the typed analogues are conclusions of Book 174.
7. Every one-prime face has the five-clause boundary-and-depth datum $({\rm SUP}_w)$, including Book 141's split-regular detector input in the marked finite shadows, and the finite-carrier reciprocity package $({\rm REC}_w)$. Thus the faithful Hecke order is generated only by verified trace and structural operators, is torsion-free with reduced generic fiber, and carries a genuine two-dimensional Galois representation of determinant $\chi_q$ whose Artinian specializations satisfy the same finite-flat and prime-to-$q$ local component quotients as $r_{A,q}$.
8. One exact Book 176 route is exhibited: either $P_{\mathrm{act}}$ is a singleton and the applicable one-prime conclusion above holds; or an ordered finite chain is given and every applicable one-prime hypothesis is verified anew on each intermediate carrier, with all previously changed places retained in the actual spectator datum; or the simultaneous route satisfies $({\rm FACE}_{P_{\mathrm{act}}})$, $({\rm CUBE}_{P_{\mathrm{act}}})$, $({\rm MIX}_{P_{\mathrm{act}}})$, $({\rm PROD}_{P_{\mathrm{act}}})$, $({\rm JSUP}_{P_{\mathrm{act}}})$, $({\rm AUG}_{P_{\mathrm{act}}})$, and $({\rm REC}_{P_{\mathrm{act}}})$ of Book 176, Theorem 9.1.
9. Every scalar step has the effective global ray quotient and strict determinant-square twist identities $({\rm RAY}_w)$. Whenever an auxiliary patch is descended, the coherent auxiliary datum $({\rm AUX}_Q)$ supplies exact augmentation for represented rings, modules, pairings, complexes, and deformation actions. Exact auxiliary acting images follow only after the applicable full base $R=T$ theorem and are not an independent input to the direct reduced finite-level comparison. In the simultaneous route the stronger datum $({\rm AUG}_{P_{\mathrm{act}}})$, including order-independent fixed-prime and Taylor--Wiles augmentation and the required joint ray quotient at the scalar places, is used instead; the ray clause is vacuous when no scalar place occurs.
...
The proof mechanism is exact. In the simultaneous case, the reduced comparison makes (8.5) factor through $\mathbb T_q^{P_{\mathrm{act}}}$; in a chain the analogous factorization is made successively through each one-place faithful order. A full scheme-theoretic $R=T$ or reducedness of the deformation source is not required, because a characteristic-zero point over a domain kills the nilradical. Specializing the Hecke-valued Galois representation gives the same traces and determinants as $r_{A,q}$ at good Frobenius elements. Chebotarev and Brauer--Nesbitt identify the rational representations. Absolute irreducibility of the common residual member makes their stable lattices homothetic, so the selected integral local conditions agree.
...
6. A post-specialization cleanability record for the actual set $P_{\mathrm{act}}$, clause by clause as in Section 8.3, together with one of its exact one-prime, validated-chain, or simultaneous Book 176 lifting routes.
...
**Proof.** Chapters 3--6 construct $F'$ and $A/F'$ with the two framed torsion identifications. Section 7.1 identifies $A[\mathfrak q]$ with the induced residual seed, while Sections 3.3 and 8.2 and the cleanability record place that seed in the exact selected automorphic localization. The post-specialization record applies the appropriate reduced comparison of Book 176—one-prime, successively along the validated chain, or finite-set—to identify the $q$-adic Tate factor of $A$ with a cuspidal packet $\Pi_A$ on the component containing that factor. If this requires a further field $E$, Section 8.6 repeats the residual seed there and sets $F^{\mathrm{pm}}=E$; otherwise set $F^{\mathrm{pm}}=F'$.
...
The definition in Section 10.1 is not presently known to be inhabited for the FLT applications. The first failure in the declared proof dependencies occurs inside the cleanability clause. Wherever the actual comparison invokes Book 139's corrected constant-coefficient filtration, Book 176 requires the finite family of **abelian Ihara conditions**
...
Later diagnostic manuscripts and retired unitary-surface proposals are not dependencies and cannot be used to discharge these obligations. The exact first constant-row gap is (10.4); after it, the construction of one global cube, (10.5) and all higher mixed exactness, the product-residue comparison, joint support, coherent joint ring--complex--module--action--pairing augmentation, and finite-set reciprocity obstruct the actual multi-place set $P_{\mathrm{act}}$ unless a validated one-prime chain is exhibited. Fixed-prime faithful-order augmentation is formal once a strict product twist exists, while auxiliary acting-image injectivity follows only after full base $R=T$.
...
The conditional bridge is mathematically explicit, but the advertised FLT-range existence theorem is **NOT READY**. Its first unresolved constant-row dependency is the localized abelian Ihara family (10.4), retained but not proved by Books 139 and 176. Even at one active place, the applicable typed parahoric extension, type-Ihara, support, reciprocity, ray, auxiliary-augmentation, primitive-residue, clean-lowering, and carrier-comparison hypotheses must be verified for the actual localization. For the actual multi-place set $P_{\mathrm{act}}$, one additionally needs the global cube, mixed exactness beginning with (10.5), product-residue comparison, joint component support, coherent joint ring--complex--module--action--pairing augmentation, and finite-set reciprocity, unless a validated one-prime chain is verified. Fixed-prime faithful-order augmentation is formal once a strict product twist exists; auxiliary acting-image injectivity follows only after full base $R=T$ and is not needed for the direct reduced finite-level comparison. Complete splitting already creates several active places above $v_0$ when $[F':F]>1$; the problem is not confined to newly acquired bad reduction.
