...
5. Every use of a two-degeneracy map satisfies Book 139's abelian Ihara vanishing, neat or prime-to-$q$ stabilizer descent, saturation, component-group control, and the independent normalized-branch Smith correction.
...
6. A post-specialization cleanability record for the actual set $P_{\mathrm{act}}$, clause by clause as in Section 8.3, and a literal application or validated chain of Book 176, Theorem 7.1.
...
be continuous, odd, absolutely irreducible, and of cyclotomic determinant. Assume $\bar\rho|_{G_{F(\zeta_\ell)}}$ is absolutely irreducible, the precise adjoint-invariant hypothesis of the selected Book 176 theorem holds, and the restrictions at places above $\ell$ are supplied by the finite-flat paired group schemes of Section 2.3. If $\bar\rho$ admits a lifting-admissible paired specialization, then there are
...
**Proof.** Chapters 3--6 construct $F'$ and $A/F'$ with the two framed torsion identifications. Section 7.1 identifies $A[\mathfrak q]$ with the induced residual seed, while Sections 3.3 and 8.2 and the cleanability record place that seed in the exact selected automorphic localization. The post-specialization record lets Book 176, Theorem 7.1, directly or through the validated chain, identify the $q$-adic Tate factor of $A$ with a cuspidal packet $\Pi_A$ on the component containing that factor. If this requires a further field $E$, Section 8.6 repeats the residual seed there and sets $F^{\mathrm{pm}}=E$; otherwise set $F^{\mathrm{pm}}=F'$.
...
The definition in Section 10.1 is not presently known to be inhabited for the FLT applications. The first failure in the declared proof dependencies occurs inside the cleanability clause. Book 176 uses Book 139's degeneracy-map theorem only under the **abelian Ihara condition**
...
Writing the active places as $v_1,\ldots,v_r$ does not allow Theorem 7.1 of Book 176 to be applied $r$ times. After the first change, the automorphic lattice, companion lattice, faithful Hecke order, and spectator components have changed. The next step is valid only after every hypothesis of that theorem has been reverified on the intermediate carrier. Book 176 proves no simultaneous finite-set alternative.
...
| Books 122, 125--129, and 168, assembled in Book 127 | compact-curve packet decomposition, rank-two extraction, determinant and Frobenius normalization, the weak family, and stable lattices | no surface realization or density theorem; integral finite-flat comparison is obtained from $A$, not from an arbitrary curve lattice |
| Book 176 | conditional Theorem 7.1 for one active place, including the generic conductor-one special face used here, and its exact finite-set boundary | the abelian Ihara, integral comparison, support, reducedness, faithful-order, and augmentation hypotheses are retained; no simultaneous finite-set theorem |
...
$$
104,\ 98,\ 127,\ 176,\ 153,\ 157,\ 167. \tag{12.0}
$$
