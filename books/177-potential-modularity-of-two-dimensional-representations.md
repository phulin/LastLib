...
6. On every actual primary and companion shadow, $({\rm TYPE}_w)$ holds, meaning both $({\rm TIC}_w)$ and $({\rm TIH}_{\mathscr F})$. The type lattices are finite free and perfectly paired at base, mixed, and Taylor--Wiles levels; all degeneracy and type maps and total complexes have the asserted saturation, exactness, and higher-Tor control. The package also includes $({\rm PRI}_{\rm type})$ whenever an extreme coefficient-one residue is used, and $({\rm NU}_{\rm type})$ together with Book 140's node-uniformization, generic-support, and common-kernel hypotheses whenever a carrier is switched to the definite side.
...
9. Every scalar step has the effective global ray quotient and strict determinant-square twist identities $({\rm RAY}_w)$. Whenever an auxiliary patch is descended, $({\rm AUG}_Q)$ is exact for represented rings, modules, pairings, complexes, deformation actions, and faithful acting images. In the simultaneous route the stronger $({\rm AUG}_{P_{\mathrm{act}}})$, including order-independent fixed-prime and Taylor--Wiles augmentation and the required joint ray quotient, is used instead.
...
Later diagnostic manuscripts and retired unitary-surface proposals are not dependencies and cannot be used to discharge these obligations. The exact first constant-row gap is (10.4); after it, the construction of one global cube, (10.5) and all higher mixed exactness, the product-residue comparison, joint support, simultaneous exact faithful-order augmentation, and finite-set reciprocity obstruct the actual multi-place set $P_{\mathrm{act}}$ unless a validated one-prime chain is exhibited.
...
The conditional bridge is mathematically explicit, but the advertised FLT-range existence theorem is **NOT READY**. Its first unresolved constant-row dependency is the localized abelian Ihara family (10.4), retained but not proved by Books 139 and 176. Even at one active place, the applicable type incidence and type-Ihara, support, reciprocity, ray, auxiliary-augmentation, primitive-residue, clean-lowering, and carrier-comparison hypotheses must be verified for the actual localization. For the actual multi-place set $P_{\mathrm{act}}$, one additionally needs the global cube, mixed exactness beginning with (10.5), product-residue comparison, joint component support, simultaneous primary/companion faithful-order augmentation, and finite-set reciprocity, unless a validated one-prime chain is verified. Complete splitting already creates several active places above $v_0$ when $[F':F]>1$; the problem is not confined to newly acquired bad reduction.
