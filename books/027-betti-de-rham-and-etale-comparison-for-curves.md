...
Books 9 and 52 identify this with $\dim_KH^1(X,\mathcal O_X)$ and show that $H^1_{\mathrm{dR}}(X/K)$ has dimension $2g$.
...
acts from $X$ to $Y$ by $(p_2)_*p_1^*$. This is the convention already fixed for de Rham and étale cohomology in Books 52 and 20.
...
Second, Book 52 normalizes the algebraic de Rham trace so that the de Rham class of a degree-one point has trace $1$. Under unnormalized integration that point class maps to $2\pi i$ times the Betti orientation class. Consequently
...
If $z=x+iy$ and $w=u+iv$ are overlapping holomorphic coordinates, the real Jacobian determinant of $w$ as a function of $z$ is $|dw/dz|^2>0$. Hence all holomorphic charts induce the same orientation. A triangulation whose two-simplices have this orientation has boundary cancellation along every internal edge. The sum of its oriented two-simplices is a cycle, and its class

$$
[X]\in H_2(X^{\mathrm{an}},\mathbf Z)
$$

is independent of the oriented triangulation by common subdivision. Local homology at every point is $\mathbf Z$, and the chosen class maps to its positive generator. This characterizes $[X]$ uniquely.

Connectedness gives $H_0(X^{\mathrm{an}},\mathbf Z)=\mathbf Z$. Poincaré duality then gives $H_2(X^{\mathrm{an}},\mathbf Z)=\mathbf Z[X]$. There is no homology above degree two because a triangulation has no higher cells.
...
Removing $r$ points replaces the top cell by $r$ boundary circles with one relation and gives $\chi_c=2-2g-r$, matching the étale calculation of Book 20.
...
Now let $X/K$ with $K\subseteq\mathbf C$. Book 52 proves arbitrary base change for de Rham cohomology of smooth proper curves, so
...
Under de Rham comparison, Book 52's Hodge exact sequence becomes
...
after identifying the top Tate twist with the constant coefficient over $\mathbf C$. The groups vanish above degree two. This recovers the computation of Book 20 from topology, but the comparison proof did not use the rank-$2g$ étale computation; it used only affine cohomological dimension and torsor classification, so there is no circularity.
...
Finite comparison becomes ℓ-adic comparison only after controlling inverse limits. On proper smooth curves the integral Betti groups are free and finite, while the finite étale groups satisfy compatible surjective reduction. These facts eliminate the derived-limit ambiguity emphasized in Book 20.
...
On the étale side, Book 20 defines continuous cohomology by a derived inverse limit and supplies the Milnor sequence
...
Equation (10.2) shows that finite comparison intertwines (10.7) with Book 20's étale trace
...
and Book 52 proves that it is perfect alternating with the Hodge subspace Lagrangian. Equation (10.6) states the precise comparison:
...
A single curve gives a static comparison. In a smooth family, cycles move locally constantly while differential forms vary holomorphically. Their period integrals therefore turn the topological local system into the horizontal sections of the Gauss--Manin connection. This explains the geometric meaning of the connection constructed algebraically in Book 52.
...
Book 52 constructs the algebraic bundle
...
analytifies to the holomorphic subbundle with fiber $H^{1,0}(X_s)$. It is generally not horizontal. Book 52 proves Griffiths transversality
