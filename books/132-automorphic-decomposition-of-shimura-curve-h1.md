...
The compact one-split inverse theorem is now available in one exact range. Suppose $[F:\mathbf Q]$ is even, fix the active real place $\tau_0$, and let $W$ be an odd nonempty finite set. If a cuspidal full-group parallel-weight-two representation $\pi$ of $\operatorname{GL}_2(\mathbf A_F)$ has the compatible unitary central character and is selected special or selected depth-zero or tame positive-depth dihedral at every $w\in W$, Book 94, Theorem 9.2 constructs the unique noncharacter representation $\Pi_B$ on the quaternion algebra ramified at every real place except $\tau_0$ and exactly at $W$. It preserves multiplicity, central character, and every good right-convolution Hecke polynomial; under Book 94's additional algebraicity hypothesis it also preserves the good Hecke field, field of rationality, and coefficient conjugation.
