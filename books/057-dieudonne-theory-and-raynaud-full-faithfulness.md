...
Suppose $\mathcal A/S$ is an abelian scheme over a divided-power lifting $S$ of $k$ to which the finite-level comparison theorem of Book 54 applies. Crystalline base change then identifies
...
Indeed a mixed monomial in the reduced coproduct has weight one. Its total degree is congruent to $1$ modulo $p-1$, so degrees $2,\ldots,p-1$ cannot occur. Terms of degree greater than $p$ lie in higher Rees degree and disappear from the first slice. The universal polynomial $C_p$ is primitive in the sense that one of its coefficients is a unit, as proved in the Oort--Tate calculation of Book 56.
