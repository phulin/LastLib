...
**Lemma 6.0 (diagonal coevaluation).** Let $X$ be separated of finite type of dimension at most one over a separably closed field, let $K\in D_c^b(X,E)$, and put $V=R\Gamma_c(X,K)$, so that Book 20, Section 3.4 gives $V^\vee=R\operatorname{Hom}_E(V,E)\simeq R\Gamma(X,D_XK)$. Then:
...
**Standing Input 6.0 (extraordinary base change along the fixed-point square).** _For the cartesian square_
...
_Consumer analysis._ Standing Input 6.0 is consumed only in the definition of the trace class immediately below, in Theorem 6.1, and through them in Proposition 7.1 and Chapter 10; every one of those consumers operates on curves with properly supported correspondences, so the recorded two-proper-legs reduction covers each use.

Granting Standing Input 6.0, the lifted class lands in $R\Gamma(\operatorname{Fix}(c),\delta^!\omega_C^\bullet)=R\Gamma(\operatorname{Fix}(c),b^!\omega_X^\bullet)$, the identification being transitivity of $f^!$ applied to $b=c_2\circ\delta$ together with $c_2^!\omega_X^\bullet=\omega_C^\bullet$ (Book 20, Section 3.3). Composing with the further transitivity identification $b^!\omega_X^\bullet\simeq a^!E$ supplied by the structure map $X\to\operatorname{Spec}\bar k$, the kernel pairing (6.1) pulled back along $\delta$ therefore yields finally a class
...
Both assertions are consequences of Lemma 6.0(c) and Standing Input 6.0 rather than separate principles: the trace-class construction is an additive functor of $K$ on localization triangles, so a class on $X$ restricts to classes on $U$ and $Z$ whose pushforwards sum to it, and conversely classes on the pieces glue over an open--closed decomposition because $H^0$ of a disjoint coproduct of supports is the direct product of the pieces. No uniqueness-by fiat is used anywhere: every extension step is the five-lemma induction over an explicit constructible filtration, exactly as in the proof of Lemma 6.0(c).

### 6.4 Proof of the trace theorem

The proof now has one task: show that categorical evaluation of the global operator becomes geometric evaluation after pulling the kernel to the diagonal. We keep Künneth, duality, and proper trace visible because each accounts for one hypothesis in the statement. One input is consumed under an explicit label: Standing Input 6.0, which is a theorem whenever both legs are proper and is otherwise recorded, not proved.
