...
**Proof strategy.** Write the rational fifth power in lowest terms. If one rational prime occurs to an exponent not divisible by five, the equation $\beta^5=a$ forces ramification index divisible by five in every field containing $\beta$. Such a field cannot be a subfield of the degree-four field $K$.
...
Put $a=\beta^5\in\mathbf Q^\times$, and write the prime factorization of $a$ with possibly negative integer exponents. If some exponent $m=v_q(a)$ is not divisible by five, let $w$ be a normalized valuation of $\mathbf Q(\beta)$ above $q$, with ramification index $e$. Then

$$
5w(\beta)=w(a)=e\,m.
$$

It follows that $5\mid e$, so $5$ divides $[\mathbf Q(\beta):\mathbf Q]$. But $\mathbf Q(\beta)\subseteq K$, so this degree also divides four, a contradiction. Therefore every prime exponent of $a$ is divisible by five. Since the exponent is odd, the sign causes no obstruction, and $a=c^5$ for some $c\in\mathbf Q^\times$. This is exactly (4.7).
...
If $5\nmid v_{\mathfrak q}(a)$, equation (8.2) forces $5\mid e$. Since $[L:K]=5$, we have $e=5$.

Now suppose $5\mid v_{\mathfrak q}(a)$. Because $\mathfrak q$ is principal, changing $a$ by a fifth power lets us arrange $v_{\mathfrak q}(a)=0$. Over the DVR $\mathcal O_{\mathfrak q}$, the algebra

$$
\mathcal O_{\mathfrak q}[T]/(T^5-a)
$$

has derivative $5T^4$. The image of $T$ is a unit, and $5$ is a unit because $\mathfrak q\nmid5$. Thus the polynomial and its derivative are relatively prime after reduction. The resulting finite algebra is unramified; localizing at any of its field factors gives ramification index one. $\square$
...
In particular, if all prime-ideal valuations of $a$ are divisible by five and $a$ is rational modulo $\lambda^3$, then Theorem 7.3 says the Kummer extension is trivial. This is stronger than merely saying it is unramified away from $\mathfrak p$.
...
For the rational factor, if $t=v_5(x+y)$, the normalized integer
...
Examples clarify the scope. With $x=y=1$, the pair is primitive and $5\nmid x+y$. The four nonrational factors $1+\zeta^i$ are units, and all five factors are pairwise coprime. With $x=1,y=4$, every nonrational factor $1+4\zeta^i$ has exact $\mathfrak p$-value one, while the rational factor $5$ has value four. Dividing the nonrational factors by their chosen uniformizers leaves pairwise coprime $\mathfrak p$-units.

By contrast, $x=y=5$ violates primitivity. Common rational content introduces common primes throughout the factorization. Cancelling $\gcd(x,y)$ before entering $\mathcal O$ is therefore a mathematical step, not a cosmetic convention.

There is a subtler warning. Pairwise coprimality of the ideals $(F_i)$ does not imply pairwise coprimality of their rational norms. Distinct prime ideals over one split rational prime may divide different $F_i$, causing the norms to share that rational prime. The ideal statement, not a statement about norm gcds, is what permits fifth-power extraction.
...
**Corollary 10.4 (norm extraction).** Under the primitive equation (10.2):
...
**Corollary 10.5 (rational fifth-power shapes).** Under (10.2):
