...
The result has a second role. A local deformation condition is usually a subgroup or subspace of $H^1(K,M)$. Duality assigns to it an exact orthogonal condition in $H^1(K,M^*(1))$. Books 18 and 19 will assemble such local conditions globally. We stop before that assembly: every statement here concerns one local field.
...
Combining this with the Euler characteristic from Book 16 yields
