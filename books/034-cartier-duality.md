...
Constant and diagonalizable groups were introduced in Books 27--28 as opposite standard families. Cartier duality explains that opposition. Let $M$ be a finite abelian group. Over $S=\operatorname{Spec}R$, write
...
Duality should turn a quotient into a subgroup and a subgroup into a quotient. In the finite-flat category, those words have the precise meaning fixed in Book 28. A sequence
...
The exact Hopf-algebra criterion from Book 27 says that (7.3)--(7.4) satisfy the hypotheses of the lemma. In particular, the transpose
...
If $G$ is killed by a positive integer $n$, then so is $G^D$. More generally, exactly the same integers annihilate $G$ and $G^D$, because $[n]_{G^D}=0$ dualizes back to $[n]_G=0$ under biduality. Thus they have the same exponent whenever that terminology is being used. The primary decomposition of Book 28 is preserved:
...
Let $k$ be a perfect field. Book 28 constructed
...
Let $R$ be a henselian local ring with perfect residue field $k$. For $G/R$ finite locally free and commutative, Book 28 gives
...
Cartier duality is a theory of finite affine groups, whereas an abelian scheme $A/S$ is proper and positive-dimensional. The connection is through multiplication kernels. For every $n\ge1$, Book 28 established that
...
In characteristic $p$, consider the Frobenius sequence from Book 28,
