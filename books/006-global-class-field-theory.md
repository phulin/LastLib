...
We instead realize the fundamental class from the reciprocity and norm
theorems just proved. This is a finite-group construction and uses no
absolute high-degree vanishing.

For a subgroup $H\subseteq G$, put $E=L^H$. Norm limitation (4.F9) and
pre-class-formation reciprocity give a canonical isomorphism

$$
\theta_H:H^{\rm ab}\xrightarrow{\sim}
 C_E/N_{L/E}C_L.                                         \tag{4.G1}
$$

The local norm/restriction and scalar-inclusion/transfer squares show that
the family $(\theta_H)$ commutes with restriction, transfer, conjugation,
and passage through chains of subgroups.

We use the following finite-group realization lemma.

**Connecting-class realization lemma.** Let $C$ be a $G$-module with
$H^1(H,C)=0$ for every $H\subseteq G$. Suppose compatible isomorphisms

$$
\theta_H:\widehat H^{-2}(H,\mathbf Z)=H^{\rm ab}
 \xrightarrow{\sim}\widehat H^0(H,C)
                                                               \tag{4.G2}
$$

are given for every subgroup. Then there is a class $u\in H^2(G,C)$ whose
two-extension connecting map in degree $-2$, after restriction to $H$, is
$\theta_H$. Cap product with $\operatorname {res}u$ is then an isomorphism
in every Tate degree and for every subgroup.

**Proof.** Take $X=G\setminus\{1\}$ and the standard relation-module
presentation

$$
0\longrightarrow K\longrightarrow
 P_0=\bigoplus_{x\in X}\mathbf Z[G]e_x
 \xrightarrow{e_x\mapsto x-1} I_G\longrightarrow0,     \tag{4.G3}
$$

where $I_G$ is the augmentation ideal. The module $K$ is generated by the
bar relations

$$
e_g+g e_h-e_{gh},\qquad
(1+h+\cdots+h^{m-1})e_h\quad(h^m=1),                   \tag{4.G4}
$$

after words in $X$ have been substituted for $g,h$. Choose representatives
in $C^H$ of the norm classes $\theta_H([h])$ for every cyclic subgroup
$H=\langle h\rangle$. Send the second relation in (4.G4) to that
representative. The multiplication relation is sent to the difference
between the representatives for $g$, $h$, and $gh$, transported by the
displayed prefixes. When this difference is specified only modulo an
$H$-norm, choose a norm preimage and add its bar boundary. Two choices differ
by a degree-one $H$-cocycle and hence by a boundary because $H^1(H,C)=0$.

Associativity, conjugation, and power relations among (4.G4) are exactly the
restriction, conjugation, and transfer identities for the family
$(\theta_H)$. Hence these assignments define a $G$-map
$\phi:K\to C$. Changing a representative by a norm changes $\phi$ by a
map extending to $P_0$; therefore it does not change the following Yoneda
class. Push out (4.G3) along $\phi$ and splice with
$0\to I_G\to\mathbf Z[G]\to\mathbf Z\to0$. This gives an actual
two-extension

$$
0\longrightarrow C\longrightarrow A
 \longrightarrow\mathbf Z[G]\longrightarrow\mathbf Z\longrightarrow0.
                                                               \tag{4.G5}
$$

The bar boundary calculation on the two generators in (4.G4) says that its
degree-$-2$ connecting map over $H$ is precisely $\theta_H$. Its
degree-$-1$ connecting map is also an isomorphism, because both sides are
zero:
$\widehat H^{-1}(H,\mathbf Z)=0$ and $H^1(H,C)=0$.
The two long exact sequences of (4.G5) therefore give two consecutive
vanishings for the middle module $A$, for every subgroup. Book 5, (5.5),
then makes $A$ cohomologically trivial on every subgroup; the other middle
module is induced. All connecting maps of (4.G5) are consequently
isomorphisms. They are cap products with its Yoneda class $u$, proving the
lemma. $\square$

For $C=C_L$, the required $H^1$-vanishing is (4.I9s), and (4.G1) supplies
the compatible family. Let

$$
u_{L/F}\in H^2(G,C_L)                                    \tag{4.G6}
$$

be the resulting class. The realization proof gives, before any canonical
$S$-unit sequence is chosen,

$$
\widehat H^{r-2}(H,\mathbf Z)
 \xrightarrow{\ \frown\operatorname {res}u_{L/F}\ }\widehat H^r(H,C_L)                                      \tag{4.G16}
$$

for every $H\subseteq G$ and every $r\in\mathbf Z$. In degree zero this
identifies
...
\qquad
\operatorname {inv}_{L/E}(\operatorname {res}u_{L/F})=1/|H|.
                                                               \tag{4.G10}
$$

The coset formulas used in the compatibility of $\theta_H$ give
...
For a Galois tower $M/L/F$, the same scalar-inclusion/transfer calculation
on the relation modules gives

$$
\operatorname {inf}_{L/F}^{M/F}u_{L/F}=[M:L]u_{M/F}.     \tag{4.G13}
$$

All these identities are literal identities of the pushout two-extensions,
up to the bar homotopy obtained by changing the chosen norm representatives.
...
is $D_w$-equivariant. The image
$(j_w)_*(u_{L_w/F_v})$ has degree-$-2$ connecting map equal to local
reciprocity followed by the $w$-coordinate map into
$C_{L^{D_w}}/N_{L/L^{D_w}}C_L$. The same is true for
$\operatorname {res}^G_{D_w}u_{L/F}$, because $\theta_{D_w}$ is the inverse
global Artin map and the pre-class-formation reciprocity theorem was proved
as the product of the local maps. The relation-module calculation
(4.G3)--(4.G5), restricted to $D_w$, therefore gives the actual equality
