...
Before stating the archimedean values in the invariant sequence, we compute
them. Nothing in the cyclic Tate cohomology of Book 5 §3.3 used the
nonarchimedean hypothesis; it applies to any finite cyclic extension, including
$\mathbf C/\mathbf R$.

**Archimedean local Brauer groups.** $\operatorname{Br}(\mathbf C)=0$ and
$\operatorname{Br}(\mathbf R)=\frac12\mathbf Z/\mathbf Z$.

**Proof.** Since $\mathbf C$ is algebraically closed,
$\operatorname{Gal}(\mathbf C/\mathbf C)$ is trivial and
$\operatorname{Br}(\mathbf C)=H^2(1,\mathbf C^\times)=0$.

Because $\mathbf C$ is already the algebraic closure of $\mathbf R$,
$\operatorname{Br}(\mathbf R)=H^2(\operatorname{Gal}(\mathbf C/\mathbf R),\mathbf C^\times)$.
The extension $\mathbf C/\mathbf R$ is cyclic of order two, generated by
complex conjugation $\sigma$, so the cyclic Tate cohomology identity of Book 5
§3.3,
...
applies with $L=\mathbf C$, $K=\mathbf R$, $G=\{1,\sigma\}$. The norm is
$N_{\mathbf C/\mathbf R}(z)=z\sigma(z)=|z|^2$, whose image is
$\mathbf R_{>0}$, so

$$
\operatorname{Br}(\mathbf R)\cong\mathbf R^\times/\mathbf R_{>0}\cong\{\pm1\},
$$

a group of order two. Its unique nontrivial element must correspond to the
unique nonzero element of order dividing two in $\mathbf Q/\mathbf Z$, namely
$\frac12$, which fixes $\operatorname{Br}(\mathbf R)=\frac12\mathbf Z/\mathbf Z$
as the target of $\operatorname{inv}_v$ at a real place $v$. $\square$
...
Here $\operatorname{Br}(\mathbf C)=0$ and $\operatorname{Br}(\mathbf R)=\frac12\mathbf Z/\mathbf Z$ by the preceding lemma, and $\operatorname{Br}(F_u)\cong\mathbf Q/\mathbf Z$ at a finite place by the local invariant map of Book 5 §4.2–4.5. Only finitely many localizations of a global class are nonzero.
...
$$
R_n=\mathbf Z/n\mathbf Z,
\qquad A_n=R_n,
\qquad A_n^D=\mu_n,
\qquad D_n=(\mathbf Q/\mathbf Z)[n]=\tfrac1n\mathbf Z/\mathbf Z.
$$

The dual used below is the derived $R_n$-linear dual. It is not the
ordinary $\mathbf Z$-linear dual with the finite group $D_n$ incorrectly
treated as injective. We record the coefficient model explicitly. Every
ideal of $R_n$ is generated by a divisor $d$ of $n$. If
$f:(d)\to D_n$ is $R_n$-linear, then $z=f(d)$ is killed by $n/d$;
conversely every such $z$ is $dy$ for some $y\in D_n$. Sending $1$ to
$y$ extends $f$. Baer's criterion therefore shows that $D_n$ is injective
as an $R_n$-module. Thus

$$
0\longrightarrow D_n\xrightarrow{\,1\,}D_n\longrightarrow0             \tag{4.D0}
$$

is the injective resolution used for $R_n$-complexes. For comparison, as
a $\mathbf Z$-module $D_n$ has the two-term injective resolution

$$
0\longrightarrow D_n\longrightarrow\mathbf Q/\mathbf Z
 \xrightarrow{\,n\,}\mathbf Q/\mathbf Z\longrightarrow0.              \tag{4.D0a}
$$

Collapsing (4.D0a) to its first term would be wrong. All the Galois, local,
and compact-support complexes below are complexes of $R_n$-modules, and we
use

$$
\mathbb D_n(K)^q=\operatorname {Hom}_{R_n}(K^{-q},D_n),
\qquad d\phi=(-1)^{q+1}\phi d.                           \tag{4.D0b}
$$

By (4.D0), this is the genuine derived dual
$R\!\operatorname {Hom}_{R_n}(K,D_n)$. If $Q$ is acyclic then
$\mathbb D_n(Q)$ is acyclic: apply exactness of
$\operatorname {Hom}_{R_n}(-,D_n)$ to the cycle--boundary short exact
sequences. Thus preservation of quasi-isomorphisms below is a chain-level
consequence of injectivity, not an inference from cohomology orders.
...
Because $D_n$ is injective over $R_n$, it extends
along $Z_v^2/B_v^2\subset C_v^2/B_v^2$. Fix one such $R_n$-linear
extension and denote the resulting homomorphism by
...
is well defined. Extend it, again by $R_n$-injectivity, to an $R_n$-linear
homomorphism
...
\Phi_{S,T}:\mathcal P^c_{S,T}\longrightarrow
 \mathbb D_n(\mathcal P_{S,T})[-3].                                  \tag{4.F4s}
$$
...
other. To see surjectivity without comparing orders, reduce the two exact
divisor rows (4.F1) modulo $n$ and place their $D_n$-duals below them. The
vertical maps are valuation and the actual local-symbol matrix. The nonzero
Smith blocks (4.F4t) identify the corresponding kernel--cokernel sequences,
and (4.F2) makes the remaining square commute. The snake lemma therefore
identifies the zero-column kernel with the full annihilator of the outside
valuation relations. This is an exact matrix calculation; no residual
quotient is killed by a cardinality formula.
...
We use only the finite-filtration comparison lemma: if the associated
graded pieces of the cone of a filtered map are acyclic in a fixed finite
...
\operatorname {cor}^G_Hu_{L/L^H}=[G:H]u_{L/F},           \tag{4.G12}
$$

and, in a Galois tower $M/L/F$,
...
$$
h_w\in\operatorname {Hom}_{\mathbf Z[D_w]}(P_1(D_w),C_L),
...
sequence. For each orbit in $S_L$, choose $w\mid v$ and a normalized local
cocycle
$z_w\in\operatorname {Hom}_{\mathbf Z[D_w]}(P_2(D_w),L_w^\times)$.
Set

$$
P_Y=\bigoplus_{v\in S_K}\operatorname {Ind}_{D_w}^G P_\bullet(D_w).
$$

Its induced augmentation is a projective resolution of $Y_S$. Include the
left endpoint of every $z_w$ in its coordinate of $I_{L,S}$ and sum. This
gives a degree-two cochain map $F_Y:P_Y\to I_{L,S}[2]$ whose Ext class is

$$
\ell_S:Y_S\longrightarrow I_{L,S}[2]                                  \tag{4.T2}
$$

Its restriction to the summand $\mathbf Z[G/D_w]$, under explicit Shapiro
evaluation at the identity coset, is $z_w$.
...
F_Z=z_{L/K}:P_Z\longrightarrow C_L[2].
$$
...
of degree $d$ has invariant $1/d$. At a real place $\operatorname{Br}(F_v)=\operatorname{Br}(\mathbf R)=\frac12\mathbf Z/\mathbf Z$ (Archimedean local Brauer groups, §4.2), so $\operatorname{inv}_v$ is the unique isomorphism onto that subgroup of $\mathbf Q/\mathbf Z$. At a complex place the Brauer group is zero. Thus the same target may be used everywhere, but the image depends on the place.
...
and the two invariant formulas agree. At $\mathbf R$ and $\mathbf C$ both formulas hold trivially: since $\operatorname{Br}(\mathbf C)=0$ (Archimedean local Brauer groups, §4.2), every $\beta\in\operatorname{Br}(\mathbf C)$ is zero and every $\operatorname{res}_{\mathbf C/\mathbf R}\alpha$ is zero, so (4.5) reads $0=0$; and (4.4) reads $0=2\operatorname{inv}_v(\alpha)$, which holds because $\operatorname{inv}_v(\alpha)\in\frac12\mathbf Z/\mathbf Z$ has order dividing two. Transitivity of restriction and corestriction proves the formulas in every local tower.
