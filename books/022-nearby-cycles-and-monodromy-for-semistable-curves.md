...
The decisive local input is the computation of these tubes. It is carried out here for the two cases a curve presents, $r=1$ and $r=2$, so that nothing below rests on the informal phrase "tamely equivalent to a torus."

**Lemma 3.1 (smooth-point tube).** Let $x$ be a geometric point of $Y^{\rm sh}$ at which $f^{\rm sh}$ is smooth, and let $\Lambda_n=\mathbf Z/n\mathbf Z$ with $n$ prime to $p$. Then

$$
R\Gamma\bigl((X^{\rm sh}_{(\bar x)})_{\bar K},\Lambda_n\bigr)
\simeq \Lambda_n
$$

concentrated in degree zero, compatibly with further pointed base changes of the trait.

_Proof._ At such a point the strict localization $X^{\rm sh}_{(\bar x)}\to S^{\rm sh}$ is a smooth morphism of traits with a section through $\bar x$. The smooth local-acyclicity theorem of Book 19, Section 8.2 computes the fibered tube of a smooth morphism over a strictly henselian base: adjoining parameters reduces it, one affine-line direction at a time, to the affine-line tube calculation of Book 19, Theorem 7.2.3, giving $H^0=\Lambda_n$, $H^1=0$, with compatibility under pointed base changes. Vanishing in degree two and above holds because the tube is a filtered limit of smooth affine curves over fields with affine transitions, so the Continuity theorem (Book 18, Section 4.6) computes its cohomology as a colimit of their cohomologies, each of which vanishes above degree one by the affine curve theorem of Book 18, Section 6.7. $\square$

**Lemma 3.2 (node tube).** Let $x$ be a geometric node of $Y^{\rm sh}$, so that after a suitable identification the completed strict-local equation is $uv=\pi$, and let $n$ be prime to $p$. The geometric generic tube is canonically identified with $\mathbf G_{m,\bar K}$, with coordinate $u$ and relation $v=\pi/u$, and

$$
R\Gamma(\mathbf G_{m,\bar K},\Lambda_n)
\simeq
\bigl[\,\Lambda_n\xrightarrow{\ 0\ }\Lambda_n(-1)\,\bigr]
$$

placed in degrees zero and one; in particular $H^0=\Lambda_n$, $H^1=\Lambda_n(-1)$ is free of rank one generated by the class dual to the Kummer class $\kappa_u\in H^1(\mathbf G_{m,\bar K},\mu_n)$ of the coordinate, and all higher groups vanish.

_Proof._ On the generic fiber $\pi$ is invertible, so the equation $uv=\pi$ solves as $v=\pi/u$: the tube is $\mathbf G_{m,\bar K}$, and passage through the filtered system defining the strict localization commutes with cohomology by the Continuity theorem, exactly as in Lemma 3.1. Connectedness gives $H^0=\Lambda_n$. Since $\mathbf G_{m,\bar K}$ is a smooth affine curve, the affine curve theorem of Book 18, Section 6.7 kills every $H^q$ for $q\ge2$.

For the first cohomology, apply the Kummer theorem for curves, part 2 (Book 18, Section 6.7), to $U=\mathbf G_{m,\bar K}\subset C=\mathbf P^1_{\bar K}$ with $D=\{0,\infty\}$. The unit group is $\bar K^\times\times u^{\mathbf Z}$, and $\bar K^\times$ is $n$-divisible, so $\mathcal O(U)^\times/n\simeq\mathbf Z/n$ is generated by the class of $u$; moreover $\operatorname{Pic}(U)$ is a quotient of $\operatorname{Pic}(C)\simeq\mathbf Z$ by the subgroup generated by the classes of $0$ and $\infty$, hence zero. The exact sequence therefore identifies $H^1(U,\mu_n)$ with $\mathcal O(U)^\times/n\simeq\mathbf Z/n$, with $\kappa_u$ as generator. Choose once and for all a compatible system of roots of unity $\zeta_n\in\bar K$, $\zeta_{mn}^m=\zeta_n$; the resulting identifications $\mathbf Z/n\simeq\mu_n$ transport the result to constant coefficients and exhibit the displayed two-term complex with zero differential. The twist records the action of $I_K$: an element $\sigma$ fixing the chart acts on the coefficient identification by $\zeta_n\mapsto\zeta_n^{\chi_n(\sigma)}$, where $\chi_n$ is the mod-$n$ tame character, and hence sends the generator of $H^1$ to $\chi_n(\sigma)$ times itself. This is precisely the Tate-twist structure $\Lambda_n(-1)$ of Section 1.2. $\square$

**Remark 3.3 (the fundamental-group presentation and what it is used for).** Book 17, Section 15.2 computes the full prime-to-$p$ geometric fundamental group of the punctured line over an algebraically closed field: normalizing $\mathbf P^1$ in any connected finite étale cover and applying the Riemann--Hurwitz formula shows that every connected cover of degree prime to $p$ is a Kummer cover $u^n=t$, so the Kummer tower is cofinal and $\pi_1^{\,(p')}(\mathbf G_{m,\bar K})\simeq\widehat{\mathbf Z}^{\,(p')}(1)$. This presentation is genuine cofinality, not an additional hypothesis, and it is what licenses the group-theoretic descriptions used in Sections 3.2 and 3.5. What it does not supply, and what is not claimed anywhere in this book, is a general theorem that the punctured line is an étale $K(\pi,1)$ for arbitrary lisse coefficients; the constant-coefficient computations above never pass through such a statement, and the one place where a ramified coefficient is evaluated — Section 3.5 — isolates the comparison it needs as a separate labeled input.
...
**Proof.** The stalk on the left is computed by the strict-local stalk formula (Theorem 2.4), and Lemmas 3.1 and 3.2 evaluate the resulting tube in the two cases a curve presents.

_At a smooth point_, $r=1$: $C_{x,n}=\operatorname{coker}(\Lambda_n\xrightarrow{1\mapsto1}\Lambda_n)=0$, so $\bigwedge^0C_{x,n}=\Lambda_n$ in degree zero and all exterior powers of positive degree vanish; Lemma 3.1 gives exactly this.

_At a node_, $r=2$: Lemma 3.2 identifies the tube with $\mathbf G_{m,\bar K}$ and its cohomology with $[\Lambda_n\to\Lambda_n(-1)]$. Here $C_{x,n}=\operatorname{coker}(\Lambda_n^2\xrightarrow{(a,b)\mapsto a+b}\Lambda_n)$ is free of rank one, generated by the class of $(1,0)$, so $\bigwedge^qC_{x,n}=0$ for $q\ge2$, and (3.2) reduces to the two identifications $R^0\Psi\Lambda_n\simeq\Lambda_n$ and $R^1\Psi\Lambda_n\simeq C_{x,n}(-1)$. For the first, degree-zero stalks are constant: both sides are the pullback of the constant sheaf from the point. For the second, match generators by sending the Kummer-dual generator of $H^1(\mathbf G_{m,\bar K},\Lambda_n(-1))$ to the class of $(1,0)\in C_{x,n}$; under the alternative kernel description of (3.1) this is the antisymmetric relation $[u]+[v]=0$ forced by $uv=\pi$, which is precisely the kernel-versus-cokernel distinction noted after (3.1). Cup products respect the identification because on the two-term complex the product of any two positive-degree classes lands in $H^2=0$, as does $\bigwedge^2C_{x,n}=0$.

The group-theoretic display used elsewhere in the book is a re-description of the same computation, not an additional input. By Remark 3.3 the prime-to-$p$ fundamental group of the node tube is $\widehat{\mathbf Z}^{\,(p')}(1)$ with the Kummer tower cofinal, so at the level where $\ell$-power roots alone act,

$$
G_x=\ker\left[
\mathbf Z_\ell(1)^2\xrightarrow{\mathrm{sum}}\mathbf Z_\ell(1)
\right]\cong\mathbf Z_\ell(1),
$$

because compatible roots must be multiplied to the chosen root of $\pi$; dualizing identifies the character module of $G_x$ with $C_{x,n}(-1)$, and the cochain complex of this procyclic group is the length-one Koszul resolution whose cohomology reproduces Lemma 3.2.

Finally the arithmetic actions. In both cases the action of $I_K$ on the stalk groups factors through the mod-$n$ tame characters: an element fixing the chart acts only on the coefficient field $\bar K$, hence only through the cyclotomic twist recorded in Lemma 3.2. Since wild inertia is the kernel of every tame character, it acts trivially; when $p=1$ there is no wild inertia. Tame deck transformations translate chosen Kummer roots by roots of unity while fixing the degree-one Kummer classes themselves, which is why each individual sheaf $R^q\Psi_f\Lambda_n$ carries the trivial internal action and the monodromy survives only in extensions — the phenomenon quantified in Chapters 4 and 5. $\square$
...
**Theorem 3.4 (full tame strict-node complex).** Relative to the chosen generator, there is a
...
$\mathbf G_{m,\bar K}$, whose prime-to-$p$ fundamental group is the procyclic Kummer group by
Remark 3.3. The passage from that presentation to the two-term complex for a ramified coefficient is the one input of this theorem that neither Remark 3.3 nor Lemma 3.2 supplies, so it is recorded explicitly.

**Standing Input (tame annulus comparison).** _Let $\mathcal L$ be a lisse sheaf of finite free $\Lambda$-modules on $\mathbf G_{m,\bar K}$ whose monodromy factors through the tame quotient $\widehat{\mathbf Z}^{\,(p')}(1)$, with fiber $W_x$ and with $M_x\subseteq W_x$ the invariants of the prime-to-$\ell$ kernel as above. Then the natural comparison map from the continuous cochain complex of $\Gamma_x\simeq\mathbf Z_\ell(1)$ acting on $M_x$ to the étale complex $R\Gamma(\mathbf G_{m,\bar K},\mathcal L)$ is a quasi-isomorphism._

_Provenance._ This is the statement that the punctured line over an algebraically closed field is a tame étale $K(\pi,1)$. Its cofinality half — every tame cover is dominated by a Kummer cover — is proved in Book 17, Section 15.2 and recalled in Remark 3.3; what remains open here is the acyclicity of the universal pro-cover, equivalently the vanishing of the higher cohomology of nonconstant tame local systems pulled from finite Kummer levels. Neither this book nor Books 16 through 20 proves that acyclicity. For constant coefficients the standing input holds unconditionally by Lemma 3.2, which computes $R\Gamma$ directly; the input is consumed only when $T_x-1$ acts nontrivially.

_Consumer analysis._ Within this book the standing input is consumed only by Theorem 3.4, its functoriality clauses (3.8), the two-row filtration built on them in Chapters 6 through 8, and the conductor bookkeeping of Chapter 10 that passes through those formulas. All representation-theoretic conclusions about strictly semistable curves with constant or extended coefficients — including (3.3), Theorem 6.2 in degree one, and the invariant-cycle theorem — rest only on Lemmas 3.1 and 3.2 and never invoke it.

Granting the standing input, the proof concludes group-theoretically. The kernel of the maximal pro-$\ell$ quotient acts through a finite group of order prime to $\ell$ on $W_x$:
...
introduces no $\varprojlim^1$ term (Book 18, Sections 8.4 and 10.2). $\square$
...
There is one extension of Theorem 3.4 which is formal but must be stated separately. It does
...
in degree zero. For the extension
...
the finite-level Hochschild--Serre spectral sequence of Book 18, Section 5.2 — applied at each quotient by an open normal subgroup, where it is ordinary spectral-sequence bookkeeping for finite groups — has $E_2^{a,b}=H^a(\Pi_x/P_j,H^b(P_j,W))$ with $H^b(P_j,W)=0$ for $b>0$ by the same averaging argument that proves (3.12), so it collapses to
