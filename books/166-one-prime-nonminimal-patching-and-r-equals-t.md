...
5. [What patching and augmentation do prove](#5-what-patching-and-augmentation-do-prove)
   ...
   - [Amplitude after Book 165](#52-amplitude-after-book-165)
...
line chart. Book 155's minimal theorem does not identify this ring with a Hecke algebra.

This distinction is structural. Book 161 shows that there is no automatic morphism between
...
Under the explicitly named hypotheses recorded in Section 2.3, Book 165 proves full
topological support of the primary and companion type complexes on the named conductor-one
and conductor-two deformation rings. Book 161, under its separate clean trace-order
...
parallel-weight-two realization used by Book 165. All global rings below use this fixed datum
...
Book 165 writes the corresponding calculation with arithmetic Frobenius. Passing from its
...
lattice, Book 159's node-uniformization and generic-support hypotheses are imposed and its
...
vanishes. Book 158 proves saturation and its corrected component filtration only after these
vanishings are supplied. Non-Eisenstein localization and strong approximation do not prove
them.

**$({\rm TYPE}_v)$ (the typed complex).** Book 165's independent hypotheses
...
Book 165 then proves that the primary and companion pull maps are split injections over the
...
$({\rm SUP}_v)$ in Book 165, Section 8.2, holds: (1) the Taylor--Wiles sets are disjoint from
...
dimension there. This is the precise hypothesis of Book 165's support theorem. It is not
...
Book 161: the module is $\mathcal O$-free, its characteristic-zero Hecke action is semisimple,
every generic factor satisfies the named local conditions, and the trace/structural generators
listed above generate this entire image. Book 161 then constructs the order-valued
...
Section 2.2 exists, and Book 165's twisting identities hold for represented deformation rings,
...
In addition, augmentation of the *acting image* is exact. Book 160 proves the torsor, module,
...
switched graph--definite comparison, must be added exactly as in Book 165. Neither is used to
...
Book 165 computes the scalar local incidence chart. With $m$ the monodromy parameter, $u$ and
...
Under $({\rm SUP}_v)$, Book 165 uses the neutral minimal patched module only to produce a
...
Under $({\rm TYPE}_v)$, at auxiliary level $Q$ Book 165 constructs a primary complex
...
source. More importantly, Book 165 constructs the deformation-side action rather than
...
component, deduced in Book 165 from the five explicit boundary and depth clauses recalled
...
Under $({\rm REC}_v)$, Book 161 constructs a continuous representation over the faithful
...
and Book 161 reaches only its trace order when unrelated local operators have been adjoined.
...
**Proof strategy.** Use Book 165's conditional full-support theorem, identify the kernel with
...
The proof uses every named component because Book 165 proves support on every named component
...
Theorem 4.2, using Book 165's support theorem together with the reduced finite Hecke order,
...
ring used as a boundary anchor in Book 165.
...
not the neutral minimal complex. Book 155 therefore cannot be used to verify (5.4) for the
right side of (5.5).

There is nevertheless no amplitude gap: Book 165 proves directly, through its typed incidence
...
output of Book 165's one-prime patching, not a new consequence of minimal $R=T$.
...
auxiliary map sends every Taylor--Wiles diamond operator to one. Book 165 proves the
...
(3.7) is valid because Book 165 proves it by global twisting and coefficient extraction, not
...
sign does not follow from the old $U_v$-root decomposition. Books 157 and 165 construct the
enhanced line and primary--companion lattices that make the independently retained sign
meaningful at the colliding root. Book 158's full two-map saturation alone would not suffice.
...
The same-coefficient pull--norm Gram block, in Book 165's standard central orientation, is
...
This calculation is not a branch-determinant theorem. Book 165 proves only that the Galois
...
Book 165 retains the full two-map complex, the two extension orientations, and the full Smith
...
$L/F$. Automorphy over $L$ does not descend from invariance alone. Book 97 applies only when
...
The declared dependency row is $97,155,157,158,159,161,165$. Their exact roles after the
audit are as follows.

| Book | Result used | Boundary retained |
|---|---|---|
| 97, *Solvable Base Change and Descent* | recursive cyclic descent selected by an extending irreducible arithmetic representation | used only after top-field automorphy; invariance under a noncyclic solvable group is not enough |
| 155, *Minimal Modularity Lifting* | comparison point for the neutral minimal theorem and its finite-flat range | its equality is not applied to $R^{\mathrm{sp},1}$; the scalar Kummer and line-special directions do not satisfy that identification |
| 157, *Integral Local Types and Type Lattices* | enhanced special line, primary and companion lattices, primitive type occurrence, Frobenius conventions | at $q_v\equiv1$ a line enhancement replaces old-root separation; local lattices alone do not prove global support |
| 158, *Ihara Theory and Saturated Degeneracy Maps on Shimura Curves* | residual two-map Ihara injectivity and primitive saturation | conditional on the abelian Ihara hypothesis; full-map saturation does not create a scalar selected summand |
| 159, *Integral Level Change and Jacquet--Langlands Comparison* | node, graph, component-group, and normalized Smith comparisons | node-uniformization and generic support remain hypotheses; branch correction and the Néron component group are distinct |
| 161, *Hecke-Valued Galois Representations and Nonminimal Reciprocity* | the order-valued representation, named local factorization, trace generation, and the surjections (3.13) | every generic branch and every structural generator must be accounted for; a pseudorepresentation alone is insufficient |
| 165, *One-Prime Type Complexes and Component Support* | scalar chart, twist identities, projective primary--companion complexes, conditional full named-component support, and exact fixed-prime module and acting-image augmentation | $({\rm TIC}_v)$, $({\rm TIH}_{\mathscr F})$, $({\rm SUP}_v)$, and the effective ray quotient are separate hypotheses; its determinant result is only the scalar equation (6.9), with no cross-complex map; no global reducedness or nonminimal $R=T$ is asserted |

The proof of Theorem 4.2 uses Books 161 and 165 directly, with Books 157--159 supplying the
hypotheses built into Book 165. Book 155 is not an augmented-fiber $R=T$ input. Book 97 is used
only for the final descent statement. Book 160's torsor and module augmentation enter
transitively through Book 165; exact auxiliary *acting-image* augmentation remains the
separate hypothesis $({\rm AUG}_Q)$ and is not attributed to Book 160.
Books 162--164 likewise enter transitively through Book 165's Taylor--Wiles and lower-boundary
...
Book 165 proves the left side of (8.1) is $C_{\infty,v}^{\mathrm{sp},1}$, not the neutral
minimal complex. Book 155 does not prove an equality for the scalar conductor-one line-special
ring with its Kummer directions. Book 161 independently warns that minimal and sign-selected
special functors need not be related by a ring map.

After replacing (8.1) by the correct augmentation (5.5), Book 165 supplies amplitude under
...
the patched level. Books 158 and 165 give precise conditional implications after the
...
full support equality (3.10), already proved in Book 165 for every named component. It does not
...
The downstream interfaces respect this boundary. Book 170's generic distinct-root
level-raising place is not the scalar family treated here, and its potential-modularity
argument separately records every active place. Book 174 uses precisely Theorem 7.1 on the
augmentation character: its semistable Tate-module point has conductor one, kills the
nilradical, and therefore needs no full scheme-theoretic equality. Book 185 studies a dyadic
...
- Book 165's arithmetic-Frobenius equations are translated by $\Phi_v=\varphi_v^{-1}$; the
...
$({\rm RAY}_v)$ for the scalar family, Book 165 supplies the exact mathematical heart of the
...
$({\rm REC}_v)$, Book 161 supplies the surjection
