...
Everything so far is unconditional, and the one geometric input that remains — that the Jacobian of a curve carries an ample line bundle — is supplied by an earlier book. Book 15, Section 10.3, proves that $\operatorname{Pic}^d_{C/k}$ is smooth and proper; Book 8, Section 3.3, records that "proper implies projective" is false in general, so ampleness is a genuine further statement. For a curve of genus $g\ge1$ it is a theorem: Book 17a, Section 8.2, constructs the theta divisor $\Theta$ on $J=\operatorname{Pic}^0_{C/k}$ by the determinant of cohomology, and Book 17a, Section 8.4, proves that $\mathcal O(\Theta)$ is ample and induces a principal polarization. That proof uses divisor theory, determinantal cycles, and coherent cohomology, and no étale cohomology whatever, so there is no circularity in citing it here. The Jacobian is therefore a projective variety in the sense required by the degree theorem, and the following corollary is unconditional.
