...
The phrase **parallel weight two** will first refer intrinsically to a representation of $B^\times(\mathbf A_F)$: minimal weight-two discrete series at the unique split real place and trivial algebraic type on the compact adjoint group at every ramified real place, with the compatible scalar central character. Local Jacquet--Langlands identifies the latter compact type with weight two. When a global split transfer is supplied under the exact hypotheses of global Jacquet--Langlands, the same phrase agrees with parallel weight two on $\operatorname{GL}_2(\mathbf A_F)$. In particular, Book 104, Theorem 9.2 supplies the compact one-split packet when $d=[F:\mathbf Q]\ge2$, the finite ramification set $W$ satisfies $(d-1)+|W|$ even, and the split packet is selected special or selected tame dihedral at every member of $W$. Thus $W$ is odd and nonempty in even degree, while in odd degree it is even and may be empty. No transfer outside that exact selected range is inferred here.
...
The compact one-split inverse theorem is now available in one exact range. Suppose $d=[F:\mathbf Q]\ge2$, fix the active real place $\tau_0$, and let $W$ be a finite set with $(d-1)+|W|$ even. If a cuspidal full-group parallel-weight-two representation $\pi$ of $\operatorname{GL}_2(\mathbf A_F)$ has the compatible unitary central character and is selected special or selected depth-zero or tame positive-depth dihedral at every $w\in W$, Book 104, Theorem 9.2 constructs the unique noncharacter representation $\Pi_B$ on the quaternion algebra ramified at every real place except $\tau_0$ and exactly at $W$. It preserves multiplicity, central character, and every good right-convolution Hecke polynomial; under Book 104's additional algebraicity hypothesis it also preserves the good Hecke field, field of rationality, and coefficient conjugation. In even degree parity forces $W$ to be odd and nonempty; in odd degree $W$ is even and may be empty.

Thus, in that selected range, the split packet is unconditionally carried by the compact one-split curve: choose a level $K$ with $\Pi_{B,f}^K\ne0$ and apply (8.1). In even degree the theorem does not cover a split packet having no eligible finite place, because then no odd nonempty $W$ can be chosen. In odd degree $W=\varnothing$ is allowed, so no finite eligible place is needed. Primitive wild, excluded dyadic, principal-series, and other local factors still cannot be inserted as ramified members of $W$ by declaring them eligible. Intrinsically, (8.1) still decomposes the representations of the actual quaternionic group defining the curve and needs no global transfer for its proof.
...
For a split parallel-weight-two cusp form in Book 104's compact one-split range, a parity-compatible eligible set $W$ of selected special or selected tame-dihedral places now supplies the quaternionic packet unconditionally, so the preceding curve realization applies after choosing a level containing its finite vectors. In even degree this means an odd nonempty $W$ and no eligible finite place gives no such carrier; in odd degree $W=\varnothing$ is allowed.
