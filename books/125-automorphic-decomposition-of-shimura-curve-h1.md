...
The phrase **parallel weight two** will first refer intrinsically to a representation of $B^\times(\mathbf A_F)$: minimal weight-two discrete series at the unique split real place and trivial algebraic type on the compact adjoint group at every ramified real place, with the compatible scalar central character. Local Jacquet--Langlands identifies the latter compact type with weight two. When a global split transfer is supplied under the exact hypotheses of global Jacquet--Langlands, the same phrase agrees with parallel weight two on $\operatorname{GL}_2(\mathbf A_F)$. In particular, Book 104, Theorem 9.2 supplies the compact one-split packet when $[F:\mathbf Q]$ is even and the finite ramification set is an odd nonempty set $W$ at which the split packet is selected special or selected tame dihedral. No transfer outside that exact selected range is inferred here.
...
The compact one-split inverse theorem is now available in one exact range. Suppose $[F:\mathbf Q]$ is even, fix the active real place $\tau_0$, and let $W$ be an odd nonempty finite set. If a cuspidal full-group parallel-weight-two representation $\pi$ of $\operatorname{GL}_2(\mathbf A_F)$ has the compatible unitary central character and is selected special or selected depth-zero or tame positive-depth dihedral at every $w\in W$, Book 104, Theorem 9.2 constructs the unique noncharacter representation $\Pi_B$ on the quaternion algebra ramified at every real place except $\tau_0$ and exactly at $W$. It preserves multiplicity, central character, and every good right-convolution Hecke polynomial; under Book 104's additional algebraicity hypothesis it also preserves the good Hecke field, field of rationality, and coefficient conjugation.

Thus, in that selected range, the split packet is unconditionally carried by the compact one-split curve: choose a level $K$ with $\Pi_{B,f}^K\ne0$ and apply (8.1). The theorem does not cover a split packet having no eligible finite place, because then no odd nonempty $W$ can be chosen; nor does it cover primitive wild, excluded dyadic, principal-series, or other local factors by declaring them eligible. Intrinsically, (8.1) still decomposes the representations of the actual quaternionic group defining the curve and needs no global transfer for its proof.
...
| rational comparison with a split packet | Book 104, Theorem 9.2; $[F:\mathbf Q]$ even; one active real place; compatible unitary central character; $W=\operatorname{Ram}_f(B)$ odd and nonempty; selected special or selected tame-dihedral split factor at every $w\in W$; its additional algebraicity hypothesis for coefficient-field assertions | with no eligible place there is no such $W$, and local matching outside the selected range does not globalize an automorphic representation |
...
For a split parallel-weight-two cusp form in Book 104's compact one-split range, an odd nonempty eligible set $W$ of selected special or selected tame-dihedral places now supplies the quaternionic packet unconditionally, so the preceding curve realization applies after choosing a level containing its finite vectors. If there is no eligible finite place, no such $W$ exists and this conclusion makes no curve-carrier claim.
