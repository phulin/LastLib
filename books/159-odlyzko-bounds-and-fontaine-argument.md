...
because the derivative of the difference is $u^2/(1+u)\geq0$. Taking $u=1/n$ gives

$$
n^2\log\left(1+\frac1n\right)\geq n-\frac12\geq n-1.
$$

This proves $a_{n+1}\leq a_n$, hence $M_{n+1}\geq M_n$. Stirling's formula with its usual
two-sided remainder gives
...
The origin of the equation is geometric, but the measure normalization must be made explicit.
On
...
For this Fourier calculation give each complex coordinate twice ordinary Lebesgue measure. This
is the self-dual measure for the trace character. Accordingly, the covolume of a fractional ideal
$\mathfrak a$ is now
...
rather than the ordinary-measure covolume $2^{-r_2}\sqrt{D_K}\,N\mathfrak a$ used in Chapter 2.
The coordinate dual is, up to coordinatewise complex conjugation, the codifferent lattice

$$
\mathfrak a^*=\mathfrak D_K^{-1}\mathfrak a^{-1}.
$$

Normalize to the covolume-one lattice

$$
L_{\mathfrak a}=\Delta_{\mathfrak a}^{-1/n}\mathfrak a.
$$

Let $Y$ be the group of positive diagonal archimedean scalings of determinant one and let $P$ be
a fundamental parallelepiped for the logarithmic unit lattice in $Y$. For $y\in Y$ and $t>0$ set
...
The determinant of $t^{1/n}y$ is $t$. Poisson summation for the self-dual Gaussian therefore gives
...
This use of determinant scale, rather than a common-coordinate square scale, is what makes the
Mellin exponent independent of $n$.

Integrating over $P$ removes the overcounting by units. Every nonzero orbit corresponds to the
integral ideal generated by an element of $\mathfrak a$, in the inverse ideal class. Separating
the determinant scale from $Y$ and applying the real and complex Gaussian Mellin integrals gives,
...
where $c_K>0$ is independent of $s$ and of the ideal class, and
...
Split the $t$-integral at $1$. On $(0,1)$ use (3.1) and substitute $t\mapsto1/t$. The nonzero
parts become exponentially convergent integrals over $(1,\infty)$, one with $s$ and one with
$1-s$, and are entire. Inversion carries $P$ to another unit fundamental parallelepiped; the
unit-invariant integral is unchanged. The zero-vector contribution is
...
Thus each completed partial zeta function continues meromorphically with simple poles only at
$0$ and $1$. The operation $\mathfrak a\mapsto\mathfrak a^*$ permutes ideal classes, so summing
(3.2) over the class group proves the continuation and

$$
\Lambda_K(s)=\Lambda_K(1-s).
$$

The same calculation explains the exponent $s/2$ on $D_K$: trace duality contributes an inverse
square root of the discriminant, and Mellin transformation converts the covolume normalization
into $D_K^{s/2}$. The zero vector accounts for both poles. Thus the discriminant, continuation,
functional equation, and pole orders all come from one correctly normalized lattice duality.
...
The zeros of $\xi_K$ are the nontrivial zeros coming from $\zeta_K$ in the closed strip

$$
0\leq\Re(s)\leq1
$$

and are called the nontrivial zeros. Nothing below requires the separate zero-free-line theorem
that removes the boundary lines: the unconditional positivity argument includes the endpoints.
The zeros occur with multiplicity and are symmetric under
...
$|a|<1/2$. At $a=\pm1/2$ the function is identically $1$; equivalently, its Fourier transform is
a positive multiple of the Dirac mass at zero. Thus the endpoint is also of positive type.
Finally,
...
put every remaining logarithm in a rapidly convergent range. The universal constants in the
certificate also need directed, rather than decimal, evaluation. For $|y|<1$,
...
This gives $\log2$ from $y=1/3$ and gives logarithms of arbitrary positive rational intervals
after power-of-two range reduction. The alternating arctangent series together with

$$
\pi=16\arctan(1/5)-4\arctan(1/239)
$$

gives rational intervals for $\pi$. Finally Euler--Maclaurin summation gives

$$
\gamma=H_N-\log N-\frac1{2N}+\frac1{12N^2}
-\frac1{120N^4}+\frac1{252N^6}+R_N,
\qquad
|R_N|<\frac1{240N^8}.
$$

The last remainder follows by integrating the periodic Bernoulli polynomial $B_8$ and using
$|B_8(x)|\leq1/30$ on $[0,1]$. With $N=10^4$ these formulas certify
...
Bounding the exponentials and logarithms in the formulas for $B_T,C_T$ by their alternating
series now gives the following rational enclosures. Put $A_T=A(F_T)=T/2$. Already $m=1$ makes
the displayed series-remainder bound smaller than the quoted interval widths:
...
The local ramification estimate from the preceding discriminant theory, recalled with its exact
normalizations in §10.5, gives
...
The upper bound in §10.1 is the arithmetic half of Fontaine's argument, so we now recall its
proved dependency chain and verify every normalization used here. The Hopf-algebra separation
argument is repeated; the weighted ramification--congruence bridge is imported in its exact form
from the preceding discriminant volume. The important feature is that the exponent depends on the
power of $\ell$ annihilating the module, but not on its rank.
...
**Dependency and normalization check.** This is the weighted cluster lemma proved in the
preceding discriminant theory. We recall enough of its mechanism to identify every term in
(10.4). Choose a finite Galois extension $E/K$ splitting the points. Congruence at depth $r$
partitions the point group into cosets of the subgroup congruent to zero, so all blocks have equal
size. For a finite $G=\operatorname{Gal}(E/K)$-stable point group $X$, put
$Y=[\ell]X$ and let $b(X)$ denote the last upper parameter at which $G$ acts nontrivially on
$X$. The weighted orbit count for these translation blocks gives
...
Here is the exact source of the weights. For a $G$-stable subgroup $C\subseteq D$ and every lower
ramification group $G_s$, counting pairs $(\sigma,x)$ with $\sigma x-x\in C$ first by $\sigma$
and then by $x$ gives
...
After division by $|G_0||D|$ and summation over lower intervals, orbit--stabilizer produces the
Herbrand weight $1/[G_0:G_s]$. Thus the factor $e(E/K)$ used to convert $v_K$-depth to
$v_E$-depth cancels in upper time. The linear term $\ell I$ in (10.2) contributes exactly
$v_K(\ell)=e_K$ when one passes from $X$ to $Y$. At the terminal order-$\ell$ layer the separation
budget contributes $c$, while the convention
$G_s=\{\sigma:v_E(\sigma z-z)\geq s+1\}$ removes the initial unit interval and supplies $-1$.
These are precisely the two alternatives in (10.4a); the $I^\ell$ terms create intermediate
equal-block refinements whose Herbrand contributions telescope.

Apply (10.4a) to $X_j=\mathcal G[\ell^j](E)$. Since
$[\ell]X_j\subseteq X_{j-1}$,

$$
b(X_1)\leq e_K+c-1,
\qquad
b(X_j)\leq\max\{b(X_{j-1})+e_K,e_K+c-1\}.
$$

Induction yields $b(X_a)\leq e_Ka+c-1$. All points lie in $X_a$, so (10.4) follows. Upper
numbering passes to quotients, making the conclusion independent of the chosen splitting field.
This matches the imported lemma's hypotheses, valuation normalization, strict endpoint, and
constant exactly.
...
**Proof.** Multiplication by $\ell^a$ vanishes on the generic fiber. The coordinate algebra of a
finite locally free $R$-scheme is $R$-torsion-free, so the morphism $[\ell^a]$ is the zero
morphism on the whole model. The separation radius (10.3) and Lemma 10.1, with
$c=e_K/(\ell-1)$, give (10.5). $\square$
...
**Proof.** Theorem 4.1 expresses $\log D_K$ as the archimedean terms plus the zero and prime
sums. For $F_T=G_T/\cosh(x/2)$, §5.2 proves the zero sum nonnegative without GRH, while
$F_T\geq0$ makes the prime sum nonnegative. Section 5.3 gives $A(F_T)=T/2$, so division by $n$
is exactly the displayed inequality. Letting $T\to\infty$ gives
$B_T\to\log2$ and $C_T\to\pi/2-1$; choosing first a finite $T$ and then $N$ with $2T/N$ below
the remaining logarithmic gap proves the effective cutoff. Under GRH the same argument uses
$G_T$, for which both archimedean losses tend to zero and the pole cost can be made $o(1)$.
$\square$
...
**Proof.** At each $v\mid\ell$, Theorem 10.2 and Hilbert's different formula give

$$
\frac{d_v}{e_v}
<e(F_v/\mathbf Q_\ell)\frac{\ell}{\ell-1}.
$$

In the absolute root-discriminant product, $Nv=\ell^{f(F_v/\mathbf Q_\ell)}$ and
$\sum_{v\mid\ell}e_v^0f_v^0=[F:\mathbf Q]$, so all coefficient-prime contributions combine to
the single factor $\ell^{\ell/(\ell-1)}$. The exact tame formula and the bounded-upper-break
formula give the factors at $S$. This proves the general ceiling. The degree rows are the
certified crossings of §§7.3--7.5, and (15.1) is the crossing (7.3). Since a faithful Galois
action cuts out a field whose degree equals the image order, the final exclusions follow.
$\square$
