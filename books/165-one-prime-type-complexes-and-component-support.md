...
### 1.2 The exact Book 158 hypothesis
...
This is exactly Book 158's localized abelian Ihara condition
$({\rm AIH})_{v,\mathfrak m_D}$ at each constant level used by flag excision. Under (1.5),
Book 158, Theorem 14.1 gives residual injectivity, integral saturation, and its corrected
...
Thus every conclusion imported from Book 158 is conditional on the finite list (1.5), not on a
...
$({\rm TIH}_{\mathscr F})$ are not consequences of (1.5): Book 158 expressly treats constant
coefficients, for which cocycles are homomorphisms, whereas a nontrivial type local system has
crossed homomorphisms. Book 159 lists extension of the type sheaf, compatibility of pull and
...
minimal problem of Books 163--164.
...
model is the verified incidence model of Book 131, so its strict completed nodal chart is
...
the Hecke-valued representation of Book 161 has polynomial
...
Book 157's two integral Steinberg lattices are
...
The generic sheaves need not be unramified along the bad fiber. Consequently Book 131's
...
Book 131 supplies the constant-coefficient local model, but it does not prove this extension
for the nontrivial quotient and augmentation type systems. Book 159, Section 13.1, records
...
ranks bounded over $A_Q$; this is the curve version of Book 160's regular-module argument.
$\square$
...
$({\rm AIH}_{\mathscr F})$ through Book 158. For a nontrivial type system, its cocycles are
crossed homomorphisms, so Book 158's constant-coefficient arithmetic-character calculation does
not apply.
...
This is the exact type-specific residual Ihara input missing from Books 157--164. The
...
The two hypotheses cannot be merged. The finite list (1.5) is indispensable for Book 158's
...
$\operatorname{Tor}$ over the deformation ring acting through Book 161's reciprocity map.
...
157--164 do not prove the required global exactness for these type coefficients; Book 159's
...
refinement of Book 159's node hypothesis.
...
The constant-coefficient node theorem in Book 159 does not by itself prove this refinement;
Book 159, Section 13.1 lists it as the extra input for a type-enhanced variant. Under
$({\rm NU}_{\rm type})$,
...
**Proof.** Apply the corrected normalization filtration supplied by Book 158 under
...
1. Taylor--Wiles sets are disjoint from $v$, kill the dual Selmer group as in Book 162, and
   retain both degeneracy maps, both type rows, the flag, sign, determinant, and companion
   pairing in every finite shadow.
2. The lower boundary problem is a clean minimal problem to which Book 164 applies; its patched
...
These are geometric and Taylor--Wiles hypotheses, not a support assumption. Book 164 supplies
the lower carrier in clause 2 and Book 162 supplies the auxiliary primes in clause 1; Books
157--164 do not prove the boundary-chart and local-dimension assertions in clauses 4--5 for
...
**Proof.** Work in the residue field of a declared boundary prime. Book 164 supplies a nonzero
...
**Proof.** The finite-shadow construction of Book 160, with the strict complexes and pairings
...
Under Book 161's reciprocity map, substitute the geometric Hecke data $t=T_v$ and $d=qS_v$;
...
$R^{\rm sp}\twoheadrightarrow\mathbb T^{\rm sp}$ of Book 161.
...
unit-minor argument of Theorem 5.2 gives clauses 3--4. The Book 158 input enters clauses 5--6
...
| Book 17 | perfect complexes, derived base change, Tor, and determinant conventions | applies only after strict projective models exist |
| Book 66 | represented enhanced special matrix-and-line condition and exceptional tangent directions | the flag and Kummer directions remain at scalar points |
| Book 70 | depth, support, annihilator radicals, and Fitting cautions | topological support is not faithfulness |
| Book 131 | constant-coefficient parahoric incidence model, node chart, nearby cycles, and the ramified-coefficient warning | type-coefficient extension is the separate hypothesis $({\rm TIC}_v)$ |
| Book 157 | quotient primary and augmentation companion lattices, twists, and type labels | no rational projector and no primitive wild type |
| Book 158 | Theorem 14.1 under localized $({\rm AIH})_{v,\mathfrak m}$ and its corrected branch filtration | this book assumes the finite family $({\rm AIH}_{\mathscr F})$; Book 158 neither proves it nor extends it to type coefficients |
| Book 159 | the datum-specific node-uniformization and generic-support interfaces | its constant-coefficient theorem does not prove $({\rm TIC}_v)$, $({\rm TIH}_{\mathscr F})$, $({\rm PRI}_{\rm type})$, $({\rm NU}_{\rm type})$, or $({\rm SUP}_v)$ |
| Book 160 | effective auxiliary torsors, regular group modules, pairings, and finite shadows | module augmentation alone does not imply acting-order augmentation |
| Book 161 | Hecke-valued representation, local factorization, and $R^{\rm sp}\twoheadrightarrow\mathbb T^{\rm sp}$ | no deformation faithfulness or component coverage follows from the surjection |
| Book 162 | adequate Taylor--Wiles primes for $({\rm SUP}_v)$ | used only in the patched boundary-depth argument |
| Book 163 | exact global deformation datum and variance | conductor labels do not replace represented functors |
| Book 164 | minimal $R=T$ and the nonzero free lower boundary carrier | no nonminimal equality is imported |

The exact direct-dependency row is therefore

```text
165 | 6, 17, 66, 70, 131, 157, 158, 159, 160, 161, 162, 163, 164
```

Book 158's transitive geometric prerequisites are Books 12, 39, 41, 122, 126, and 131. Their
...
|---|---|
| primary/companion type control on curves | not supplied by Books 157--164: $({\rm TIC}_v)$ assumes extension and adjunction on the actual incidence model, while $({\rm TIH}_{\mathscr F})$ is the missing crossed-coefficient residual Ihara theorem; Sections 3--5 derive strict complexes and split amplitude from these exact inputs |
| primitive type residues | not supplied by Books 157--164: Proposition 3.1 proves nodewise coefficients $\pm1$, but global primitivity additionally assumes the exact residual normalization statement $({\rm PRI}_{\rm type})$; the switched definite comparison further requires $({\rm NU}_{\rm type})$ |
| faithful acting-image augmentation | proved in Section 9 by a genuine global twist and coefficient extraction, provided the effective ray quotient of Section 2.2 exists; it is not a formal consequence of Book 160's module augmentation |
...
| Book 158 is treated as unconditional or as a type-Ihara theorem | pass: constant terms use $({\rm AIH}_{\mathscr F})$, while nontrivial type terms use the separate $({\rm TIH}_{\mathscr F})$ |
...
external hypotheses are: $({\rm AIH}_{\mathscr F})$ for the constant rows and Book 158's
...
augmentation; and $({\rm SUP}_v)$ only for component support. Books 157--164 do not establish
