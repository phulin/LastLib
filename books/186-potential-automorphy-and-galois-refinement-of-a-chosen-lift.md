...
enough arithmetic certificates for Book 187 to construct its elementary-subfield packet array.
...
Book 155 passes from item 2 to item 3.  It does not pass from item 1 to item 2.  Book 185 now
...
here is a genuine conclusion of Book 185, not an unexplained input.  The conditionality has not
disappeared: the first unproved input in the attempted construction of
$(\mathrm{Seed}_{\mathrm{SP}})$ is the localized abelian Ihara vanishing identified in Book
185, Section 4.3; simultaneous multi-place cleanability and exact carrier support remain after
that.  A controlled Galois version must solve those problems while also controlling the normal
closure.
...
\\ \Downarrow \\
\text{Book 185 constructs a normalized primitive SP lift }\rho
\\[-2pt]
\text{using that same seed}
\\ \Downarrow \\
\rho|_{G_F}\text{ is automorphic by the exact minimal }R=\mathbb T\text{ theorem}
\\ \Downarrow \\
\rho|_{G_M}\text{ gives the controlled SP top datum of Book 187}.
\end{array}
\tag{1.2}
$$
...
particular, Book 185 proves the chosen-lift implication conditional on its seed; it does not
...
The obstruction is not local deformation theory.  Books 154 and 182 give the primitive
signed-special residual chart and its represented minimal deformation condition.  Books
183--184 give the supported cohomology and balanced presentation.  Those results do not make
the required automorphic module nonzero.  Patching begins only after that module exists.

### 1.3 The chosen lift supplied by Book 185
...
of Book 185, Theorem 1.1, together with its exact signed-special seed hypothesis
$(\mathrm{Seed}_{\mathrm{SP}}^{185})$.  Select one normalized horizontal point produced by
...
exactly Book 185, equations (10.1)--(10.5), not new hypotheses invented here.

There can a priori be two seed fields to distinguish.  The hypothesis
$(\mathrm{Seed}_{\mathrm{SP}}^{185})$ contains an even-degree totally real field $F_0$ on
which the restricted residual problem has the exact clean Book 155 realization.  Book 185
uses it to prove finiteness of the global ring and construct (1.1).  Because the resulting
$\rho|_{G_{F_0}}$ is a point of the same represented restricted problem, Book 155 also makes
that restriction automorphic.  But Book 185 neither asserts that $F_0$ has a controlled normal
closure nor supplies a solvable bridge, an admissible ascent to a Galois top, or Book 187's
finite descent and attachment ledger.

Book 186 therefore begins with the *conditional theorem output* of Book 185 and asks for a
controlled refinement, not for a second construction of the lift.  The lift may not be used
backwards to prove $(\mathrm{Seed}_{\mathrm{SP}}^{185})$ or the stronger controlled seed below.
For the dependency-closed conditional chain, the controlled seed of Section 4 may itself be
used as $(\mathrm{Seed}_{\mathrm{SP}}^{185})$, so one takes $F_0=F$ and does not assume two
...
This is the SP condition of Books 154 and 182, transported by the split local
...
from being the map to which Book 155 applies.
...
An **exact SP automorphic seed over $K$** means that the hypotheses of Book 155, Theorem 1.1,
...
7. the relevant Hecke orders are generically reduced, the named Hecke action is faithful, and
   a branchwise Hecke-valued Galois representation has integral local--global compatibility
   on every named quotient.
...
Thus the absolute and cyclotomic absolute irreducibility hypotheses needed by Book 155 survive
on the whole fixed-field diagram.  Disjointness only from a projective cutout would not prove
(3.3).

### 3.4 What the Book 185 seed already gives

**Proposition 3.2 (automorphy over the uncontrolled seed field).**  Let $F_0$ be the field in
$(\mathrm{Seed}_{\mathrm{SP}}^{185})$ and let $\rho$ be the Book 185 point selected in Section
...
**Proof.**  Book 185 constructs $\rho$ as a point of its global represented SP problem.
Restriction to $G_{F_0}$ is therefore a point of the exact restricted problem occurring in
$(\mathrm{Seed}_{\mathrm{SP}}^{185})$.  Book 185, Proposition 4.1, applies Book 155 to identify
that restricted deformation ring with its faithful minimal Hecke algebra.  The final clause of
Book 155, Theorem 1.1, makes every characteristic-zero point automorphic with a matching stable
...
$(\mathrm{Seed}_{\mathrm{SP}}^{185})$ makes the normal closure of $F_0$ controlled or the
...
It must be minimal away from $\{2,\ell\}$.  In particular, every additional bad prime of an
auxiliary abelian variety used to build the seed must already have been removed through a
proved clean chain, with the exact lower-level automorphic classes and integral maps required
...
the transfer range of Books 96--97.  The package must therefore include an
**ascent-admissible** prime-cyclic chain from $F$ to $M$.  At each step the selected packet must
be trace-comparable, its local norm fibers must be antecedent-complete selected, the reverse
comparison must have the required strongly cuspidal factor, and the relevant finite
quotient-character fibers must be saturated.

The top packet eventually delivered to Book 187 needs a second, finite ledger.  For every
elementary subgroup $H\leq\Gamma$, at least one prime-cyclic descent chain from $M$ to $M^H$
must satisfy Book 97's descent-admissibility and fiber-saturation hypotheses.  Every corrected
packet along those chains must lie in the all-coefficient weight-two attachment range of Books
135--137.  At the dyadic places, their special-scalar and raw-to-semisimple local-preservation
hypotheses must be verified uniformly.  Those conditions are exactly why Book 187 calls its
top packet “selected.”

This volume does not construct the elementary packets.  That is the subject of Book 187.  It
only identifies the certificates which must accompany $\pi_M$ so that Book 187 may construct
...
$(\mathrm{Seed}_{\mathrm{SP}}^{185})$; the new content is that the seed field is embedded in
...
$(\mathrm{Seed}_{\mathrm{SP}}^{185})$.  Thus this single controlled seed both enables Book 185
to construct the chosen lift and enables Book 155 to make its restriction automorphic.  This
...
Book 185 and a controlled exact SP residual seed.  Use that seed as
$(\mathrm{Seed}_{\mathrm{SP}}^{185})$, and select any normalized Book 185 output $\rho$ as in
...
**Proof.**  Lemma 3.1 supplies the full and cyclotomic image hypotheses over $F$.  Complete
...
augmentation, reducedness, and integral local--global hypothesis of Book 155, Theorem 1.1.
That theorem gives
...
controlled SP top datum assumed by Book 187.

**Proof.**  Starting with $\pi_F$ from Theorem 5.1, apply Book 97's prime-cyclic ascent theorem
along the supplied ascent-admissible chain.  Existence of each transfer and compatibility of
its attached Galois representation with restriction are conclusions of that theorem because
the chain's selector, fiber, and saturation hypotheses have been supplied.  Thus the endpoint
has (5.3).

The endpoint is cuspidal.  Indeed, Lemma 3.1 makes
$\bar\rho|_{G_M}$ absolutely irreducible, whereas the attached representation of a rank-two
noncuspidal isobaric endpoint would be reducible.  Complete splitting and Lemma 2.2 preserve
the full SP pair at every dyadic place.  The remaining certificates in Section 4.3 are already
part of the package, so $(M,\rho,\pi_M)$ satisfies precisely the input ledger of Book 187.
$\square$

It would be invalid to replace the supplied ascent-admissibility by the sentence “$J$ is
solvable.”  Book 97 explicitly separates the prime-cyclic group tower from the automorphic
...
Book 186 stops here.  It does not select packets over every elementary fixed field, prove
overlap identities, form a Brauer class, prove effectivity, or construct a compatible system.
Those are respectively the jobs of Books 187--189.

## 6. Where the attempted construction stops

### 6.1 The first obstruction: the exact localized carrier

The seed hypothesis consumed by Book 185 and the controlled seed required here both need an
exact nonzero localization on the inherited SP component.  Book 185, Section 4.3, audits the
attempted construction in proof order.  The first unproved assertion is the localized abelian
Ihara vanishing retained by the integral level comparison:
...
localized module and its saturated maps exist; none can be invoked to construct them.

Consequently the first missing theorem is already the **clean Frey-SP carrier theorem** of
Book 185: over an even-degree totally real split field, construct the nonzero minimal SP
localization and prove every clean integral hypothesis of Book 155, beginning with (6.1).
Book 186 needs a stronger controlled form in which that field is the quotient field of a
prescribed Galois top.  Normal-closure control does not repair the missing localized support.

### 6.2 Subsequent multi-place cleanability

Even if (6.1) and the initial exact localization are granted, the two-prime route has a second
arithmetic obstruction.  Book 174 constructs the Hilbert--Blumenthal bridge only for a
**two-prime admissible** datum.  Its post-specialization item 7 requires the specialized
auxiliary-adic Tate module to be a one-prime clean datum or to admit a finite clean chain, and
item 8 requires verified curve-realization data.  These clauses are assumed by its conditional
residual potential-modularity theorem, not proved for every Frey SP target.
...
At $x\in B(A)$ the target residual representation can be unramified while the
...
not even recover the ordered root and sign automatically.  Book 166 can change one previously
...
This is Book 174's stated remaining obstruction after the earlier localized-carrier issue: a
prior, acyclic theorem proving post-specialization simultaneous cleanability for every extra
semistable prime.  Even if a Galois approximation datum and (6.1) are granted, the two-prime
method stops here before it has produced the exact seed of Section 3.2.

### 6.3 Independent normal-closure input

Book 171 proves Galois-refined approximation after a **Galois approximation datum** for the
...
local top torsors for the paired-frame Hilbert--Blumenthal moduli space used in Book 174.

Consequently the controlled construction has three distinct unmet tasks:

$$
\begin{array}{ll}
\text{initial support:}&
\text{prove localized Ihara and construct the exact minimal SP carrier;}\\
\text{field geometry:}&
\text{construct the moduli-specific Galois approximation datum;}\\
\text{later support:}&
\text{prove simultaneous post-specialization cleanability.}
\end{array}
\tag{6.3}
$$

Each remains an obstruction if the other two are assumed.  Ordinary Moret--Bailly
approximation cannot be promoted to normal-closure control, and a controlled normal closure
does not force either initial or post-specialization minimal automorphic support.
...
* **Use group solvability alone.**  It gives a prime-cyclic series, not the selected packet
  fibers and saturation required by Book 97.
* **Invoke minimal patching.**  Book 155 needs the exact minimal automorphic module before
  patching; it cannot prove that module nonzero.
* **Keep the extra primes.**  Then the auxiliary packet does not realize
  $\mathcal D_F^{\mathrm{SP}}$, so Theorem 5.1 does not apply to the chosen minimal lift.
* **Use a later simultaneous-carrier theorem.**  Any later theorem which takes Book 186 or
  the compatible system of Book 189 as input would make the argument circular.
...
The following theorem, placed strictly before Book 186, would remove the first missing object.
...
residual representation of Book 154, satisfying the determinant, finite-flat, absolute
irreducibility, cyclotomic absolute irreducibility, and other global image hypotheses required
by Book 155.  Given a finite Galois avoidance field $D$, finitely many prescribed split places, and an
even-index solvable-bridge group datum $(\Gamma,J)$, construct a controlled exact SP residual
seed as in Section 4.4.

In particular, the theorem must prove rather than assume:

1. the localized abelian Ihara vanishing (6.1), nonvanishing of the inherited minimal SP
   localization, and every saturated integral comparison needed to begin Book 155;
2. a Galois approximation datum for the actual paired-frame moduli space, with full top group
   $\Gamma$, point stabilizer $J$, all required local top fibers, and closure-level
   disjointness from $D$;
3. a totally real specialization whose extra bad primes all admit simultaneous clean chains;
4. exact occurrence of $\bar\rho|_{G_F}$ in the minimal SP localization, including every
   saturation, pairing, augmentation, reducedness, and integral local--global hypothesis of
   Book 155.

The proof must be acyclic: it may not use Book 186, the elementary packet array of Book 187,
the effectivity theorem of Book 188, the compatible system of Book 189, or any later theorem
...
The first missing assertion in the audited proof order is (6.1).  The smallest honest direct
repair is a localized Ihara theorem proving that vanishing at the Frey non-Eisenstein maximal
ideal and deriving the saturated integral level maps needed for the exact signed-SP carrier.
It must apply at the inherited nontrivial-unipotent dyadic component; a scalar-boundary
comparison for another local deformation ring does not suffice.

After that repair, the next missing support theorem is:

> For every specialized two-prime Hilbert--Blumenthal carrier, construct simultaneously at all
> uncontrolled extra semistable primes the lower-face automorphic classes, ordered flags,
> saturated maps, pairings, and integral vanishing-cycle nullhomotopies which put its
> auxiliary-adic representation into a finite clean chain.

This is Book 174, Section 2.5, item 7, expanded rather than renamed.  It would not by itself
supply the independent Galois presentation or Book 187's transfer and attachment certificates.

### 7.3 The later bridge-readiness obligation

After Required Theorem 7.1, Book 185, and Theorem 5.1 have produced the chosen point and its
...
Book 187's finite descent and attachment ledger.  This may be packaged with Required Theorem
...
|---|---|---|
| Book 97 | prime-cyclic transfer once the selected, trace-comparable, antecedent-complete, cuspidal, and saturated fiber hypotheses are supplied | transfer from solvability alone |
| Books 135--137 | weight-two coefficient attachment and the SP local comparison in their explicitly stated range | an automatic all-coefficient SP comparison for an arbitrary packet |
| Book 154 | the primitive residual Frey SP tuple, intrinsic line, sign, and conductor-one record used in the FLT specialization | a characteristic-zero chosen lift or automorphic seed |
| Book 155 | exact minimal $R=\mathbb T$ and automorphy of every point of a clean minimal deformation problem | existence of the exact residual automorphic localization |
| Book 166 | a local change at a previously named place under its exact clean augmentation hypotheses | simultaneous lowering at the unknown extra bad primes of a specialization |
| Book 170 | a conditional two-prime potential-modularity route and point-field control in its admitted range | an unconditional clean seed or controlled normal closure |
| Book 171 | Galois-refined approximation from a supplied Galois approximation datum | that datum for the paired-frame moduli problem |
| Book 174 | conditional two-prime specialization and residual potential modularity for a two-prime admissible datum | proof of post-specialization cleanability item 7 |
| Books 182--184 | the SP local deformation problem, supported cohomology, and balanced presentation | a horizontal point or a nonzero exact automorphic module |
| Book 185 | under $(\mathrm{Seed}_{\mathrm{SP}}^{185})$, finite-flatness of the balanced global SP ring and the normalized primitive SP point used here | construction of its seed, normal-closure control, or the controlled seed required in Section 4 |

Book 185 now genuinely supplies the chosen point conditional on its exact seed; this volume no
longer treats that point as a free-standing assumption.  Proposition 3.2 also extracts its
automorphy over Book 185's seed field.  The controlled seed remains stronger and unproved.
...
$$
\boxed{
186\mid 97,135,136,137,154,155,171,174,182,185.}
\tag{8.1}
$$

Books 166 and 170 are background to the seed-construction audit inherited through Books 174
and 185 and need not be direct edges here.  Books 183--184 are direct inputs to Book 185, but
transitive for this volume.  Book 182 is retained because this volume directly identifies the
represented local SP problem; it could be made transitive by treating all local identifications
as part of the Book 185 export.

Equation (8.1) does not make the result unconditional.  To mark Book 186 **READY**, a new
...
\\ \Downarrow \\
\text{the controlled seed supplies }(\mathrm{Seed}_{\mathrm{SP}}^{185})
\\ \Downarrow \\
\text{Book 185: a normalized primitive chosen SP lift }\rho
\\ \Downarrow \\
\text{Book 155: }R_F^{\mathrm{SP}}=\mathbb T_F^{\mathrm{SP}}
\\ \Downarrow \\
\rho|_{G_F}\text{ is automorphic}
\\ \Downarrow \\
\text{the bridge-readiness ledger is verified for the selected packet}
\\ \Downarrow \\
\text{Book 97 along the supplied ascent-admissible chain}
\\ \Downarrow \\
(M,\rho,\pi_M,\text{certificates})\text{ is the controlled SP top datum}
\\ \Downarrow \\
\text{Books 187, 188, and 189}.
\end{array}
\tag{8.2}
$$
...
* Book 187 assumes the controlled SP top datum with $N_2\ne0$ of rank one, the intrinsic line,
  sign, and conductor exponent one.  It constructs the elementary packets and their overlap
  identities.  Its own dependency audit states that Book 186 has not unconditionally
  constructed the seed.
* Book 188 proves effectivity from Book 187's packet array and retains the same SP record.  Any
  phrase there saying that Book 187 exports an “unconditional package” is read relative to its
  controlled-top hypothesis, not as a proof of that hypothesis.
* Book 189 assumes the controlled SP top datum and the effective array.  It again retains
...
Book 186 handoff consistent with the committed texts is (5.4)--(5.5).

Book 177 is not a source of the missing seed.  It starts from a compatible family and changes
...
condition in (2.2) is therefore essential, but Book 177 lies after the compatible-family input
and cannot repair the gap found here.

## 9. Verdict

Book 185 now genuinely constructs the chosen primitive SP lift under its explicit
$(\mathrm{Seed}_{\mathrm{SP}}^{185})$ hypothesis, and Proposition 3.2 makes that lift automorphic
over Book 185's uncontrolled seed field.  Given the stronger controlled residual seed, Book
155 makes the same *chosen* lift automorphic over the solvable-bridge point field, and a
supplied Book 97 transfer ledger carries it to the controlled top.  This conditional argument
exports precisely the datum consumed by Book 187 and does not duplicate Book 187's
...
of Section 4.  In the available construction, its first irreducible arithmetic blocker is the
localized abelian Ihara vanishing (6.1) and the resulting exact clean SP carrier.  Even after
that is granted, simultaneous post-specialization cleanability at every uncontrolled extra
semistable prime remains unproved, as does the independent moduli-specific Galois presentation.
