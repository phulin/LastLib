...
3. [Multiplicative uniformization](#3-multiplicative-uniformization)
   ...
   - [Kernel, surjectivity, and the group law](#34-kernel-surjectivity-and-the-group-law)
   - [Finite maps of complete curves](#33-finite-maps-of-complete-curves)
   - [Galois equivariance and descent](#35-galois-equivariance-and-descent)
   - [Why algebraic points do not require completion](#36-why-algebraic-points-do-not-require-completion)
   - [Analytic and algebraic uniformization compared](#37-analytic-and-algebraic-uniformization-compared)
...
### 3.3 Finite maps of complete curves

The second part of Theorem 3.1 compares the quotient with the cubic through a map built from two functions and concludes from an equality of function fields that the map is an isomorphism. Rather than allude to that principle, this subsection proves it once, together with exactly the finiteness facts it needs. Throughout, $X$ is a smooth complete curve over a field $F$; in applications $F$ is $C$, or a finite extension of $K$ when algebraic points are tracked. Smoothness means that every point $x$ has a local parameter: a function regular at $x$ and vanishing to order exactly one there. Completeness is properness: every meromorphic function has equally many zeros and poles, counted with multiplicity, and images of closed subcurves are closed.

**Lemma 3.3 (orders).** _Let $x\in X$. The functions regular at $x$ form a local ring $\mathcal O_{X,x}$ inside the field $k(X)$ of meromorphic functions, with maximal ideal the functions vanishing at $x$. This ring is a discrete valuation ring: there is a local parameter $t_x$ generating its maximal ideal, and every nonzero $f\in k(X)$ factors uniquely as $u\,t_x^{\operatorname{ord}_x(f)}$ with $u$ a unit of $\mathcal O_{X,x}$ and $\operatorname{ord}_x(f)\in\mathbf Z$, zero precisely when $f$ neither vanishes nor has a pole at $x$._

_Proof._ Divide a nonzero $f\in\mathcal O_{X,x}$ by powers of $t_x$. The division stops after finitely many steps: if $f$ were divisible by every power of $t_x$ within the local ring, then $f t_x^{-n}$ would be regular at $x$ for all $n$, so $f$ would vanish to arbitrarily high order; but the intersection of the powers of the maximal ideal in this noetherian one-dimensional local domain is zero (Krull's intersection theorem), so such an $f$ is zero. Thus $f=u\,t^n$ within $\mathcal O_{X,x}$ for some $n\geq0$ and some unit $u$: after the last division the remainder is not in the maximal ideal, hence invertible. For a general meromorphic $f$, apply the same to $f$ or to $f^{-1}$, whichever is regular at $x$. Uniqueness is forced because $t_x$ is not a zero-divisor and units have order zero. Additivity of $\operatorname{ord}_x$ under multiplication is immediate from the factorization. Finally the maximal ideal is generated by $t_x$, so the ring is a discrete valuation ring in the usual sense: a domain whose ideals are the powers $(t_x^n)$. $\square$

**Lemma 3.4 (functions give finite maps).** _A nonconstant $f\in k(X)$ extends to a morphism $f:X\to\mathbf P^1_F$ which is finite, and_
$$
[k(X):F(f)]=-\sum_{x}\min\bigl(\operatorname{ord}_x(f),0\bigr)\,[\kappa(x):F],
$$

_the total pole order weighted by residue degrees. In particular, when $F$ is algebraically closed the scheme-theoretic fibers of $f$ are finite sets, and the length of the fiber over a pole-free value $a$ is the total zero order of $f-a$._

_Proof._ At points where $f$ is regular the map is given by $f$ itself; at a pole, rewrite $f=1/g$ with $g$ regular and vanishing, which gives the value $\infty$. These formulas glue because they agree wherever both apply. Finiteness: each fiber is cut out by one nonzero equation $f=a$ or $1/f=0$, and by Lemma 3.3 that equation has finite zero divisor, so every fiber is a proper zero-dimensional scheme, hence a finite disjoint union of spectra of artinian algebras; the map $f$ has finite fibers. A map out of a complete curve is proper, and a proper morphism with finite fibers between noetherian schemes is finite—the standard finiteness half of Zariski's main theorem, which is also the input used when a nonconstant function on a complete curve is viewed as a finite map to the projective line in Book 9's divisor theory. For the degree formula, put $S=1/T$ and complete $F(f)=F(T)$ at the place $T=\infty$, obtaining the Laurent series field $F(\!(S)\!)$; tensoring gives
$$
k(X)\otimes_{F(f)}F(\!(S)\!)\;\cong\;\prod_{f(x)=\infty}\kappa(x)(\!(t_x)\!),
$$

where the factor at a pole $x$ records $\operatorname{ord}_x(f)$ powers of the uniformizer $t_x=1/f$ and $[\kappa(x):F]$ residue degrees: this is the standard decomposition of a finite extension after completion at a place, each point above the place contributing one Laurent series field. Comparing dimensions of both sides over $F(\!(S)\!)$ gives the displayed identity; the same comparison at any value of $f$ gives the fiber statement. $\square$

**Lemma 3.5 (morphisms of complete curves are finite).** _A nonconstant morphism $f:X\to Y$ of smooth complete curves over the same field is surjective and finite. Its degree is $[k(X):f^{*}k(Y)]$, and degrees multiply along composites: $\deg(g\circ f)=\deg g\cdot\deg f$ for a further nonconstant $g:Y\to Z$._

_Proof._ The image is irreducible and closed, because $X$ is complete; it is not a point, because $f$ is nonconstant; since $Y$ is an irreducible curve, the image is all of $Y$. To see finiteness, choose any nonconstant $h\in k(Y)$. By Lemma 3.4 the functions $h\circ f\in k(X)$ and $h\in k(Y)$ define finite maps to $\mathbf P^1$, so all their fibers are finite sets. Every fiber $f^{-1}(y)$ is contained in the fiber of $h\circ f$ over $h(y)$, so $f$ has finite fibers; being proper, it is finite by the same criterion as in Lemma 3.4. Dominance identifies $k(X)$ with a finite extension of $f^*k(Y)$, which defines the degree. Multiplicativity is then the tower law for fields: for $g:Y\to Z$ nonconstant,
$$
\deg(g\circ f)=[k(X):g^{*}f^{*}k(Z)]
=[k(X):f^{*}k(Y)]\cdot[f^{*}k(Y):f^{*}g^{*}k(Z)],
$$

and pullback by the injective map $f^{*}:k(Y)\hookrightarrow k(X)$ preserves field degrees, so the second factor is $\deg g$. $\square$

**Proposition 3.6 (finite birational maps are isomorphisms).** _Let $f:X\to Y$ be a finite morphism of smooth complete curves over $F$, and suppose the pullback $F(Y)\to F(X)$ is an isomorphism of fields. Then $f$ is an isomorphism of curves._

_Proof._ Identify both function fields with a common field $\Omega$ through the pullback. Fix $y\in Y$ and a point $x$ above it. The stalk of a finite morphism is a finite module, so $\mathcal O_{X,x}$ is a finite, hence integral, $\mathcal O_{Y,y}$-algebra; and the sheaf map embeds $\mathcal O_{Y,y}$ into $\mathcal O_{X,x}$ compatibly with the inclusions of both into $\Omega$, because a function regular at $y$ pulls back to a function regular at every point above $y$ with the same value. Every element of $\mathcal O_{X,x}$ thus lies in $\Omega$ and is integral over $\mathcal O_{Y,y}$. But $\mathcal O_{Y,y}$ is a discrete valuation ring of $\Omega$ by Lemma 3.3, and discrete valuation rings are integrally closed in their fraction fields; here the fraction field of $\mathcal O_{Y,y}$ inside $\Omega$ is all of $\Omega$. Hence $\mathcal O_{X,x}\subseteq\mathcal O_{Y,y}$, and with the reverse inclusion,
$$
\mathcal O_{X,x}=\mathcal O_{Y,y}.
$$

Two distinct points of a curve determine distinct local rings inside the function field—their maximal ideals differ, since a valuation center is detected by which functions vanish—so the fiber over $y$ consists of the single point $x$. As $y$ varies, every stalk of $f_*\mathcal O_X$ equals the corresponding stalk of $\mathcal O_Y$, including residue fields, since the two rings coincide together with their maximal ideals. The sheaf $f_*\mathcal O_X$ is coherent because $f$ is finite, and a coherent sheaf agreeing with $\mathcal O_Y$ on every stalk is $\mathcal O_Y$; finally $f$ is affine, being finite, so $X\cong\underline{\operatorname{Spec}}_Y\, f_*\mathcal O_X=\underline{\operatorname{Spec}}_Y\,\mathcal O_Y=Y$ over $Y$. $\square$

For the analytic quotient this applies verbatim: smoothness of the quotient is visible from the parameter $du/u$ near the identity and from translation elsewhere, completeness holds because $C^\times$ is complete and $q^{\mathbf Z}$ acts freely and properly discontinuously with compact quotient, and the local rings of the quotient curve have exactly the DVR structure described, with orders computed by Laurent expansion. This is the sense in which the criterion below is an analytic statement.

### 3.4 Kernel, surjectivity, and the group law
...
Thus the map is locally an isomorphism. Second, $X$ has divisor of poles $2[1]$ on the quotient, so by Lemma 3.4 the function $X$ is finite of degree two as a map to the projective line, its degree being exactly the total pole order. Identity (3.3) exhibits the two sheets of $X$ over a generic value as $[u]$ and $[u^{-1}]$, and $Y$ separates them unless they coincide. Consequently $(X,Y)$ induces an isomorphism of meromorphic function fields between the quotient and the smooth cubic (2.2). The morphism $(X,Y)$ is nonconstant between smooth complete curves, hence finite by Lemma 3.5; its degree equals the degree of that function-field extension, which is one, and Proposition 3.6—finiteness together with equality of function fields—then makes it an isomorphism. In particular its preimage of $O$ is exactly $q^{\mathbf Z}$, and surjectivity is part of this degree-one conclusion.
...
### 3.5 Galois equivariance and descent
...
### 3.6 Why algebraic points do not require completion
...
Here the correct argument is more precise. If $q,u\in L$ for a finite extension $L/K$, every term of (3.1) lies in $L$, and $L$ is complete. Hence $X(u),Y(u)\in L$. Conversely, let $P\in E_q(\overline K)$ and choose a finite extension $L$ containing its coordinates. Theorem 3.1 gives a parameter $u\in C^\times$, unique modulo $q^{\mathbf Z}$ because $\phi_q$ is an isomorphism. For $\sigma\in G_L$, equivariance (3.11) and $P\in E_q(L)$ give $\phi_q(\sigma u)=\sigma P=P$, so $\sigma u=q^{n(\sigma)}u$ for an integer $n(\sigma)$. The assignment $\sigma\mapsto n(\sigma)$ is a continuous homomorphism into the discrete group $\mathbf Z$: near a given $\sigma$ the point $\sigma u$ stays close to its limit value $q^nu$, while the orbit $q^{\mathbf Z}u$ is discrete, so nearby automorphisms read off the same exponent. Since $G_L$ is compact and $\mathbf Z$ torsion-free, the image is trivial. Thus $u$ is fixed by $G_L$, hence belongs to the algebraic closure of $L$ inside $C$—an element transcendental over $L$ can always be moved by some automorphism of $C$ over $L$—and $L(u)$ is a finite extension of $L$. The inverse is separable because (3.9) has nonzero differential, so it preserves separable fields of definition. Thus
...
### 3.7 Analytic and algebraic uniformization compared
...
**Proof strategy.** Normal convergence gives the map and its differential. The theta quotients of Section 1.5 show that every meromorphic function on the quotient with a prescribed degree-zero divisor is generated by multiplicative parameters. In particular, $X$ has degree two and $(X,Y)$ has degree one, proving the algebraically closed complete case: by Lemma 3.5 the map $(X,Y)$ is finite, and Proposition 3.6 upgrades equality of function fields to an isomorphism. For an algebraic field $M/K$, embed an algebraic closure of $M$ in its completion and apply that case there. A point $P\in E_q(M)$ then has a parameter $u\in C^\times$ with $\phi_q(\sigma u)=\sigma P=P$ for all $\sigma\in G_M$, so $u$ satisfies (3.16) with $G_K$ replaced by $G_M$; the exponent homomorphism is continuous from the compact group $G_M$ into the discrete torsion-free group $\mathbf Z$, hence vanishes, and the parameter is fixed, therefore algebraic over $M$ rather than merely an element of the completion—and being algebraic and fixed up to $q^{\mathbf Z}$ by every automorphism, a representative already lies in $M$ after the exponent vanishes. This also covers purely inseparable $M/K$, because the inverse has nonzero differential and hence its parameter is separable over the point's field of definition. The differential identity was established locally at $u=1$ and then globally because the ratio of two invariant differentials is constant. $\square$
