...
arbitrary finite étale $k$-algebra. For full faithfulness, let $B$ and $C$ be finite étale over
$A$. An $A$-algebra map $B\to C$ is the same thing as a section of the finite étale
$C$-algebra $B\otimes_AC\to C$. The finite algebra $C$ is a product of henselian local rings:
its special-fiber idempotents lift uniquely, and each resulting local factor is finite over the
henselian local ring $A$, so the simple-root form of Hensel's lemma applies in that factor. A
section of a finite étale algebra over each factor of $C$ is therefore uniquely determined by,
and uniquely lifts from, its residue-field section. Taking the product over the factors gives

$$
\operatorname{Hom}_{A\text{-alg}}(B,C)
\xrightarrow{\ \sim\ }
\operatorname{Hom}_{k\text{-alg}}(B/\mathfrak mB,C/\mathfrak mC).
$$

Thus reduction is full and faithful as well as essentially surjective. This direct section
argument proves the categorical equivalence before the descent theorem of Chapter 6 is used.
...
of $\pi_1(\mathbf G_{m,\bar k})$ in characteristic $p$, with the convention $p=0$ meaning all
primes. In characteristic zero this is the full geometric fundamental group. Indeed, work over
an algebraically closed field and let $V\to\mathbf G_m$ be a connected finite étale cover of
degree $d$. Normalize $\mathbf P^1$ in its function field. The resulting smooth proper curve
$\overline V$ maps separably to $\mathbf P^1$ and can ramify only over $0$ and $\infty$. If
$r_0$ and $r_\infty$ are the numbers of points over those two values, characteristic zero makes
all ramification tame, and Riemann--Hurwitz gives
...
Both $r_0$ and $r_\infty$ are positive, while the left side is at least $-2$, so
$r_0=r_\infty=1$ and $g(\overline V)=0$. The two points are totally ramified. Choose a coordinate
$u$ on $\overline V\simeq\mathbf P^1$ having divisor the point over $0$ minus the point over
$\infty$. The pullback of the coordinate $t$ then has divisor $d\operatorname{div}(u)$, hence
$t=cu^d$ for a constant $c$; algebraic closedness absorbs $c$ into $u$. Thus every connected
cover is Kummer, and compatible roots identify the full group with $\widehat{\mathbf Z}$, up to
the choice of those roots. In positive characteristic the full group is much larger because wild
covers of $p$-power order also occur.
