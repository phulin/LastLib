...
**From the affine line to smooth morphisms.** Since $\mathbf A^N_S=\mathbf A^1_{\mathbf A^{N-1}_S}$, iterating (3.1) and composing the adjunctions shows that $Rp_!$ for $p:\mathbf A^N_S\to S$ has right adjoint $p^*(N)[2N]$. An étale morphism $w$ has $Rw_!=w_!$ with right adjoint $w^*$, so any $V$ étale over $\mathbf A^N_Y$ inherits the statement by composing adjunctions, and the residue normalization is unchanged because the residue is étale-local.

Now let $q:W\to Y$ be smooth separated of pure relative dimension $N$. Write $\pi_0(W/Y)$ for the étale sheaf of geometric connected components of the fibers and $\Lambda_n[\pi_0(W/Y)]$ for the sheaf freely generated by it. The chartwise affine-space calculation gives

$$
R^mq_!\Lambda_n(N)=0\quad(m>2N),
\qquad
R^{2N}q_!\Lambda_n(N)
\simeq\Lambda_n[\pi_0(W/Y)].
$$

The augmentation that sums the basis elements defines the top-degree trace

$$
\Lambda_n[\pi_0(W/Y)]\longrightarrow\Lambda_n.
$$

Equivalently, every geometric connected component has fundamental class of trace $1$. These local augmentations are invariant under permutation of the components and therefore descend. Composing the canonical truncation map with this augmentation defines

$$
t_q:Rq_!\Lambda_n(N)[2N]\longrightarrow\Lambda_n.
$$

When the geometric fibers are connected, the component sheaf is $\Lambda_n$ and this reduces to the formerly displayed isomorphism. For the factorization used below, $W$ is open in $\mathbf P^N_Y$; every nonempty geometric fiber is an open subscheme of the irreducible space $\mathbf P^N$, hence is connected, and the component sheaf is extension by zero from the open image of $q$. Repeating the construction of $\theta$ verbatim with $t_q$ in place of $t_p$ gives a global morphism
