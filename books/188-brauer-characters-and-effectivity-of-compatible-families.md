...
1. [The effectivity problem](#1-the-effectivity-problem)
   - [The package received from Book 187](#11-the-package-received-from-book-187)
...
13. [Exact endpoint and conclusion](#13-exact-endpoint-and-conclusion)
    ...
    - [The boundary with Book 189](#134-the-boundary-with-book-189)
...
summands. Book 187 identifies the two restrictions on every elementary intersection field with
...
coherent global tasks belong to Book 189.

**Interface received.** Book 187, Proposition 6.2 and equations (6.4e)--(6.4h), now export the
...
### 1.1 The package received from Book 187
...
Book 187 supplies a selected parallel-weight-two cuspidal packet $\pi_H$ over $F_H$. The packets
...
No normality of $D$ in $H$ or $xJx^{-1}$ is required. Book 187 proves (1.3) by comparing the
...
For Brauer characters $\theta_H,\theta_J$, Book 187 further retains
...
$n_i\in\mathbf Z$. Put $F_i=F_{H_i}$. Use the common Galois number field $K$ of Book 187,
...
All representations entering (1.6) are understood semisimply. Book 187 proves
...
base-field compatible system. That assembly is the task of Book 189.
...
3. Insert Book 187's overlap identities (1.3)--(1.3b), separately for every double coset, to
...
That is precisely the geometry on which Book 187 supplied compatible packets.
...
Book 187, Proposition 6.2 and (6.4e)--(6.4h), identifies both restricted rank-two
...
in Mackey are the semisimple representations identified by Book 187.
...
$L_{ij}(x)/F_i$ may be non-Galois; ordinary Galois restriction, Book 187's algebraic
...
by a character of the finite quotient. The selected cyclic descents of Book 187 choose one
...
silently interpreted as automorphic base change to $L$. Instead Book 187 applies its
...
in one common finite overfield. They are exactly Book 187's equation (6.4e).
...
the coefficient embedding makes the equality algebraic in $K[X]$, and Book 187's
Chebotarev--Brauer--Nesbitt lemma identifies the two already existing semisimple
representations over $G_L$ at every embedding. Book 188 imports this completed theorem; it does
...
$r_{D,\tau}$ and hence does not change its semisimple isomorphism class. Book 187 neither
...
rank-one Galois--Hecke dictionary already used in Book 187 for its finite-order twists; here the
...
**Proof.** Formula (6.2) proves entrywise equality. Book 187's overlap theorem applies to every
...
At $\tau_0$, Book 187's tensor identity gives
...
Thus exactly one $m_a$ is $1$ or $-1$. On the other hand, Book 187's rank calculation gives
...
The determinant is multiplicative in the semisimple Grothendieck group. Book 187 proved the
virtual identity
...
part of Book 189's assembly problem.
...
This is stronger than the polynomiality statement of Book 187. There the right side was the
...
The roots retain the purity record proved in Book 187: under every complex embedding they have
...
is the effective rank-two class represented by Book 187's virtual Weil--Deligne Mackey sum.
...
Book 187 also exports archimedean and conductor records. Effectivity turns both from signed
...
the cyclotomic determinant. The virtual gamma factor $\Gamma_{\mathbf C}(s)$ from Book 187 is
...
induction formula exported by Book 187 gives
...
Retain Book 187's finite set $S_0$, containing $2$, the distinguished prime $\ell$, every
...
intersections. Book 187's Proposition 6.2 applies to every cross pair, including non-Galois
...
field and therefore relation-independence of the effective irreducible. Book 187 already proves
...
Book 189 begins with the effective isomorphism classes, the common polynomials, and the
...
elementary-intersection interface exported by Book 187, fix an integral Brauer datum
...
subgroup is elementary, and Book 187, Proposition 6.2 and (6.4e)--(6.4h), identifies the two
...
Book 187's polynomial record became a representation only after the signed character was proved
...
Book 188 ends with algebraically closed local isomorphism classes
