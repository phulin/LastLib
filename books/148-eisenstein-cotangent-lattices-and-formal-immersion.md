...
Pullback on invariant differentials is contravariant and agrees with the normalized weight-two Hecke action. These conventions are the ones fixed in Books 113--114.
...
Second, Book 114 identifies $\omega_{\mathcal J/R}$ with the regular dualizing differentials on the modular curve, and Book 113 identifies these at good level with integral weight-two cusp forms. If a primitive differential remains nonzero modulo $r$, its expansion at a cusp is nonzero. Some coefficient is therefore a unit modulo $r$. A Hecke operator moves that coefficient into the first position.
...
Fix $r\ne N$. Since $J_0(N)$ has good reduction at $r$, Book 114 identifies its invariant differentials with regular dualizing differentials on the compactified modular curve:
...
At good level, logarithmic Kodaira--Spencer from Book 113 identifies the right side with the integral weight-two cusp-form lattice
...
First, the lattice $\omega_{\mathcal J/\mathbf Z_r}$ is defined intrinsically from the abelian scheme. Book 114 compares it with regular dualizing differentials, including the residue conditions at singular stack charts.

Second, primitivity comes from the exact sequence of abelian schemes, not from coefficientwise divisibility. After that integral fact is established, Book 113's q-expansion principle is used only to detect a nonzero section and a unit coefficient.
...
**Book 113.** Integral modular forms are global Hodge sections on the compactified modular stack; weight-two cusp forms are regular dualizing differentials; q-expansion at a cusp is injective on a geometrically integral component; and the away-level coefficient formula gives

$$
a_1(T_mf)=a_m(f)
$$

for $(m,N)=1$. We prove the $U_N$ extension (5.6)--(5.7) in the incoming normalization. At $r=3$, the stack and stabilizer cautions of Book 113 are retained; a naive coarse q-lattice is never used.
