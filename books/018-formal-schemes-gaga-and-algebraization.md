...
9. [Grothendieck existence for coherent sheaves](#9-grothendieck-existence-for-coherent-sheaves)
   ...
   - [Essential surjectivity in the projective case](#93-essential-surjectivity-in-the-projective-case)
   - [Passage from projective to proper](#94-passage-from-projective-to-proper)
   - [What the theorem does and does not say](#95-what-the-theorem-does-and-does-not-say)
...
Book 8 supplies relative Proj, ampleness, and projective embeddings. Book 15 supplies proper coherent cohomology and the theorem on formal functions. Book 62 supplies the algebra of separated complete rings and finite modules. We recall the interfaces precisely when they are used.
...
### 9.3 Essential surjectivity in the projective case

Assume first that $X$ is projective over $A$ and choose a relatively ample line bundle $\mathcal O_X(1)$. Let $\mathfrak F$ be coherent on $\widehat X$. The strategy is to manufacture an algebraic presentation of $\mathfrak F$ from sufficiently positive twists.

Formal Serre vanishing gives an integer $m_0$ such that, for $m\geq m_0$, the sheaf $\mathfrak F(m)$ is generated by finitely many formal global sections and has no higher cohomology in the range needed for the presentation. Proper formal functions makes the module of formal sections finite over $A$. Choose generators; they define a surjection

$$
\widehat{\mathcal O_X}(-m)^{\oplus r}\twoheadrightarrow\mathfrak F.
$$

Its kernel is coherent. Twist the kernel positively and repeat to obtain a finite presentation

$$
\widehat{\mathcal O_X}(-m_1)^{\oplus s}
\longrightarrow
\widehat{\mathcal O_X}(-m_0)^{\oplus r}
\longrightarrow\mathfrak F\longrightarrow0.
$$

Full faithfulness algebraizes the first arrow to a map of algebraic vector bundles on $X$. Define $\mathcal F$ as its cokernel. Exactness of completion then gives $\widehat{\mathcal F}\simeq\mathfrak F$.

The decisive point is uniformity. It would not suffice to generate each $\mathfrak F_n$ by a number or twist depending on $n$. Properness and ampleness give one bound and one finite module of sections for the entire tower.

### 9.4 Passage from projective to proper
...
### 9.5 What the theorem does and does not say
...
Choose $d$ large enough that $\mathfrak L^d$ is generated by formal sections and gives a closed immersion at every level. Formal Serre vanishing makes
