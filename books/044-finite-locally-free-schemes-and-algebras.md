...
1. A quotient $A/I$ is finite for every ideal $I$. It is flat precisely when the map $I\otimes_AN\to N$ induced by $I\hookrightarrow A$ is injective for every $A$-module $N$. If $I=Ae$ for an idempotent $e$, then $A/I\cong A(1-e)$ is projective and hence flat. Conversely, a finitely presented flat quotient arises this way.
...
3. The module $P$ is finitely presented and, for every prime $\mathfrak p$, $P_{\mathfrak p}$ is a finite free $A_{\mathfrak p}$-module.
...
Localizing at $\mathfrak p$ proves (2)$\Rightarrow$(3). Conversely, suppose (3) holds. A basis of $P_{\mathfrak p}$ lifts, after clearing denominators, to a map $A^r\to P$. Its cokernel is finitely generated and vanishes at $\mathfrak p$, so after passing to a principal neighborhood of $\mathfrak p$ the map is surjective. Over that neighborhood its kernel is finitely generated because the localized module $P$ is finitely presented. The kernel vanishes at $\mathfrak p$, so after one more shrinking the map is an isomorphism. Quasi-compactness then gives (4).

If (4) holds, $P$ is finitely presented by the local criterion and flat because flatness can be checked after localization: an injection $N'\hookrightarrow N$ remains injective after tensoring with every $P_{f_i}$, hence after tensoring with $P$ since vanishing is detected on the cover. It remains to recover projectivity rather than only local freeness. Because $P$ is now finitely presented, each local dual basis on $D(f_i)$ may be cleared of denominators: for some exponent $N_i$, multiplication by $f_i^{N_i}$ on $P$ is a finite sum of operators $p\mapsto\lambda(p)q$ with $q\in P$ and $\lambda\in P^\vee$. Increasing the exponents if necessary, the elements $f_i^{N_i}$ generate the unit ideal. A linear combination equal to $1$ combines these local formulas into a global finite dual basis. The dual-basis criterion below then makes $P$ a direct summand of a finite free module. Thus (4)$\Rightarrow$(1), and all four conditions are equivalent. $\square$
...
Let $M$ be finitely presented, with a presentation $A^m\xrightarrow{R}A^n\to M\to0$. The $i$-th Fitting ideal $\operatorname{Fitt}_i(M)$ is generated by the $(n-i)\times(n-i)$ minors of $R$, with the conventions that minors of size at most zero generate $A$ and impossible minors generate $0$. This ideal is independent of the presentation.

Here is the comparison argument. Replacing bases multiplies $R$ on the left or right by an invertible matrix, so Cauchy–Binet leaves every determinantal ideal unchanged. Adding a relation already generated by the old relations adds a column that is a linear combination of old columns and changes no determinantal ideal. Adding one generator together with a relation identifying it with a chosen linear combination of the old generators adds a row and a column with a single new unit entry; expansion along that entry shifts the size of the relevant minors exactly once and again leaves $\operatorname{Fitt}_i$ unchanged. Any two finite module presentations are connected by finitely many such moves: add lifts of the second set of generators to the first presentation, add lifts of its defining relations, and then perform the symmetric operation in the other direction. These are the module-presentation analogues of adding redundant generators and relations. Hence the ideals depend only on $M$.
...
If rank is only locally constant, the finitely many clopen rank strata carry characteristic polynomials of different degrees. They may be assembled through the corresponding product decomposition of $A$, but the result has no one globally meaningful monic degree. It is therefore cleaner to state characteristic-polynomial results separately on each $S_n(P)$.
...
Indeed a finite projective $A$-algebra $B$ is finitely presented as an $A$-algebra. To see this, choose module generators $b_1,\ldots,b_r$. The kernel of $A^r\to B$ is finite, and finitely many relations record it. Add the finitely many unit and multiplication-table relations expressing $1$ and every $b_ib_j$ in terms of the $b_k$. These relations reduce every polynomial to a linear combination of the variables and then impose exactly the kernel of $A^r\to B$, so they present $B$ as an algebra.

Conversely, suppose $B$ is finite as an $A$-module and finitely presented as an $A$-algebra. Choose finitely many module generators of $B$. They are integral by the determinant trick. Adjoin them one at a time using chosen monic equations; the resulting iterated monogenic algebra $B'$ is finite free as an $A$-module and admits a surjective $A$-algebra map $B'\twoheadrightarrow B$. Both $B'$ and $B$ are finitely presented as $A$-algebras, so the kernel of this surjection is finitely generated as a $B'$-ideal: lift a finite algebra presentation of $B$ through a finite presentation of $B'$ and compare their finitely generated relation ideals. Since $B'$ is finite over $A$, this kernel is finite as an $A$-module. The exact sequence

$$
0\longrightarrow\ker(B'\to B)\longrightarrow B'\longrightarrow B\longrightarrow0
$$

then gives a finite $A$-module presentation of $B$. If $B$ is also flat, Theorem 8.1 applies.
...
A finite locally free morphism whose geometric fibers are reduced is finite étale. This criterion is useful as orientation here; Chapter 17 proves it from differentials and then relates it to the trace pairing. For the moment, the transparent examples already show the boundary: constant finite schemes and spectra of finite separable field extensions are finite étale, while dual numbers and purely inseparable field extensions are not.
...
**Proof.** By definition, (2) and (3) are equivalent for the finite-type algebra $B$. Finite local freeness supplies finite presentation and flatness, so (2) implies that $B$ is étale. Conversely, an étale algebra is unramified by definition. Thus all three conditions are equivalent. $\square$

The theorem cleanly identifies the boundary: finite locally free geometry governs constant length, while finite étale geometry is the locus where that length consists geometrically of distinct reduced points.
...
This theorem proves the claim deferred in §10.7. It also turns the étale locus into an explicit open subset: it is the complement of the vanishing locus of the discriminant section. For a finite locally free family, ramification is therefore a closed condition, although its detailed multiplicities require a theory of the different beyond the present scope.
...
If every fiber map is injective, then $u$ is in fact a split injection locally on the base and its cokernel is finite projective. To prove this, work on a neighborhood where $P$ and $Q$ are free, of ranks $r$ and $s$. Fiberwise injectivity says that at each prime some $r\times r$ minor of the matrix of $u$ is invertible. On the principal open defined by that minor, projection to the corresponding $r$ coordinates followed by inversion of the minor gives a left inverse to $u$. These opens cover the base. The cokernel is therefore locally a direct summand of $Q$, so it is finite locally free. In particular, hidden torsion cannot survive injectivity on every fiber when both source and target are finite projective.

Equal-rank maps admit the sharper determinant test. On a constant-rank-$n$ stratum,
...
Open immersions require more geometry than closed equations. Suppose $U'\hookrightarrow X'$ is an open immersion and $\operatorname{Res}(X')$ is represented; this includes the affine case of Theorem 20.1. Over the representing scheme there is a universal map from its base change to $S'$ into $X'$. The inverse image of the closed complement $X'\setminus U'$ is closed in a scheme finite over the representing scheme, so its image is closed. Its complement represents exactly those maps that factor through $U'$. Thus $\operatorname{Res}(U')$ is an open subfunctor of $\operatorname{Res}(X')$. In the affine principal case $U'=D(g)$, the condition is that $g(c)$ be a unit in $R\otimes_AB$, equivalently that
...
Suppose $X'\to S'$ is quasi-projective and of finite presentation. Choose a locally closed immersion $X'\hookrightarrow\mathbf P^N_{S'}$. We first construct the restriction of projective space. Over $S=\operatorname{Spec}A$ and $S'=\operatorname{Spec}B$, an $R$-point of $\operatorname{Res}_{B/A}(\mathbf P^N_B)$ is an invertible quotient

$$
(R\otimes_AB)^{N+1}\twoheadrightarrow L
$$

as an $R\otimes_AB$-module. As an $R$-module, $L$ is a rank-$d$ direct summand, where $d$ is the rank of $B$ on the relevant clopen stratum. The Grassmannian construction of Book 8 represents rank-$d$ quotients of the finite projective $A$-module $B^{N+1}$. Requiring its universal kernel to be stable under multiplication by each member of a finite set of $A$-module generators of $B$ is a closed condition. On that closed locus the quotient is a finitely presented module over the pulled-back algebra. Its rank-one locally free locus is open by the Fitting-ideal criterion. The complement of this locus in the finite cover is closed, and its image on the Grassmannian is closed; removing that image forces the quotient to be invertible at every point of the finite cover. The resulting locally closed subscheme of the Grassmannian represents $\operatorname{Res}_{B/A}(\mathbf P^N_B)$ and is quasi-projective over $A$. The construction is independent of choices and glues over $S$.

Restriction preserves closed immersions by §20.3 and open immersions by §20.4, hence it preserves locally closed immersions. Therefore $\operatorname{Res}_{S'/S}(X')$ is represented by a locally closed subscheme of $\operatorname{Res}_{S'/S}(\mathbf P^N_{S'})$ and is quasi-projective over $S$. Finite presentation of $X'$ keeps the locally closed conditions finitely presented.
...
2. composition $X\to Y\to S$ preserves finite local freeness, and multiplies degrees when $X\to Y$ has constant degree on the part of $Y$ lying over the relevant component of $S$ and $Y\to S$ has constant degree there;
